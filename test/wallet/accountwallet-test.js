...
import FullNodeServices, {
  setFullNodeServicesURL,
} from "../../lib/http/fullnodeServices";
...
setFullNodeServicesURL({ url: "http://51.161.119.66:9001" });
...
async function TestCreateAndSendPrivacyTokenInit() {
  ...
  let senderPaymentAddressStr = senderKeyWallet.base58CheckSerialize(
    PaymentAddressType
  );
...
async function TestGetBalanceV2() {
  try {
    await sleep(5000);
    // sender key (private key)
    console.debug("AAAA");
    ...
    console.debug("BBBB");
    let senderKeyWallet = keyWallet.base58CheckDeserialize(senderPrivateKeyStr);
    console.debug("CCCCC", senderKeyWallet.KeySet.PrivateKey);
    await senderKeyWallet.KeySet.importFromPrivateKey(
      senderKeyWallet.KeySet.PrivateKey
    );
    console.debug("DDDD");
    let accountSender = new AccountWallet();
    accountSender.key = senderKeyWallet;
    return await accountSender.getBalanceV2();
  } catch (e) {
    console.log("Error when get balance: ", e);
  }
}
...
async function TestCreateAndSendReplacePrivacyTokenTransfer() {
  ...
  // try {
  let response1 = await accountSender.createAndSendPrivacyToken(
    paymentInfos,
    tokenParams,
    feePRV,
    feePToken,
  ...
  let response2 = await accountSender.replaceTx(
    response1.txId,
    newFee,
    newFeePToken,
    newInfo,
...
const main = async () => {
  try {
    ...
    const balance = await TestGetBalanceV2();
