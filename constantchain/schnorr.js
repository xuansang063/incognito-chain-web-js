...
class SchnPrivKey {
    ...
    getPublicKey(GGenerator, HGenerator) {
        ...
        let res = new Uint8Array(constants.COMPRESS_POINT_SIZE * 3);npm
    ...
    sign(data) {
        ...
        let t = (this.R == 0) ? (this.PK.G.mul(s1.toRed(ec.N).fromRed())) : (this.PK.G.mul(s1.toRed(ec.N).fromRed())).add(this.PK.H.mul(s2.toRed(ec.N).fromRed()));
        let E = hash(t, data);
