...
class AccountWallet {
  constructor() {
    ...
    this.measureAsyncFn = this.measureAsyncFn.bind(this);
    this.measureFn = this.measureFn.bind(this);
    this.rpc = null;
  ...
  async replaceTxPToken(
    ...
    let response;
    try {
      response = await this.rpc.sendRawTxCustomTokenPrivacy(
        b58CheckEncodeTx
      );
    } catch (e) {
      throw new CustomError(
  ...
  async stakerStatus() {
    ...
    let reps;
    try {
      reps = await this.rpc.getPublicKeyRole(
  ...
  getListSerialsNumberEncodeBase64({ listSerialsNumberStr, size }) {
  ...
  async checkKeyImages({ listOutputsCoins, shardId, tokenId }) {
    const listSerialsNumberStr = await this.measureAsyncFn(
      this.getListSerialsNumber,
      'timeCheckKeyImages.timeGetSNStr',
      {
        listOutputsCoins,
      }
    );
    const listSerialsNumberEncodeBase64 = this.measureFn(
      this.getListSerialsNumberEncodeBase64,
      'timeCheckKeyImages.timeGetListSNEncodeBase64',
      {
    ...
    if (listSerialsNumberEncodeBase64.length !== 0) {
      const listSerialNumberStatus = await this.measureAsyncFn(
        apiCheckKeyImages,
