...
import {
  ...
  MAX_INPUT_PER_TX,
  MAX_DEFRAGMENT_TXS, DEFAULT_INPUT_PER_TX,
} from '../tx/constants';
...
import {
  ...
  PRVID,
  PRVIDSTR,
  PercentFeeToReplaceTx, PrivacyVersion,
} from './constants';
...
class AccountWallet {
  constructor() {
    ...
    this.keyInfo = {};
    this.allKeyInfoV1 = {};
    this.version = PrivacyVersion.ver1;
  ...
  getPrefixKeyStorage() {
    return `PRIVACY-${this.version || PrivacyVersion.ver2}`;
  }
  ...
  async getListSpentCoinsStorage(tokenId) {
    try {
      if (this.storage) {
        const key = this.getPrefixKeyStorage() + this.getKeyListSpentCoinsByTokenId(tokenId);
        const listUnspentCoins = await this.getAccountStorage(key);
        return listUnspentCoins || [];
      }
    } catch (error) {
  ...
  async getAllUnspentCoinsV1() {
    ...
    const task = Object.keys(allKeyInfoV1).map((tokenId) => {
      return this.getUnspentCoinsV1ByTokenId({ tokenId, total: allKeyInfoV1[tokenId].total });
    })

    const listUnspentCoinsV1 = await Promise.all(task);

    console.log('SANG TEST: ', listUnspentCoinsV1)
  }
  ...
  async getUnspentCoinsV2(tokenId) {
    ...
    // tracking
    await this.setCoinsStorageV2({ value: this.coinsStorage, tokenId });
    return listUnspentCoinsMerged;
  }

  async getUnspentCoins(tokenId, version = 2) {
    try {
      switch (version) {
        case 1:
