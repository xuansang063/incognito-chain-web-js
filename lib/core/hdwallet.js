...
class KeyWallet {
  ...
  serialize(keyType, useLegacyEncoding = false) {
    ...
    let checksum = checkSumFirst4Bytes(keyBytes, useLegacyEncoding);
  ...
  base58CheckSerialize(keyType, useLegacyEncoding = false) {
    let serializedKey = this.serialize(keyType, useLegacyEncoding);
    // do not use legacy encoding
    return checkEncode(serializedKey, ENCODE_VERSION, useLegacyEncoding);
  }
  ...
  getPublicKeyCheckEncode(useLegacyEncoding = false) {
    return checkEncode(this.KeySet.PaymentAddress.Pk, ENCODE_VERSION, useLegacyEncoding);
  }
  ...
  static deserialize(bytes) {
    ...
    if (!cs1.equals(cs2)) {
      ...
      const checkSum = checkSumFirst4Bytes(bytes.slice(0, bytes.length - 4), true);
  ...
  static base58CheckDeserialize(str, useLegacyEncoding = false) {
    let bytes;
    try {
    ...
    return KeyWallet.deserialize(bytes, useLegacyEncoding);
  }

  static async getKeySetFromPrivateKeyStr(privateKeyStr, useLegacyEncoding = false) {
    let kw;
    try {
      kw = KeyWallet.base58CheckDeserialize(privateKeyStr, useLegacyEncoding);
    } catch (e) {
      throw e;
    }
    await kw.KeySet.importFromPrivateKey(kw.KeySet.PrivateKey);
    let paymentAddressStr = kw.base58CheckSerialize(PaymentAddressType, useLegacyEncoding);
