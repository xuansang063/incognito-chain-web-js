...
import { PaymentAddrSerializeSize } from "@lib/core/constants";
import bn from "bn.js";
import isArray from "lodash/isArray";

const isPaymentAddress = (paymentAddr) => {
  return true;
  let result = false;
  try {
    ...
    result = [
      ...
      PaymentAddrSerializeAddCheckSumSize + 33,
    ].includes(decodeBase58.length);
  } catch {
    //
  }
  return result;
...
class Validator {
  ...
  amount(message = "Invalid amount") {
    return this._onCondition(() => new bn(this.value).toNumber() >= 0, message);
  }
