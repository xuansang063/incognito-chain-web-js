...
import { KeyWallet, PRVIDSTR } from '../core';
...
import {
  ...
  MAX_INPUT_PER_TX, DEFAULT_INPUT_PER_TX,
} from '../tx/constants';
...
const prepareInputForTxV2 = async (
  ...
  inputVersion = 2,
  numOfOtherPks = 20,
  maxInputs = MAX_INPUT_PER_TX
) => {
  const unspentCoins = await account.getUnspentCoins(tokenID, inputVersion);
...
async function getMaxWithdrawAmount(
  ...
  try {
    let unspentToken = await account.getUnspentCoins(
...
const getUnspentCoinExceptSpendingCoin = async (account, coins) => {
  let unspentCoins = [...coins];
  try {
    ...
    const spendingCoins = await account.rpcCoinService.apiGetSpendingCoinInMemPool();
...
const getUnspentCoinExceptSpendingCoinV1 = async ({ account, tokenId, unspentCoins }) => {
  unspentCoins = cloneDeep(unspentCoins);
  try {
    const spendingCoinsStorage = await account.getSpendingCoinsV1ByTokenId({ tokenId });
    unspentCoins = unspentCoins.filter(
      (item) =>
        !spendingCoinsStorage?.find((coin) => coin?.SNDerivator === item?.SNDerivator)
    );
    const spendingCoins = await account.rpcCoinService.apiGetSpendingCoinInMemPool() || [];
