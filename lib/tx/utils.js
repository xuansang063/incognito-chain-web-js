...
import { PrivacyTokenParamTx, Wallet } from "../wallet/wallet";
 ...
 */
const prepareInputForTxPrivacyToken = async (tokenParams, account, rpcClient, feeToken) => {
...
const estimateFee = async (paymentAddrSerialize, numInputCoins, numOutputs, hasPrivacy, metadata, rpcClient, customTokenParams = null, privacyTokenParams = null, isGetTokenFee = false) => {
  let tokenIDStr = null;
  ...
  let txSize = estimateTxSize(numInputCoins, numOutputs, hasPrivacy, metadata, customTokenParams, privacyTokenParams);
 ...
 */
const getEstimateFee = async (from, to, amount, account, isPrivacy, rpcClient, customTokenParams = null, privacyTokenParams = null, isGetTokenFee = false) => {
  ...
  let inputForTx;
  try {
    inputForTx = await prepareInputForTx(amountBN, new bn(0), isPrivacy, tokenID, account, rpcClient);
  } catch (e) {
    throw e;
  }
  ...
  let fee;
  try {
    fee = await estimateFee(from, inputForTx.inputCoinStrs.length, paymentInfos.length, isPrivacy, null, rpcClient, customTokenParams, privacyTokenParams, isGetTokenFee);
  } catch (e) {
    throw e;
  }
...
const getEstimateFeeForSendingToken = async (from, to, amount, tokenObject, privatekeyStr, account, rpcClient, isPrivacyForPrivateToken, feeToken) => {
  ...
  if (tokenObject.Privacy === false) {
    ...
    try {
      inputForPrivacyCustomToken = await prepareInputForTxPrivacyToken(privacyCustomTokenParams, account, rpcClient, new bn(feeToken));
...
async function getEstimateTokenFee(from, to, amount, tokenObject, privatekeyStr, account, rpcClient, isPrivacyForPrivateToken) {
  ...
  if (tokenObject.Privacy === false) {
    ...
    let inputForPrivacyCustomToken;
    try {
      inputForPrivacyCustomToken = await prepareInputForTxPrivacyToken(privacyCustomTokenParams, account, rpcClient, new bn(0));
      privacyCustomTokenParams.tokenInputs = inputForPrivacyCustomToken.tokenInputs;
    } catch (e) {
      throw e;
    }
...
const estimateTxSize = (numInputCoins, numOutputCoins, hasPrivacy, metadata, customTokenParams, privacyCustomTokenParams) => {
  ...
  let sizeProof = PaymentProof.estimateProofSize(numInputCoins, numOutputCoins, hasPrivacy);
  ...
  if (privacyCustomTokenParams !== null && typeof privacyCustomTokenParams !== "undefined") {
    ...
    if (privacyCustomTokenParams.tokenInputs !== null) {
      privacyTokenDataSize += PaymentProof.estimateProofSize(privacyCustomTokenParams.tokenInputs.length, privacyCustomTokenParams.receivers.length, true);
    }
