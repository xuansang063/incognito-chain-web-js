...
const prepareInputForTx = async (amountTransfer, fee, hasPrivacy, tokenID, account, rpcClient) => {
  ...
  console.debug('unspentCoinStrs',unspentCoinStrs?.length)
  // remove spending coins from list of unspent coins
  const unspentCoinExceptSpendingCoin = await getUnspentCoinExceptSpendingCoin(unspentCoinStrs);
  console.debug('unspentCoinExceptSpendingCoin',unspentCoinExceptSpendingCoin?.length)
...
const prepareInputForTxPrivacyToken = async (tokenParams, account, rpcClient, feeToken, hasPrivacyForPToken) => {
  const paymentAddressStr = account.key.base58CheckSerialize(PaymentAddressType);
  let response;
  try {
...
const prepareInputForReplaceTxPrivacyToken = async (inCoinSerialNumbers, account, rpcClient, hasPrivacyForPToken, tokenID) => {
  // get all output coins
  const paymentAddrSerialize = account.key.base58CheckSerialize(PaymentAddressType);
  const readOnlyKeySerialize = account.key.base58CheckSerialize(ReadonlyKeyType);
...
const estimateFee = async (paymentAddrSerialize, numInputCoins, numOutputs, hasPrivacyForNativeToken, hasPrivacyForPToken, metadata, rpcClient, customTokenParams = null, privacyTokenParams = null, isGetTokenFee = false) => {
  ...
  const txSize = estimateTxSize(numInputCoins, numOutputs, hasPrivacyForNativeToken, hasPrivacyForPToken, metadata, customTokenParams, privacyTokenParams);
...
async function getMaxWithdrawAmount (from, to, tokenObject, account, rpcClient, isPrivacyForPrivateToken) {
  ...
  try {
    const unspentToken = await account.getUnspentToken(tokenParamJson.propertyID.toLowerCase(), rpcClient);
...
const estimateTxSize = (numInputCoins, numOutputCoins, hasPrivacyForNativeToken, hasPrivacyForPToken, metadata, customTokenParams, privacyCustomTokenParams) => {
  ...
  const sizeProof = estimateProofSize(numInputCoins, numOutputCoins, hasPrivacyForNativeToken);
...
const getUTXOsToDefragment = async (spendingKeyStr, fee, account, amount, rpcClient, coinId) => {
  ...
  if (uncachedOutputCoinStrs.length > 0) {
    const uncachedInputCoins = parseInputCoinFromEncodedObject(uncachedOutputCoinStrs, account.key, account.derivatorPointCached);
  ...
  const { unspentInputCoinsFromCached, unspentInputCoinsFromCachedStrs } = account.analyzeSpentCoinFromCached(inputCoins, allOutputCoinStrs);

  const resp = await getUnspentCoin(unspentInputCoinsFromCached, paymentAddrSerialize, unspentInputCoinsFromCachedStrs, null, rpcClient);
...
const getUnspentCoinExceptSpendingCoin = async (unspentCoins) => {
  ...
  console.debug('spendingCoins',spendingCoins?.length);
  if (spendingCoins.length && spendingCoins.length > 0) {
    let unspentCoinExceptSpendingCoin = cloneInputCoinJsonArray(unspentCoins);
    unspentCoinExceptSpendingCoin = unspentCoinExceptSpendingCoin.filter(coin => !spendingCoins.includes(coin.SerialNumber));
