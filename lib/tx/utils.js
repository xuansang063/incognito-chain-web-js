 ...
 */
// PRV fee
const getEstimateFeeForSendingToken = async (from, to, amount, tokenObject, account, rpcClient, isPrivacyForNativeToken, isPrivacyForPrivateToken, feeToken, isGetTokenFee = false) => {
  let id = "";
  let name = "";
  let symbol = "";
  ...
  console.log("BBB feeToken: ", feeToken);
...
//     let fee;
//     try {
//       fee = await getEstimateFee(from, to, amount, privatekeyStr, account, isPrivacyForNativeToken, isPrivacyForPrivateToken, rpcClient, null, privacyCustomTokenParams, true);
//     } catch (e) {
//       throw e;
//     }
//     return fee;
...
async function getMaxWithdrawAmount(from, to, tokenObject, privatekeyStr, account, rpcClient, isPrivacyForNativeToken, isPrivacyForPrivateToken) {
  let id = "";
  let name = "";
  let symbol = "";
  if (tokenObject.TokenID !== null) {
  ...
  let fee;
  try {
    fee = await getEstimateFee(from, to, 0, privatekeyStr, account, isPrivacyForNativeToken, isPrivacyForPrivateToken, rpcClient, null, privacyCustomTokenParams, true);
  } catch (e) {
    throw e;
  }
...
const estimateTxSize = (numInputCoins, numOutputCoins, hasPrivacyForNativeToken, hasPrivacyForPToken, metadata, customTokenParams, privacyCustomTokenParams) => {
  ...
  let sizeProof = PaymentProof.estimateProofSize(numInputCoins, numOutputCoins, hasPrivacyForNativeToken);
  ...
  if (privacyCustomTokenParams !== null && typeof privacyCustomTokenParams !== "undefined") {
    ...
    if (privacyCustomTokenParams.tokenInputs !== null) {
      privacyTokenDataSize += PaymentProof.estimateProofSize(privacyCustomTokenParams.tokenInputs.length, privacyCustomTokenParams.receivers.length, hasPrivacyForPToken);
    }
