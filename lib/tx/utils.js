...
async function getMaxWithdrawAmount(from, to, tokenObject, account, rpcClient, isPrivacyForNativeToken, isPrivacyForPrivateToken) {
  let id = "";
  let name = "";
  let symbol = "";
  if (tokenObject.TokenID !== null) {
  ...
  let fee;
  try {
    fee = await getEstimateFee(from, to, 0, account, isPrivacyForNativeToken, isPrivacyForPrivateToken, rpcClient, null, privacyCustomTokenParams, true);
  } catch (e) {
    throw e;
  }
