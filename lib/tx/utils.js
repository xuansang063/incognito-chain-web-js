...
import { apiGetSpendingCoinInMemPool } from '../http/coinsServices';
...
const prepareInputForReplaceTxNormal = async (
  inCoinSerialNumbers,
  hasPrivacy,
  tokenID,
  account,
  ...
  const paymentAddrSerialize = account.key.base58CheckSerialize(
    PaymentAddressType
  );
  ...
  const inputCoinsToSpent = new Array(inCoinSerialNumbers.length);
  for (let i = 0; i < inputCoinsToSpent.length; i++) {
    ...
    inputCoinsToSpent[i].SerialNumber =
      account.derivatorToSerialNumberCache[sndStr];
  }
  ...
  if (hasPrivacy) {
    let response;
    try {
      response = await rpcClient.randomCommitmentsProcess(
...
const prepareInputForTx = async (
  ...
  const unspentCoinStrs = await account.getUnspentToken(tokenID);
  console.log('unspentCoinStrs', unspentCoinStrs);
  // remove spending coins from list of unspent coins
  const unspentCoinExceptSpendingCoin = await getUnspentCoinExceptSpendingCoin(
    unspentCoinStrs
  );
  console.log('unspentCoinExceptSpendingCoin', unspentCoinExceptSpendingCoin);
  // total amount transfer and fee
  amountTransfer = amountTransfer.add(fee);
  const respChooseBestCoin = chooseBestCoinToSpent(
    unspentCoinExceptSpendingCoin,
    amountTransfer
  );
  console.log('respChooseBestCoin', respChooseBestCoin);
  const inputCoinsToSpent = respChooseBestCoin.resultInputCoins;
...
const prepareInputForDefragments = async (
  coinId,
  account,
  rpcClient,
  noInputPerTx
  ...
  const unspentCoinStrs = await account.getUnspentToken(coinId);
  // remove spending coins from list of unspent coins
  const unspentCoinExceptSpendingCoin = await getUnspentCoinExceptSpendingCoin(
    unspentCoinStrs
  );
  const sortedUnspentCoins = _.orderBy(
    unspentCoinExceptSpendingCoin,
    (item) => item.Value
  );
  ...
  Wallet.Debug = 'Starting random commitment...';
  let index = 1;
  for (const part of parts) {
    Wallet.Debug = `Randomizing commitment ${index}...`;
    const result = await randomCommitment(
      1,
      true,
      coinId,
      account,
  ...
  Wallet.Debug = 'Ending random commitment...';

  return results;
};

const randomCommitment = async (
  amountTransfer,
  hasPrivacy,
  tokenID,
  account,
 ...
 */
const prepareInputForTxPrivacyToken = async (
  tokenParams,
  account,
  rpcClient,
  feeToken,
  ...
  const paymentAddressStr = account.key.base58CheckSerialize(
    PaymentAddressType
  );
  let response;
  try {
  ...
  const listPrivacyToken = response.listPrivacyToken;
  const tokenId = tokenParams.propertyID.toLowerCase();
  ...
  switch (tokenParams.tokenTxType) {
    ...
    case CustomTokenTransfer: {
      ...
      for (let i = 0; i < tokenParams.paymentInfoForPToken.length; i++) {
        amountTokenPrivacyOutput = amountTokenPrivacyOutput.add(
          new bn(tokenParams.paymentInfoForPToken[i].amount)
        );
      }
      if (feeToken) {
      ...
      try {
        ...
        unspentCoinStrs = await account.getUnspentToken(tokenId, rpcClient);
      } catch (e) {
        throw new CustomError(
          ErrorObject.GetUnspentPrivacyTokenErr,
          e.message || e.Message || 'Can not get unspent privacy token'
        );
      }
      ...
      let tokenInputs;
      try {
        tokenInputs = chooseBestCoinToSpent(
          unspentCoinExceptSpendingCoin,
          amountTokenPrivacyOutput
        ).resultInputCoins;
      } catch (e) {
        throw e;
      }
      ...
      if (hasPrivacyForPToken) {
        ...
        try {
          response = await rpcClient.randomCommitmentsProcess(
            paymentAddressStr,
            tokenInputs,
            tokenParams.propertyID.toLowerCase()
          );
        } catch (e) {
          throw e;
        }
        ...
        if (myCommitmentIndices.length !== tokenInputs.length) {
          throw new Error(
            'pToken Number of list my commitment indices must be equal to number of input coins'
          );
        }
      }
 ...
 * @param {bool} hasPrivacyForPToken
 */
const prepareInputForReplaceTxPrivacyToken = async (
  inCoinSerialNumbers,
  account,
  rpcClient,
  hasPrivacyForPToken,
  ...
  const readOnlyKeySerialize = account.key.base58CheckSerialize(
    ReadonlyKeyType
  );
  ...
  const listPrivacyToken = resGetOutputCoins.listPrivacyToken;
  ...
  // get coin detail of input coins to spent
  const tokenInputs = new Array(inCoinSerialNumbers.length);
  for (let i = 0; i < tokenInputs.length; i++) {
...
const cloneInputCoinJsonArray = (inputCoinsJson) => {
  const inputCoinsClone = new Array(inputCoinsJson.length);

  for (let i = 0; i < inputCoinsClone.length; i++) {
    const object = new Coin();
...
const estimateFee = async (
  ...
  metadata,
  rpcClient,
  customTokenParams = null,
  privacyTokenParams = null,
  isGetTokenFee = false
) => {
  let tokenIDStr = null;
  ...
  let unitFee;
  try {
    unitFee = await rpcClient.getEstimateFeePerKB(
      paymentAddrSerialize,
      tokenIDStr
    );
  } catch (e) {
...
const getEstimateFee = async (
  ...
  const receiverKeyWallet = keyWallet.base58CheckDeserialize(to);
  let paymentInfos = [];
  ...
  if (customTokenParams == null && privacyTokenParams == null) {
    ...
    paymentInfos[0] = new PaymentInfo(
      receiverKeyWallet.KeySet.PaymentAddress,
      new bn(amount)
    );
 ...
 */
// PRV fee
const getEstimateFeeForPToken = async (
  from,
  to,
  amount,
  tokenObject,
  ...
  try {
    inputForPrivacyToken = await prepareInputForTxPrivacyToken(
      privacyTokenParam,
      account,
      rpcClient,
      new bn(feeToken),
...
async function getMaxWithdrawAmount(
  ...
  try {
    const unspentToken = await account.getUnspentToken(
  ...
  let isRatePToken;
  try {
    isRatePToken = await Wallet.RpcClient.isExchangeRatePToken(
...
const estimateTxSize = (
  ...
  const sizeVersion = 1; // int8
  const sizeType = 5; // string, max : 5
  const sizeLockTime = 8; // int64
  const sizeFee = 8; // uint64

  let sizeInfo = 0;
  if (hasPrivacyForNativeToken) {
    sizeInfo = 64;
  }
  const sizeSigPubKey = SIG_PUB_KEY_SIZE;
  let sizeSig = SIG_NO_PRIVACY_SIZE;
  if (hasPrivacyForNativeToken) {
    sizeSig = SIG_PRIVACY_SIZE;
  }

  const sizeProof = estimateProofSize(
    numInputCoins,
    numOutputCoins,
    hasPrivacyForNativeToken
  );

  const sizePubKeyLastByte = 1;

  const sizeMetadata = 0;
  ...
  if (customTokenParams !== null && typeof customTokenParams !== 'undefined') {
    ...
    for (let i = 0; i < customTokenParams.receivers.length; i++) {
      customTokenDataSize += customTokenParams.receivers[
        i
      ].paymentAddress.toBytes().length;
      customTokenDataSize += 8;
    }

    if (customTokenParams.vins !== null) {
      for (let i = 0; i < customTokenParams.vins.length; i++) {
        customTokenDataSize += customTokenParams.vins[
          i
        ].paymentAddress.toBytes().length;
        customTokenDataSize += customTokenParams.vins[i].txCustomTokenID.slice(
          0
        ).length;
  ...
  if (
    privacyCustomTokenParams !== null &&
    typeof privacyCustomTokenParams !== 'undefined'
  ) {
    ...
    if (privacyCustomTokenParams.tokenInputs !== null) {
      privacyTokenDataSize += estimateProofSize(
        privacyCustomTokenParams.tokenInputs.length,
        privacyCustomTokenParams.paymentInfoForPToken.length,
        hasPrivacyForPToken
      );
    }
...
// @@@NOTE: for defragment feature
const getUTXOsToDefragment = async (
  spendingKeyStr,
  fee,
  account,
  amount,
  ...
  const paymentAddrSerialize = myKeyWallet.base58CheckSerialize(
    PaymentAddressType
  );
  const readOnlyKeySerialize = myKeyWallet.base58CheckSerialize(
    ReadonlyKeyType
  );
  ...
  const {
    ...
    cachedInputCoins,
  } = account.analyzeOutputCoinFromCached(allOutputCoinStrs);
  ...
  if (uncachedOutputCoinStrs.length > 0) {
    const uncachedInputCoins = parseInputCoinFromEncodedObject(
      uncachedOutputCoinStrs,
      account.key,
      account.derivatorPointCached
    );
    account.mergeDerivatorCached();
    account.mergeInputCoinJsonCached(
      uncachedOutputCoinStrs,
      uncachedInputCoins
    );
  ...
  const {
    unspentInputCoinsFromCached,
    unspentInputCoinsFromCachedStrs,
  } = account.analyzeSpentCoinFromCached(inputCoins, allOutputCoinStrs);

  const resp = await getUnspentCoin(
    unspentInputCoinsFromCached,
    paymentAddrSerialize,
    unspentInputCoinsFromCachedStrs,
    null,
  ...
  const defragmentUTXO = [];
  const defragmentUTXOStr = [];
  ...
  if (totalAmount.cmp(new bn(0)) == -1) {
    throw new CustomError(
      ErrorObject.InvalidNumberUTXOToDefragment,
      'No UTXO has value less than amount defragment'
    );
  }
...
const getUnspentUTXOs = async (account, rpcClient, coinId) => {
  ...
  if (uncachedOutputCoinStrs.length > 0) {
    const res = await account.deriveSerialNumbers(
      spendingKeyStr,
      uncachedOutputCoinStrs,
      coinId
    );
  ...
  const {
    unspentInputCoinsFromCachedStrs,
  } = account.analyzeSpentCoinFromCached(allOutputCoinStrs, coinId);
...
const getUTXOsExceptSpendingCoin = (unspentCoinStrs, account) => {
  ...
  if (account.spendingCoins && account.spendingCoins.length) {
    ...
    const UTXOExceptSpendingCoinStrs = unspentCoinStrs.filter(
      (coin) => !spendingSNs.includes(coin.SerialNumber)
    );
...
const getUnspentCoinExceptSpendingCoin = async (unspentCoins) => {
  const spendingCoins = await apiGetSpendingCoinInMemPool();
  if (spendingCoins.length && spendingCoins.length > 0) {
    ...
    unspentCoinExceptSpendingCoin = unspentCoinExceptSpendingCoin.filter(
      (coin) => !spendingCoins.includes(coin.SerialNumber)
    );
...
const getUnspentCoin = async (
  ...
  const unspentCoinStrs = new Array();
  const serialNumberStrs = new Array();
  ...
  const existed = response.existed;
...
function newParamInitPrivacyTokenTx(
  senderSkStr,
  paramPaymentInfos,
  inputCoinStrs,
  fee,
  ...
  metaData,
  info,
  commitmentIndicesForNativeToken,
  myCommitmentIndicesForNativeToken,
  commitmentStrsForNativeToken,
  sndOutputsForNativeToken,
  commitmentIndicesForPToken,
  myCommitmentIndicesForPToken,
  commitmentStrsForPToken,
  sndOutputsForPToken
) {
  const param = {
    senderSK: senderSkStr,
