...
// getUnspentPrivacyToken returns all unspent privacy token of spendingKeyStr corressponding to tokenParams.propertyID.toLowerCase()
const getUnspentPrivacyToken = async (spendingKeyStr, tokenIDStr, account, rpcClient) => {
  // tokenParams for tx custom token privacy data, but haven't tokenParam's tokenInputs
...
const chooseBestCoinToSpent = (inputCoins, amount) => {
  ...
  if (amount.cmp(new bn(0)) === 0) {
    return {
      resultInputCoins: [],
      remainInputCoins: cloneInputCoins,
  ...
  let inCoinsUnderAmount = [];
	...
	for (let i = 0; i< cloneInputCoins.length; i++) {
		if (cloneInputCoins[i].coinDetails.value.cmp(amount) === -1) {
			inCoinsUnderAmount.push(cloneInputCoins[i]);
		} else if (inCoinOverAmount === null) {
			inCoinOverAmount = cloneInputCoins[i];
		} else if (inCoinOverAmount.coinDetails.value.cmp(cloneInputCoins[i].coinDetails.value) === 1) {
			remainInputCoins.push(cloneInputCoins[i]);
		} else {
...
const estimateFee = async (paymentAddrSerialize, inputCoins, payments, hasPrivacy, metadata, rpcClient, customTokenParams = null, privacyCustomTokenParams = null, isGetTokenFee = false) => {
  ...
  console.log("tokenIDStr AAAAAA: ", tokenIDStr)
...
async function getMaxWithdrawAmount(from, to, tokenObject, privatekeyStr, account, rpcClient, isPrivacyForPrivateToken){
  ...
  try {
    let resp = await getUnspentPrivacyToken(privatekeyStr, privacyCustomTokenParams.propertyID.toLowerCase(), account, rpcClient);
  ...
  let maxWithdrawAmount = totalpTokenAmount.sub(new bn(2*fee));
  return maxWithdrawAmount.toNumber();
}
