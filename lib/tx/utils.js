...
import { estimateProofSize } from 'privacy-js-lib/lib/zkps/utils'
...
const prepareInputForTx = async (spendingKeyStr, paymentInfos, fee, account, rpcClient) => {
  ...
  let resp = await rpcClient.getOutputCoin(paymentAddrSerialize, readOnlyKeySerialize);
  ...
  // let resGetUnspentCoin = await Wallet.RpcClient.getUnspentCoin(unspentInputCoinsFromCached, paymentAddrSerialize, unspentInputCoinsFromCachedStrs);
  let { unspentCoins } = await getUnspentCoin(unspentInputCoinsFromCached, paymentAddrSerialize, unspentInputCoinsFromCachedStrs, rpcClient);
  ...
  let inputCoinsToSpentStr = rpcClient.parseInputCoinToEncodedObject(inputCoinsToSpent);
...
const prepareInputForCustomTokenTx = async (spendingKeyStr, tokenParams, rpcClient) => {
  ...
  switch (tokenParams.tokenTxType) {
    ...
    case CustomTokenTransfer: {
      ...
      try {
        response2 = await rpcClient.getUnspentCustomToken(senderKeyWallet.base58CheckSerialize(PaymentAddressType),
...
const prepareInputForTxCustomTokenPrivacy = async (spendingKeyStr, tokenParams, account, rpcClient) => {
  ...
  let listUnspentToken;
  try {
    listUnspentToken = await rpcClient.listPrivacyCustomTokens();
  } catch(e){
    throw e;
  }
  ...
  switch (tokenParams.tokenTxType) {
    ...
    case CustomTokenTransfer: {
      ...
      let res = await rpcClient.getOutputCoin(paymentAddrSerialize, readOnlyKeySerialize, tokenParams.propertyID.toLowerCase());
      let allOutputCoinStrs;
...
const parseInputCoinToEncodedObject = (coins) => {
  ...
  for (let i = 0; i < coinStrs.length; i++) {
    ...
    coinStrs[i].PublicKey = checkEncode(coins[i].coinDetails.publicKey.compress(), ENCODE_VERSION);
    coinStrs[i].CoinCommitment = checkEncode(coins[i].coinDetails.coinCommitment.compress(), ENCODE_VERSION);
    coinStrs[i].SNDerivator = checkEncode(coins[i].coinDetails.snderivator.toArray(), ENCODE_VERSION);
    coinStrs[i].Randomness = checkEncode(coins[i].coinDetails.randomness.toArray(), ENCODE_VERSION);
    coinStrs[i].SerialNumber = null;
    coinStrs[i].Value = coins[i].coinDetails.value;
    coinStrs[i].Info = checkEncode(coins[i].coinDetails.info, ENCODE_VERSION);
  }
...
const estimateFee = async (paymentAddrSerialize, inputCoins, payments, hasPrivacy, metadata, customTokenParams, privacyCustomTokenParams, rpcClient) => {
  let unitFee;
  try{
...
const getEstimateFee = async (from, to, amount, privatekeyStr, customTokenParams = null, privacyCustomTokenParams = null, account, isPrivacy, rpcClient) => {
  ...
  let fee;
  try {
    fee = await estimateFee(from, inputForTx.inputCoins, paymentInfos, isPrivacy, null, customTokenParams, privacyCustomTokenParams, rpcClient);
  } catch (e) {
    throw e;
  }
 ...
 */
const getEstimateFeeForSendingToken = async (from, to, amount, tokenObject, privatekeyStr, account, rpcClient) => {
  let id = "";
  let name = "";
  let symbol = "";
  ...
  if (tokenObject.Privacy === false) {
    ...
    let fee;
    try {
      fee = await getEstimateFee(from, to, amount, privatekeyStr, customTokenParams, null, account, rpcClient);
    } catch (e) {
      throw e;
    }
    return fee;
    ...
    let fee;
    try {
      fee = await getEstimateFee(from, to, amount, privatekeyStr, null, privacyCustomTokenParams, account, rpcClient);
    } catch (e) {
      throw e;
    }
    return fee;
...
const getEstimateFeeToDefragment = async (from, amount, privatekeyStr, account, isPrivacy, rpcClient) => {
  ...
  let fee;
  try {
    fee = await estimateFee(from, defragmentUTXO, paymentInfos, isPrivacy, null, null, null, rpcClient);
  } catch (e) {
    throw e;
  }
  return fee;
...
const estimateTxSize = (inputCoins, payments, hasPrivacy, metadata, customTokenParams, privacyCustomTokenParams) => {
  ...
  let sizeProof = estimateProofSize(inputCoins.length, payments.length, hasPrivacy);
  ...
  if (privacyCustomTokenParams !== null && typeof privacyCustomTokenParams !== "undefined") {
    ...
    if (privacyCustomTokenParams.tokenInputs !== null) {
      customTokenDataSize += estimateProofSize(privacyCustomTokenParams.tokenInputs.length, privacyCustomTokenParams.receivers.length, true);
    }
...
const getUTXOsToDefragment = async (spendingKeyStr, fee, account, amount, rpcClient) => {
  ...
  let { unspentCoins, unspentCoinStrs } = await getUnspentCoin(unspentInputCoinsFromCached, paymentAddrSerialize, unspentInputCoinsFromCachedStrs, rpcClient);
...
const getUnspentCoin = async (inputCoins, paymentAddrSerialize, inCoinStrs, tokenID, rpcClient) => {
  ...
  for (let i = 0; i < inputCoins.length; i++) {
    serialNumberStrs[i] = checkEncode(inputCoins[i].coinDetails.serialNumber.compress(), 0x00);
  }

  // check whether each input coin is spent or not
  let res = await rpcClient.hasSerialNumber(paymentAddrSerialize, serialNumberStrs, tokenID);
  let existed = [];
  if (res.err !== null) {
