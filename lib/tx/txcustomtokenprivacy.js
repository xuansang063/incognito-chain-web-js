...
class TxCustomTokenPrivacy extends Tx {
    ...
    async init(input, paymentInfos, fee, tokenParams, listCustomTokens, metaData, hasPrivacy) {
        // create normal tx for fee
        await super.init(input, paymentInfos, fee, hasPrivacy, null, metaData);
        ...
        switch (tokenParams.tokenTxType) {
            case constantsTx.CustomTokenInit: {
                ...
                txNormal.proof.outputCoins[0].coinDetails.publicKey = P256.decompress(tokenParams.receiver[0].PaymentAddress.Pk);
                ...
                // get last byte
                txNormal.pubKeyLastByteSender = tokenParams.receiver[0].PaymentAddress.Pk[tokenParams.receiver[0].PaymentAddress.Pk.length-1];

                // sign Tx
                txNormal.SigPubKey = tokenParams.receiver[0].PaymentAddress.Pk;
                txNormal.sigPrivKey = senderSK;
                let err = txNormal.sign();
                if (err !== null) {
                    return new Error("Can not sign tx nornal in tx custom token data");
                }
                ...
                for (let i =0; i<listCustomTokens.length; i++) {
                    if (convertHashToStr(hashTxTokenPrivacyData) === listCustomTokens[i].ID) {
                        return new Error("Custom token privacy is existed");
                    }
                }

                this.txTokenPrivacyData.propertyID = hashTxTokenPrivacyData;
            ...
            case constantsTx.CustomTokenTransfer: {
                ...
                for (i=0;i<listCustomTokens.length; i++){
                    if (listCustomTokens[i].ID === convertHashToStr(propertyID)){
                        break;
                    }
                }
                ...
                let err = txNormal.init(input, tokenParams.receiver, fee, true, propertyID, null);
                if (err !==null)
                {
                    return new Error("Can not create normal tx for tx custom token privacy data")
                }

                this.txTokenPrivacyData.set(txNormal, propertyID, tokenParams.propertyName, tokenParams.propertySymbol, tokenParams.tokenTxType, false, 0);
            }
        }
        if (!handled) {
