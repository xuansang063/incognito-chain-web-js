  ...
  async init(senderSK, paymentAddressStr, paymentInfo, inputCoins, inputCoinStrs, fee, tokenParams, listCustomTokens, metaData, hasPrivacy) {
    ...
    switch (tokenParams.tokenTxType) {
      case constantsTx.CustomTokenInit: {
        ...
        hashTxTokenPrivacyData = privacyUtils.hashSha3BytesToBytes(hashTxTokenPrivacyData);

        // validate PropertyID is the only one
        for (let i = 0; i < listCustomTokens.length; i++) {
          let tokenIDStr = convertHashToStr(hashTxTokenPrivacyData);
          console.log("TEST PRIVACY TOKEN ID: new token id: ", tokenIDStr);
          console.log("TEST PRIVACY TOKEN ID: old token id: ", i, " : ", listCustomTokens[i].ID)

          if (tokenIDStr.toLowerCase() === listCustomTokens[i].ID.toLowerCase()) {
      ...
      case constantsTx.CustomTokenTransfer: {
        ...
        // let propertyID = privacyUtils.hashSha3BytesToBytes(tokenParams.propertyID);
        let i = 0;
        ...
        let err = await txNormal.init(senderSK, paymentAddressStr, tokenParams.receivers, tokenParams.tokenInputs, tokenInputsStr, new bn(0), true, tokenParams.propertyID, null);
        if (err !== null) {
