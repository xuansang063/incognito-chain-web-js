...
class TxCustomTokenPrivacy extends Tx {
  ...
  async init(senderSK, paymentAddressStr, paymentInfo, inputCoins, inputCoinStrs, fee, tokenParams, listCustomTokens, metaData, hasPrivacyForToken) {
    // create normal tx for fee
    await super.init(senderSK, paymentAddressStr, paymentInfo, inputCoins, inputCoinStrs, fee, false, null, metaData);
    ...
    switch (tokenParams.tokenTxType) {
      ...
      case CustomTokenTransfer: {
        ...
        let err = await txNormal.init(senderSK, paymentAddressStr, tokenParams.receivers, tokenParams.tokenInputs, tokenInputsStr, new bn(0), hasPrivacyForToken, tokenParams.propertyID, null);
        if (err !== null) {
