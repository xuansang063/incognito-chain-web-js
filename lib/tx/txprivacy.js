...
class Tx {
  ...
  async init(senderSK, paymentAddressStr, paymentInfo, inputCoins, inputCoinStrs, fee, hasPrivacy, tokenID, metaData) {
    ...
    let inputCoinStrsClone = [...inputCoinStrs];
    ...
    if (hasPrivacy) {
      let randCommitments = await this.rpcClient.randomCommitmentsProcess(paymentAddressStr, inputCoinStrsClone, tokenID);
      ...
      if (commitmentIndices.length !== inputCoinsClone.length * constants.CM_RING_SIZE) {
    ...
    witness.init(hasPrivacy, new bn(senderSK, 'be', constants.BIG_INT_SIZE), inputCoinsClone, outputCoins, pkLastByteSender, commitmentProving, commitmentIndices, myCommitmentIndices, fee);
