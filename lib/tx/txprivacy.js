...
class Tx {
  ...
  async init(input, paymentInfo, fee, hasPrivacy, db, tokenID, metaData) {
    ...
    if (inputCoins.length === 0 && fee.cmp(new bn(0)) === 0 && !hasPrivacy) {
    ...
    for (i = 0; i < paymentInfo.length; i++) {
      ...
      outputCoins[i].CoinDetails.Pk = P256.decompress(paymentInfo[i].PaymentAddress.PublicKey);
    ...
    if (hasPrivacy) {
      ...
      for (i = 0; i < this.Proof.inputCoins.length; i++) {
        ...
        this.Proof.inputCoins[i].CoinDetails.Pk = null;
