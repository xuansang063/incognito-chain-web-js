...
class Tx {
  ...
  toString = () => {
    ...
    if (this.proof != null) {
      ...
      // console.log("this.proof.toBytes()", this.proof)
      // console.log("this.proof.toBytes()", this.proof.toBytes())
      // console.log("this.proof.toBytes()", this.proof.toBytes().join(", "))
      let tmp = base58.checkEncode(bytes, constants.PRIVACY_VERSION);
      // console.log("Proof base 58:", tmp);
      record += tmp
    }
  ...
  hash = () => {
    ...
    if (this.hashValue == null) {
      hash = privacyUtils.hashBytesToBytes(privacyUtils.stringToBytes(this.toString()));
      this.hashValue = hash;
    } else {
      hash = this.hashValue;
    }
