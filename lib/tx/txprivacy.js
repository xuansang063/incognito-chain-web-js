...
class Tx {
  ...
  async init(senderSK, paymentAddressStr, paymentInfo, inputCoins, inputCoinStrs, fee, hasPrivacy, tokenID, metaData) {
    ...
    let i;
    // set version tx
    this.version = constantsTx.TxVersion;
    ...
    while (ok) {
      ...
      for (i = 0; i < paymentInfo.length; i++) {
        ...
        let sndOutStrs = base58.checkEncode(sndOut.toArray(), ENCODE_VERSION);
        ...
        while (true) {
          ...
          if (res.existed[0]) {
            sndOut = privacyUtils.randScalar();
            sndOutStrs = base58.checkEncode(sndOut.toArray(), ENCODE_VERSION);
          } else {
            break
          }
        }
    ...
    witness.init(hasPrivacy, new bn(senderSK, 'be', constants.BIG_INT_SIZE), inputCoins, outputCoins, pkLastByteSender, commitmentProving, commitmentIndices, myCommitmentIndices);
  ...
  convertTxToByte() {
    ...
    if (this.proof !== null) {
      this.proof = base58.checkEncode(this.proof.toBytes(), ENCODE_VERSION);
    }
  ...
  toString = () => {
    ...
    if (this.proof != null) {
      ...
      let tmp = base58.checkEncode(bytes, ENCODE_VERSION);
    ...
    if (this.metadata != null) {
      ...
      tmp = privacyUtils.hashSha3BytesToBytes(tmp);
  ...
  hash = () => {
    ...
    if (this.hashValue == null) {
      ...
      hash = privacyUtils.hashSha3BytesToBytes(bytes);
      this.hashValue = hash;
    } else {
      hash = this.hashValue;
    }
