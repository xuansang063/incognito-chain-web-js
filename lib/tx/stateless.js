...
class StatelessTransactor {
    ...
    */
    ...
    async pdexv3TxContribute({ transfer: { fee, info = "", tokenID = null }, extra: { pairID, pairHash, contributedAmount, nftID = null, accessTicket = null, amplifier } = {}}) {
        await this.updateProgressTx(10, 'Generating Metadata');
        let burningAddress = await getBurningAddress(this.rpc);
        let burningPayments = [{
            PaymentAddress: burningAddress,
        ...
        if (!!accessTicket) metadata.NextOTA = accessTicket;
    ...
    async pdexv3TxAddOrder({ transfer: { fee, info = "" }, extra: { tokenIDToSell, tokenIDToBuy, poolPairID, sellAmount, minAcceptableAmount, nftID = null, accessTicket = null }}) {
        await this.updateProgressTx(10, 'Generating Metadata');
        let burningAddress = await getBurningAddress(this.rpc);
        let burningPayments = [{
            PaymentAddress: burningAddress,
    ...
    async pdexv3TxStake({ transfer: { fee, info = "", tokenID = PRVIDSTR }, extra: { tokenAmount, nftID = null, accessTicket = null }}) {
        await this.updateProgressTx(10, 'Generating Metadata');
        let burningAddress = await getBurningAddress(this.rpc);
        let burningPayments = [{
            PaymentAddress: burningAddress,
    ...
    async pdexv3GetAccessTickets(tokenID = PDEX_ACCESS_ID, useBase58Encoding = false) {
        ...
        utxos.forEach(c => {
            try {
                ...
                const decode = useBase58Encoding ? (m => checkDecode(m).bytesDecoded) : base64Decode;
                const decoded = bytesToString(decode(c.Info));
                const k = new AccessTicketChooser(null, useBase58Encoding).getAccessTicket(c);
                result[k] = c;
                return decoded.startsWith(PdexAccessTicketConfig.memo);
            } catch(e){
                throw e;
            }
        });
    ...
    async pdexv3TxWithdrawOrder_accessOTA({ transfer: { fee, tokenID = PDEX_ACCESS_ID, info = "" }, extra: { withdrawTokenIDs, poolPairID, orderID, amount, accessTicket, nextAccessTicket, useBase58Encoding = false }}) {
        await this.updateProgressTx(10, 'Generating Metadata');
        let burningAddress = await getBurningAddress(this.rpc);
        let burningPayments = [{
            PaymentAddress: burningAddress,
        ...
        let receivingTokens = withdrawTokenIDs;
        let receiver = {};
        // create new OTAs
        receivingTokens.forEach(async t => {
            receiver[t] = await wasm.createOTAReceiver(this.key.base58CheckSerialize(PaymentAddressType));
        ...
        let metadata = {
            ...
            OrderID: orderID,
            Amount: amount,
            BurntOTA: accessTicket,
            NextOTA: nextAccessTicket,
            Receiver: receiver,
            Type: pdexv3.WithdrawOrderRequestMeta,
        };

        try {
            let result = await this._transact({ transfer: { prvPayments: [], fee, tokenID, info, tokenPayments: burningPayments, tokenCoinChooser: new AccessTicketChooser(accessTicket, useBase58Encoding) }, extra: { metadata }});
            this.saveTxHistory(result, false, "", messageForNativeToken);
            await this.updateProgressTx(100, 'Completed');
            return result;
        } catch (e) {
    ...
    async pdexv3TxWithdrawLiquidity_accessOTA({ transfer: { fee, tokenID = PDEX_ACCESS_ID, info = "" }, extra: { poolTokenIDs, poolPairID, shareAmount, accessTicket, nextAccessTicket, useBase58Encoding = false }}) {
        await this.updateProgressTx(10, 'Generating Metadata');
        let burningAddress = await getBurningAddress(this.rpc);
        let burningPayments = [{
            PaymentAddress: burningAddress,
        ...
        let receivingTokens = poolTokenIDs;
        let receiver = {};
        // create new OTAs
        receivingTokens.forEach(async t => {
            receiver[t] = await wasm.createOTAReceiver(this.key.base58CheckSerialize(PaymentAddressType));
        ...
        try {
            let result = await this._transact({ transfer: { prvPayments: [], fee, info, tokenID, tokenPayments: burningPayments, tokenCoinChooser: new AccessTicketChooser(accessTicket, useBase58Encoding) }, extra: { metadata }});
            this.saveTxHistory(result, false, "", messageForNativeToken);
            await this.updateProgressTx(100, 'Completed');
            return result;
        } catch (e) {
    ...
    async pdexv3TxUnstake({ transfer: { fee, info = "" }, extra: { unstakingAmount, accessTicket, nextAccessTicket, useBase58Encoding = false, stakingTokenID, stakingPoolID }}) {
        await this.updateProgressTx(10, 'Generating Metadata');
        let burningAddress = await getBurningAddress(this.rpc);
        let burningPayments = [{
            PaymentAddress: burningAddress,
        ...
        let receivingTokens = [stakingTokenID];
        let receiver = {};
        // create new OTAs
        receivingTokens.forEach(async t => {
            receiver[t] = await wasm.createOTAReceiver(this.key.base58CheckSerialize(PaymentAddressType));
    ...
    async pdexv3TxWithdrawLPFee({ transfer: { fee, info = "" }, extra: { withdrawTokenIDs, poolPairID, accessTicket, nextAccessTicket, useBase58Encoding = false }}) {
        await this.updateProgressTx(10, 'Generating Metadata');
        let burningAddress = await getBurningAddress(this.rpc);
        let burningPayments = [{
            PaymentAddress: burningAddress,
    ...
    async pdexv3TxWithdrawStakingReward({ transfer: { fee, info = "" }, extra: { stakingTokenID, accessTicket, nextAccessTicket, useBase58Encoding = false }}) {
        await this.updateProgressTx(10, 'Generating Metadata');
        let burningAddress = await getBurningAddress(this.rpc);
        let burningPayments = [{
            PaymentAddress: burningAddress,
