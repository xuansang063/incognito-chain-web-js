    ...
    */
    ...
    async pdexv3TxMintNft({ transfer: { fee, info = ""}}) {
        ...
        let metadata = {
            ...
            OtaReceive: await wasm.createOTAReceiver(this.key.base58CheckSerialize(PaymentAddressType)),
    ...
    async pdexv3TxContribute({ transfer: { fee, info = "", tokenID = null }, extra: { pairID, pairHash, contributedAmount, nftID = "", amplifier } = {}}) {
        ...
        let metadata = {
            ...
            Amplifier: amplifier,
            OtaReceive: await wasm.createOTAReceiver(this.key.base58CheckSerialize(PaymentAddressType)),
            OtaRefund: await wasm.createOTAReceiver(this.key.base58CheckSerialize(PaymentAddressType)),
    ...
    async pdexv3TxAddOrder({ transfer: { fee, info = "" }, extra: { tokenIDToSell, poolPairID, sellAmount, minAcceptableAmount, nftID }}) {
        await this.updateProgressTx(10, 'Generating Metadata');
        let burningAddress = await getBurningAddress(this.rpc);
        let burningPayments = [{
            PaymentAddress: burningAddress,
        ...
        receiver[tokenIDToSell] = await wasm.createOTAReceiver(this.key.base58CheckSerialize(PaymentAddressType));
        // prepare meta data for tx
        let metadata = {
    ...
    async pdexv3TxWithdrawLiquidity({ transfer: { fee, info = "" }, extra: { poolTokenIDs, poolPairID, shareAmount, nftID }}) {
        ...
        let metadata = {
            ...
            NftID: nftID,
            OtaReceiveNft: await wasm.createOTAReceiver(this.key.base58CheckSerialize(PaymentAddressType)),
            OtaReceiveToken0: await wasm.createOTAReceiver(this.key.base58CheckSerialize(PaymentAddressType)),
            OtaReceiveToken1: await wasm.createOTAReceiver(this.key.base58CheckSerialize(PaymentAddressType)),
    ...
    async pdexv3TxWithdrawOrder({ transfer: { fee, info = "" }, extra: { withdrawTokenID, poolPairID, orderID, amount, nftID }}) {
        await this.updateProgressTx(10, 'Generating Metadata');
        let burningAddress = await getBurningAddress(this.rpc);
        let burningPayments = [{
            PaymentAddress: burningAddress,
