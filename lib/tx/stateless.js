...
class StatelessTransactor {
    ...
    async consolidate({ transfer: { fee, tokenID = null } = {}, extra: { noOfInputPerTx = MaxInputNumberForDefragment } = {}}) {
        await this.updateProgressTx(20, `Analyzing Coins`);
        const info = "consolidate";
        const prepareInput = async (fee, tokenID, account, numOfOtherPks = 20, maxInputs = MAX_INPUT_PER_TX) => {
            const spendableCoins = await account.getListUnspentCoins(tokenID, inputVersion);
            const groupedCoins = coinConsolidator.coinsToSpend(spendableCoins, fee, maxInputs);
            if (groupedCoins.length === 0 || groupedCoins[0].length === 0) {
                throw new CustomError(ErrorObject.NotEnoughCoinError, "No spendable coins");
            }
            const shardID = getShardIDFromLastByte(account.key.KeySet.PaymentAddress.Pk[(account.key.KeySet.PaymentAddress.Pk.length - 1)]);
        ...
        const finalize = async (txParams, isToken, returnedValues) => {
            ...
            if (isToken) await this.waitTx(hash);
            return result;
        }
        ...
        if (isToken){
            for (const inputs of groupedInputs) {
                ...
                txParams = newParamTxV2(this.key, [], prvInputs, fee, null, null, info, prvCoinsForRing);
                tokenParams = newTokenParamV2([], inputs, tokenID, coinsForRing, {});
                txParams.TokenParams = tokenParams;
                let temp = await finalize(txParams, true, {});
                results.push(temp);
            }
        } else {
            results = await Promise.all(groupedInputs.map(inputs => newParamTxV2(this.key, [], inputs, fee, null, null, info, coinsForRing))
                .map(p => finalize(p, false, {}))
            );
        }
