...
class StatelessTransactor {
    ...
    */
    ...
    async submitKey(scan = false, batchSize = 5000, poolSize = 20) {
        const otaKey = this.key.base58CheckSerialize(OTAKeyType);
        await this.rpc.submitKey(otaKey).catch(console.warn);
        ...
        if (scan) {
            ...
            for (const tokenID of tokenIDs) {
                ...
                for (let i = 0; i < coinlen; i += batchSize * poolSize) {
                    let tasks = [];
                    for (let taskInd = 0; taskInd < poolSize; taskInd++) {
                        ...
                        tasks.push(this.rpc.getOutputCoinByIndex(indexes, shardID, tokenID));
                    }
