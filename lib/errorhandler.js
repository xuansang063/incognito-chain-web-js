  ...
  constructor(errorObj, message, detailError) {
    ...
    if (detailError && typeof detailError === 'object') {
      if (detailError.StackTrace) {
        this.stackTrace = detailError.StackTrace;
        this.stackTraceCode = detailError.StackTrace.match(/-[0-9]+: -[0-9]+/)[0]
      } else if (detailError.response) {
        this.stackTrace = detailError.message;
        this.stackTraceCode = detailError.response.status;
      }
    }
  }
}
