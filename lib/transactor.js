...
import { KeySet, addressToUnmarshallable } from './common/keySet';
import {
    RpcClient
...
class Transactor {
    ...
    async createAndSendNativeToken(paramPaymentInfos, fee, info = "", isEncryptMessageOutCoin = false, metadata = null) {
        ...
        try {
            let result = await this._transact(paramPaymentInfos, fee, metadata, info);
    ...
    async _transact(paramPaymentInfos, fee, metadata, info = "", tokenID = null, tokenPayments = null, tokenParams = null) {
    ...
    async createAndSendConvertTx(paramPaymentInfos, fee, info = "", isEncryptMessageOutCoin = false) {
        ...
        try {
            let result = await this._transactConvert(paramPaymentInfos, fee, info);
            this.saveTxHistory(result, false, "", messageForNativeToken);
            await this.updateProgressTx(100, 'Completed');
            return result;
        } catch (e) {
    ...
    async createAndSendTokenConvertTx(tokenID, prvPaymentInfo, tokenPaymentInfo, fee, info = "", isEncryptMessage = false, isEncryptMessageToken = false) {
        ...
        try {
            let result = await this._transactConvert(prvPaymentInfo, fee, info, tokenID, tokenPaymentInfo);
            this.saveTxHistory(result, false, "", messageForNativeToken);
            await this.updateProgressTx(100, 'Completed');
            return result;
        } catch (e) {
    ...
    async _transactConvert(paramPaymentInfos, fee, info, tokenID = null, tokenPayments = null, numOfDefragInputs = 0) {
        await this.updateProgressTx(20, 'Preparing Your Payments');
        info = base64Encode(stringToBytes(info));
    ...
    async createAndSendStakingTx(param, feeNativeToken, candidatePaymentAddress, candidateMiningSeedKey, rewardReceiverPaymentAddress, autoReStaking = true) {
        ...
        try {
            let result = await this._transact(paramPaymentInfos, feeNativeToken, meta, "");
            this.saveTxHistory(result, false, "", messageForNativeToken);
            await this.updateProgressTx(100, 'Completed');
            return result;
        } catch (e) {
    ...
    async createAndSendPrivacyToken(
        ...
        tokenPaymentInfo = await encryptMessageOutCoin(tokenPaymentInfo, isEncodeOnly);
        try {
            let result = await this._transact(prvPaymentInfo, feeNativeToken, metadata, info, tokenID, tokenPaymentInfo, tokenParams);
            this.saveTxHistory(result, false, "", messageForNativeToken);
            await this.updateProgressTx(100, 'Completed');
            return result;
        } catch (e) {
    ...
    async createAndSendInitTokenTx(tokenPaymentInfo, fee, tokenName = "", tokenSymbol = "", info = "") {
        ...
        try {
            ...
            let temp = await wasm.createCoin(JSON.stringify({PaymentInfo : tokenPaymentInfo, TokenID: null}));
        ...
        let metadata = {
            ...
            Amount: new bn(tokenPaymentInfo.Amount).toString(),
        ...
        try {
            let result = await this._transact(prvPaymentInfos, fee, metadata, info);
            // re-compute token ID
    ...
    async defragmentNativeCoin(fee, noOfInputPerTx = MaxInputNumberForDefragment) {
        ...
        for (let i=0;i<MAX_ITERATIONS;i++){
            ...
            try{
                ...
                try {
                    let result = await this._transactConvert([], fee, info, null, null, noOfInputPerTx);
    ...
    async createAndSendBurningRequestTx(
        ...
        let addrForMd = addressToUnmarshallable(emptyKeySet.PaymentAddress);
        ...
        try {
            let result = await this._transact(prvPaymentInfo, fee, burningReqMetadata, info, burningTokenID, tokenPaymentInfo);
            this.saveTxHistory(result, false, "", messageForNativeToken);
            await this.updateProgressTx(100, 'Completed');
            return result;
        } catch (e) {
    ...
    async createAndSendIssuingEthRequestTx(prvPaymentInfo = [], fee, tokenID, ethBlockHash, ethDepositProof, txIndex, info = "", isEncryptMessageOutCoinNativeToken = false,     isEncryptMessageOutCoinPToken = false) {
        ...
        try {
            let result = await this._transact(prvPaymentInfo, fee, metadata, info);
            this.saveTxHistory(result, false, "", messageForNativeToken);
            await this.updateProgressTx(100, 'Completed');
            return result;
        } catch (e) {
    ...
    async createAndSendWithdrawRewardTx(fee, tokenID = null) {
        ...
        try {
            let result = await this._transact([], fee, md, "");
    ...
    */
    ...
    async createAndSendTxWithContribution(fee, pdeContributionPairID, contributedAmount, info = "", tokenIDStr = null) {
        ...
        try {
            let result;
            if (isToken){
                result = await this._transact([], fee, metadata, info, tokenIDStr, paramPaymentInfos);
            }else{
                result = await this._transact(paramPaymentInfos, fee, metadata, info);
            }
            this.saveTxHistory(result, false, "", messageForNativeToken);
            await this.updateProgressTx(100, 'Completed');
            return result;
    ...
    async createAndSendNativeTokenTradeRequestTx(fee, tokenIDToBuyStr, sellAmount, minimumAcceptableAmount, tradingFee, info = "", tokenIDToSellStr = null) {
        ...
        try {
            let result;
            if (sellPRV){
                result = await this._transact(prvPaymentInfos, fee, metadata, info);
            }else{
                result = await this._transact(prvPaymentInfos, fee, metadata, info, tokenIDToSellStr, tokenPaymentInfos);
            }
            this.saveTxHistory(result, false, "", messageForNativeToken);
            await this.updateProgressTx(100, 'Completed');
            return result;
