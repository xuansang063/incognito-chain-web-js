...
class Transactor extends StoredAccount {
    ...
    */
     ...
     * @param {string} remoteAddress
     */
     async createAndSendUnshieldPortalV4RequestTx({ transfer: { prvPayments = [], fee, info = "", tokenID}, extra: { burningType = PortalV4UnshieldRequestMeta, isEncryptMessage = false, isEncryptMessageToken = false, chainName, unshieldAmount, remoteAddress } = {}}) {
        const unshieldTokenID = tokenID;

        let portalParams = new PortalParams(chainName)
        if (!portalParams.SupportedTokens.includes(unshieldTokenID)) {
            throw new CustomError(ErrorObject.SupportPortalTokenIDErr, "TokenID is not supported")
        }
