    ...
    async getReceivedTransaction() {
        ...
        for (let i = 0; i < txs.length; i++) {
            ...
            if (txs[i].ReceivedAmounts[PRVIDSTR]) {
                //
                messageForNativeToken = await decryptMessageOutCoin(this, txs[i].ReceivedAmounts[PRVIDSTR].CoinDetails.Info);
            }
            if (txs[i].ReceivedAmounts[tx.PrivacyCustomTokenID]) {
                //
                messageForPToken = await decryptMessageOutCoin(this, txs[i].ReceivedAmounts[tx.PrivacyCustomTokenID].CoinDetails.Info);
            }
