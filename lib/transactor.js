...
class Transactor {
    ...
    async setKey(privateKey){
        ...
        this.newToken = this.createAndSendInitTokenTx; // async (tokenPaymentInfo, fee, tokenParams) => this.token("", [], tokenPaymentInfo, fee, "", false, false, tokenParams);
    ...
    async createAndSendInitTokenTx(tokenPaymentInfo, fee, tokenName = "", tokenSymbol = "", info = "") {
        ...
        try {
            let result = await this.doPRVTransactionV2(prvPaymentInfos, fee, metadata, info);
            // re-compute token ID
