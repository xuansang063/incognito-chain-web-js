...
} from '@lib/module/Account/account.tokenModal';
...
class RpcHTTPCoinServiceClient {
  ...
  apiGetContributeHistories = ({ offset = 0, limit = 50, paymentAddress } = {}) => {
    new Validator("apiGetContributeHistories-offset", offset).required().number();
    new Validator("apiGetContributeHistories-limit", limit).number();
    new Validator("apiGetContributeHistories-paymentAddress", paymentAddress).required().string();
  ...
  apiGetLiquidityRemoveHistories = ({ offset = 0, limit = 50, paymentAddress } = {}) => {
    new Validator("apiGetLiquidityRemoveHistories-offset", offset).required().number();
    new Validator("apiGetLiquidityRemoveHistories-limit", limit).number();
    new Validator("apiGetLiquidityRemoveHistories-paymentAddress", paymentAddress).required().string();
    const url = `getwithdrawhistory?offset=${offset}&limit=${limit}&paymentkey=${paymentAddress}`;
    return this.http.get(url).then(histories => (histories || []).map((history) => formatWithdrawHistory(history)));
  }

  apiGetLiquidityWithdrawFeeHistories = ({ offset = 0, limit = 50, paymentAddress } = {}) => {
    new Validator("apiGetLiquidityWithdrawFeeHistories-offset", offset).required().number();
    new Validator("apiGetLiquidityWithdrawFeeHistories-limit", limit).number();
    new Validator("apiGetLiquidityWithdrawFeeHistories-paymentAddress", paymentAddress).required().string();
    const url = `getwithdrawfeehistory?offset=${offset}&limit=${limit}&paymentkey=${paymentAddress}`;
    return this.http.get(url).then(histories => (histories || []).map((history) => formatWithdrawFeeHistory(history)));
  }
}
