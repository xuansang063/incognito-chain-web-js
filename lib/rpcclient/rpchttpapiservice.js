...
class RpcHTTPApiServiceClient {
  ...
  apiGetQuote = async (data) => {
    ...
    return this.http.get(
      `uniswap/quote?tokenIn=${tokenInContractId}&tokenOut=${tokenOutContractId}&amount=${amount}&chainId=80001`
    );
  };
  ...
  apiGetUniHistory = (payload) => {
    ...
    new Validator("apiGetUniHistory-walletAddress", walletAddress)
      ...
      .string();
    return this.http.get(
      `trade/history?filter[wallet_address]=${walletAddress}&filter[trade_type]=2`
    );
  };
  ...
  apiGetUniRewardHistory = (payload) => {
    ...
    new Validator("apiGetUniRewardHistory-walletAddress", walletAddress)
      ...
      .string();
    return this.http.get(
      `/trade/reward-history?filter[status]=2&filter[wallet_address]=${walletAddress}&filter[trade_type]=2&limit=${limit}&page=${page}`
    );
  };
}
