...
class RpcHTTPApiServiceClient {
  ...
  apiGetRaydiumTokens = () => this.http.get(`raydium/tokens`);

  apiEstimateRaydiumTradingFee = async (data) => {
    const { walletAddress, srcTokens, destTokens, srcQties } = data;
    new Validator("apiEstimateRaydiumTradingFee-walletAddress", walletAddress)
        .required()
        .string();
    new Validator("apiEstimateRaydiumTradingFee-srcTokens", srcTokens)
        .required()
        .string();
    new Validator("apiEstimateRaydiumTradingFee-destTokens", destTokens)
        .required()
        .string();
    new Validator("apiEstimateRaydiumTradingFee-srcQties", srcQties)
        .required()
        .string();
    const payload = {
      WalletAddress: walletAddress,
    ...
    console.log(payload);
    return this.http.post("raydium/estimate-fees", payload);
  };

  apiGetQuoteRaydium = async (data) => {
    const { tokenInContractId, tokenOutContractId, amount } = data;
    return this.http.get(
        `raydium/quote?tokenIn=${tokenInContractId}&tokenOut=${tokenOutContractId}&amount=${amount}`
    );
  };

  apiSubmitRaydiumTradingTx = (payload) => {
    const {
      tradeID,
      burnTxID,
      paymentAddress,
    ...
    new Validator("apiSubmitRaydiumTradingTx-tradeID", tradeID)
        .required()
        .number();
    new Validator("apiSubmitRaydiumTradingTx-burnTxID", burnTxID)
        .required()
        .string();
    new Validator("apiSubmitRaydiumTradingTx-paymentAddress", paymentAddress)
        .required()
        .string();
    new Validator("apiSubmitRaydiumTradingTx-srcTokenID", srcTokenID)
        .required()
        .string();
    new Validator("apiSubmitRaydiumTradingTx-destTokenID", destTokenID)
        .required()
        .string();
    new Validator("apiSubmitRaydiumTradingTx-expectedDestAmt", expectedDestAmt)
        .required()
        .string();
    new Validator("apiSubmitRaydiumTradingTx-srcQties", srcQties)
        .required()
        .string();
    let data = {
      ID: tradeID,
    ...
    console.log("data", data);
    return this.http.post("raydium/submit-trading-tx", data);
  };

  // get list swap history for Raydium
  apiGetRaydiumHistory = (payload) => {
    const { walletAddress } = payload;
    new Validator("apiGetRaydiumHistory-walletAddress", walletAddress)
        .required()
        .string();
    return this.http.get(
        `raydium/history?filter[wallet_address]=${walletAddress}`
    );
  };

  apiGetRaydiumHistoryDetail = (payload) => {
    const { tradeID } = payload;
    new Validator("apiGetRaydiumHistoryDetail-tradeID", tradeID)
        .required()
        .number();
    return this.http.get(`raydium/history/${tradeID}`);
  };

  apiGetRaydiumRewardHistory = (payload) => {
    const { walletAddress, page, limit } = payload;
    new Validator("apiGetRaydiumRewardHistory-walletAddress", walletAddress)
        .required()
        .string();
    return this.http.get(
        `raydium/reward-history?filter[wallet_address]=${walletAddress}&page=${page}&limit=${limit}`
    );
  };
