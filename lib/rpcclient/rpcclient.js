...
class RpcClient {
  ...
  isExchangeRatePToken = async (tokenIDStr1, tokenIDStr2 = "") => {
    ...
    if (pdeStateRes.state.PDEPoolPairs[keyValue] !== null && pdeStateRes.state.PDEPoolPairs[keyValue] !== undefined) {
      if (tokenIDStr1 == PRVIDSTR && pdeStateRes.state.PDEPoolPairs[keyValue].Token1PoolValue < 10000 *1e9){
        return false;
      }

      if (tokenIDStr2 == PRVIDSTR && pdeStateRes.state.PDEPoolPairs[keyValue].Token2PoolValue < 10000 *1e9){
        return false;
      }
