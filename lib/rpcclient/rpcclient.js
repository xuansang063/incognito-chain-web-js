...
class RpcClient {
  ...
  parseInputCoinFromEncodedObject = (serializedObjects, keyWallet) => {
    ...
    const spendingKeyBNInRed = spendingKeyBN.toRed(BN.red(P256.n.clone()))
    ...
    for (let i = 0; i < serializedObjects.length; i++) {
      ...
      inputCoins[i].coinDetails.serialNumber = P256.g.derive(spendingKeyBNInRed, new BN(inputCoins[i].coinDetails.snderivator));
