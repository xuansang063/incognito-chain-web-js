...
class RpcClient {
  ...
  parseInputCoinFromEncodedObject = (serializedObjects, keyWallet, deriveCache = {}) => {
    ...
    for (let i = 0; i < serializedObjects.length; i++) {
      ...
      const snderivator = inputCoins[i].coinDetails.snderivator;
      let serialNumber = {}
      if (deriveCache[snderivator] != undefined) {
        serialNumber = deriveCache[snderivator];
      } else {
        serialNumber = P256.g.derive(spendingKeyBN, snderivator);
        deriveCache[snderivator] = serialNumber;
      }
