...
class RpcClient {
  ...
  parseInputCoinFromEncodedObject = (serializedObjects, keyWallet, deriveCache = {}, tokenID = 'constant') => {
    ...
    for (let i = 0; i < serializedObjects.length; i++) {
      ...
      let serialNumber = {}
      if (deriveCache[sndStr] != undefined) {
        serialNumber = deriveCache[sndStr];
      } else {
        serialNumber = P256.g.derive(spendingKeyBN, inputCoins[i].coinDetails.snderivator);
        deriveCache[sndStr] = serialNumber;
      }
