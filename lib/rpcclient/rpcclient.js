...
class RpcClient {
  ...
  prepareInputForTx = async (spendingKeyStr, paymentInfos, fee, account) => {
    ...
    let { unspentCoins } = await Wallet.RpcClient.getUnspentCoin(unspentInputCoinsFromCached, paymentAddrSerialize, unspentInputCoinsFromCachedStrs);

    // remove spending coins from list of unspent coins
    let unspentCoinExceptSpendingCoin = Wallet.RpcClient.getUnspentCoinExceptSpendingCoin(unspentCoins, account);
    ...
    try {
      respChooseBestCoin = this.chooseBestCoinToSpent(unspentCoinExceptSpendingCoin, amount);
    } catch (e) {
  ...
  getUnspentCoinExceptSpendingCoin(unspentCoins, account) {
    ...
    if (account.spendingCoins){
      if (account.spendingCoins.length){
        let unspentCoinExceptSpendingCoin = this.cloneInputCoinArray(unspentCoins);
  ...
  parseInputCoinFromEncodedObject = (encodedCoinObjects, keyWallet, derivatorCached = {}, tokenID = 'constant') => {
  ...
  prepareInputForCustomTokenTx = async (spendingKeyStr, tokenParams) => {
    ...
    switch (tokenParams.tokenTxType) {
      ...
      case CustomTokenTransfer: {
        ...
        let response2 = await this.getUnspentCustomToken(senderKeyWallet.base58CheckSerialize(PaymentAddressType),
  ...
  prepareInputForTxCustomTokenPrivacy = async (spendingKeyStr, tokenParams, account) => {
    ...
    switch (tokenParams.tokenTxType) {
      ...
      case CustomTokenTransfer: {
        ...
        try {
          unspentOutputCoins = await this.getUnspentPrivacyCustomToken(unspentInputCoinsFromCached, paymentAddrSerialize, unspentInputCoinsFromCachedStrs, tokenParams.propertyID.toLowerCase());
        } catch (e) {
          throw e;
        }
