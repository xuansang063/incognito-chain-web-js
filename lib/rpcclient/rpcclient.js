...
class RpcClient {
  ...
  sendRawTx = async (tx) => {
    console.log("TX fee when send tx: ", tx.fee);
  ...
  prepareInputForTxCustomTokenPrivacy = async (spendingKeyStr, tokenParams, account) => {
    ...
    switch (tokenParams.tokenTxType) {
      ...
      case constantsTx.CustomTokenTransfer: {
        ...
        let unspentOutputCoins = await this.getUnspentPrivacyCustomToken(inputCoinsRet, paymentAddrSerialize, allOutputCoinStrsRet, tokenParams.propertyID.toLowerCase());
  ...
  getEstimateFee = async (from, to, amount, privatekeyStr, customTokenParams = null, privacyCustomTokenParams = null, account, isPrivacy) => {
    ...
    let inputForTx = await Wallet.RpcClient.prepareInputForTx(privatekeyStr, paymentInfos, new bn(0), account);

    console.log("input for tx when getEstimateFee: ", inputForTx.inputCoins);

    let fee = await this.estimateFee(from, inputForTx.inputCoins, paymentInfos, isPrivacy, null, customTokenParams, privacyCustomTokenParams);
    return fee;
  };
  /**
   *
  ...
  getEstimateFeeForSendingToken = async (from, to, amount, tokenObject, privatekeyStr, account) => {
    ...
    if (tokenObject.Privacy === false) {
      ...
      return await this.getEstimateFee(from, to, amount, privatekeyStr, customTokenParams, null, account);
      ...
      privacyCustomTokenParams.set(id, name, symbol, amount, tokenObject.TokenTxType, [receivers],
        []);
      let inputForPrivacyCustomToken = await Wallet.RpcClient.prepareInputForTxCustomTokenPrivacy(privatekeyStr, privacyCustomTokenParams, account);
      privacyCustomTokenParams.tokenInputs = inputForPrivacyCustomToken.tokenInputs;
      return await this.getEstimateFee(from, to, amount, privatekeyStr, null, privacyCustomTokenParams, account)
    }
  }
  ...
  getEstimateFeeToDefragment = async (from, amount, privatekeyStr, account, isPrivacy) => {
    ...
    let fee = await this.estimateFee(from, defragmentUTXO, paymentInfos, isPrivacy, null, null, null);
    return fee;
  };
  ...
  estimateTxSize = (inputCoins, payments, hasPrivacy, metadata, customTokenParams, privacyCustomTokenParams) => {
    ...
    return Math.ceil(sizeTx / 1024.0) + 1;
  };
  ...
  getTransactionByHash = async (txHashStr) => {
    ...
    // call API to send tx
    const response = await this.rpcHttpService.postRequest(data);
    // console.log("response get tx by hash: ", response);
  ...
  getUTXOsToDefragment = async (spendingKeyStr, fee, account, amount) => {
    ...
    for (let i=0; i<unspentCoins.length; i++){
      if (unspentCoins[i].coinDetails.value.cmp(amount) == -1){
        defragmentUTXO.push(unspentCoins[i]);
        defragmentUTXOStr.push(unspentCoinStrs[i]);
        totalAmount = totalAmount.add(unspentCoins[i].coinDetails.value);
        numUTXO ++;
        if (numUTXO >= 255){
          break;
        }
