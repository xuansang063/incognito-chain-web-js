...
class RpcClient {
  ...
  prepareInputForTx = async (spendingKeyStr, paymentInfos, fee, account) => {
    ...
    try {
      ...
      inputCoinsToSpent = Object.assign([], bestCoinToSpent.resultOutputCoins);
    } catch (e) {
  ...
  chooseBestCoinToSpent = (inputCoins, amount) => {
    ...
    // for (let i = 0; i < inputCoins.length; i++) {
    //   console.log("inputCoins[i].coinDetails.value after choose best coin: ", inputCoins[i].coinDetails.value);
    // }
  ...
  getEstimateFeeToDefragment = async (from, amount, privatekeyStr, account, isPrivacy) => {
    ...
    console.log("Amount when estimate fee:", amount);
  ...
  getUTXOsToDefragment = async (spendingKeyStr, fee, account, amount) => {
    ...
    for (let i = 0; i < unspentCoins.length; i++) {
      console.log("unspentCoins[i].coinDetails.value: ", unspentCoins[i].coinDetails.value);
