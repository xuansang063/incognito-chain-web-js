...
class RpcClient {
  ...
  parseInputCoinFromStr = (coinStrs, keyWallet) => {
    ...
    console.log("Spending key: ", keyWallet.KeySet.PrivateKey);
  ...
  getEstimateFeeForSendingToken = async (from, to, amount, tokenObject, privatekeyStr) => {
    ...
    let receivers = Object.keys(tokenObject.TokenRecievers);
    let vins = Object.values(tokenObject.TokenReceivers);
    if (tokenObject.Privacy === true){
      let customtTokenParams = new CustomTokenParamTx();
      customtTokenParams.set(tokenObject.TokenID, tokenObject.TokenName,tokenObject.TokenSymbol,
                        amount, tokenObject.TokenTxType,receivers,
                        vins, tokenObject.TokenAmount);
      return await this.getEstimateFee(from,to,amount,privatekeyStr,tokenObject)
    }
    else {
      let privacyCustomtTokenParams = new CustomTokenParamTx();
      privacyCustomtTokenParams.set(tokenObject.TokenID, tokenObject.TokenName,tokenObject.TokenSymbol,
        amount, tokenObject.TokenTxType,receivers,
        vins, tokenObject.TokenAmount);
      return await this.getEstimateFee(from,to,amount,privatekeyStr,null,tokenObject)
    }
  }
