...
class RpcClient {
  ...
  parseInputCoinFromEncodedObject = (encodedCoinObjects, keyWallet, derivatorCached = {}, tokenID = 'constant') => {
    ...
    for (let i = 0; i < encodedCoinObjects.length; i++) {
      // decode object coins
      let publicKeyDecode = base58.checkDecode(encodedCoinObjects[i].PublicKey).bytesDecoded;
      let commitmentDecode = base58.checkDecode(encodedCoinObjects[i].CoinCommitment).bytesDecoded;
      let sndDecode = base58.checkDecode(encodedCoinObjects[i].SNDerivator).bytesDecoded;
      let randDecode = base58.checkDecode(encodedCoinObjects[i].Randomness).bytesDecoded;
      ...
      inputCoins[i].coinDetails.info = base58.checkDecode(encodedCoinObjects[i].Info).bytesDecoded;
