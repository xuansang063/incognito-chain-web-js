...
class RpcClient {
  ...
  parseInputCoinFromEncodedObject = (serializedObjects, keyWallet) => {
    ...
    const spendingKeyBNInRed = spendingKeyBN.toRed(ec.N)
    ...
    for (let i = 0; i < serializedObjects.length; i++) {
      ...
      inputCoins[i].coinDetails.serialNumber = P256.g.derive(spendingKeyBN, inputCoins[i].coinDetails.snderivator);
