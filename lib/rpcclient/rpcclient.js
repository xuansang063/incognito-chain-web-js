...
class RpcClient {
  ...
  getEstimateFee = async (from, to, amount) =>{
    ...
    paymentInfos[0] = new key.PaymentInfo(receiverKeyWallet.KeySet.PaymentAddress, amount);

    let senderKeyWallet = keyWallet.base58CheckDeserialize(from);
    senderKeyWallet.KeySet.importFromPrivateKey(senderKeyWallet.KeySet.PrivateKey);
    let senderSkStr = senderKeyWallet.base58CheckSerialize(constantsWallet.PriKeyType);

    let inputForTx = await Wallet.RpcClient.prepareInputForTx(senderSkStr, paymentInfos);
    return await this.estimateFee(from,inputForTx,paymentInfos,  true, null,null,null);
  };
  /**
   * tokenObject {
      Privacy: true,
   ...
   */
  getEstimateFeeForSendingToken = async (from, to, amount, tokenObject) => {
    return 0
  }
