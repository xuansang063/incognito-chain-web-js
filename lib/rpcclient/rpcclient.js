...
class RpcClient {
  ...
  prepareInputForTx = async (spendingKeyStr, paymentInfos, fee) => {
  ...
  prepareInputForCustomTokenTx = async (spendingKeyStr, tokenParams, fee) => {
  ...
  prepareInputForTxCustomTokenPrivacy = async (spendingKeyStr, tokenParams, fee) => {
    ...
    switch (tokenParams.tokenTxType) {
      ...
      case constantsTx.CustomTokenTransfer: {
        ...
        amountTokenPrivacyOutput = amountTokenPrivacyOutput.add(fee);
  ...
  getEstimateFee = async (from, to, amount, privatekeyStr, customTokenParams = null, privacyCustomTokenParams = null) => {
    ...
    let inputForTx = await Wallet.RpcClient.prepareInputForTx(privatekeyStr, paymentInfos, new bn(0));
  ...
  getEstimateFeeForSendingToken = async (from, to, amount, tokenObject, privatekeyStr) => {
    ...
    if (tokenObject.Privacy === false) {
      ...
      let inputForCustomTx = await Wallet.RpcClient.prepareInputForCustomTokenTx(privatekeyStr, customTokenParams, new bn(0));
      customTokenParams.vins = inputForCustomTx.tokenVins;
      return await this.getEstimateFee(from, to, amount, privatekeyStr, customTokenParams, null);
      ...
      privacyCustomTokenParams.set(id, name, symbol, amount, tokenObject.TokenTxType, [receivers],
        []);
<<<<<<< HEAD
      let inputForPrivacyCustomToken = await Wallet.RpcClient.prepareInputForTxCustomTokenPrivacy(privatekeyStr,privacyCustomTokenParams, new bn(0));
=======
      let inputForPrivacyCustomToken = await Wallet.RpcClient.prepareInputForTxCustomTokenPrivacy(privatekeyStr, privacyCustomTokenParams);
>>>>>>> 5eb817248326912cd2a6ac2f3d83932ddc013cfa
      privacyCustomTokenParams.tokenInputs = inputForPrivacyCustomToken.tokenInputs;
      return await this.getEstimateFee(from, to, amount, privatekeyStr, null, privacyCustomTokenParams)
    }
  }
