...
class RpcClient {
  ...
  parseInputCoinFromEncodedObject = (serializedObjects, keyWallet, derivatorCache = {}, tokenID = 'constant') => {
    ...
    for (let i = 0; i < serializedObjects.length; i++) {
      ...
      let serialNumber = {}
      if (derivatorCache[sndStr] != undefined) {
        serialNumber = derivatorCache[sndStr];
      } else {
        serialNumber = P256.g.derive(spendingKeyBN, inputCoins[i].coinDetails.snderivator);
        derivatorCache[sndStr] = serialNumber;
      }
