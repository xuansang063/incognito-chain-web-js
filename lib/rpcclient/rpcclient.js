...
class RpcClient {
  ...
  prepareInputForTx = async (spendingKeyStr, paymentInfos) => {
    ...
    let inputCoinsToSpent = this.chooseBestCoinToSpent(unspentCoins, amount).resultOutputCoins;

    let inputCoinsToSpentStr = this.parseInputCoinToStr(inputCoinsToSpent);
    ...
    switch (tokenParams.tokenTxType) {
      ...
      case constantsTx.CustomTokenTransfer: {
        ...
        for (let i=0; i<tokenParams.receivers.length; i++){
          vOutAmount += tokenParams.receivers.value;
        }
        ...
        let tokenVoutsTmp = new Array(listUnspentCustomToken.length);
        ...
        for (let i=0; i< listUnspentCustomToken.length; i++){
          ...
          tokenVoutsTmp[i] = new TxTokenVout();
          tokenVoutsTmp[i].set(senderKeyWallet.KeySet.PaymentAddress, listUnspentCustomToken[i].Value);
          ...
          let signature = senderKeyWallet.KeySet.sign(tokenVoutsTmp[i].hash());
          tokenVins[i].signature = base58.checkEncode(signature, constants.PRIVACY_VERSION);
  ...
  prepareInputForTxCustomTokenPrivacy = async (spendingKeyStr, paymentInfos, tokenParams) => {
    // paymentInfo for tx normal for fee
    // tokenParams for tx custom token privacy data, but haven't tokenParam's tokenInput
    // deserialize spending key string to key wallet
    let myKeyWallet = keyWallet.base58CheckDeserialize(spendingKeyStr);
    ...
    // prepare tokenParams' tokenInput for tx custom token privacy
    let amountTokenPrivacyOutput = 0;
    for (let i=0; i<tokenParams.receiver.length; i++){
      amountTokenPrivacyOutput += tokenParams.receiver[i].Amount;
    }

    let res = await this.getOutputCoin(paymentAddrSerialize, readOnlyKeySerialize, tokenParams.propertyID);
    let allOutputCoinStrs;
    ...
    // get coin to spent using Knapsack
    let tokenInputs = this.chooseBestCoinToSpent(unspentOutputCoins.unspentCoin, new bn(amountTokenPrivacyOutput)).resultOutputCoins;
