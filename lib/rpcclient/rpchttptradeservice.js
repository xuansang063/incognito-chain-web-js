...
class RpcHTTPTradeServiceClient {
  ...
  apiGetListShare({ nftId, otaKey }) {
    const useAccessOTA = Boolean(otaKey)
    if (useAccessOTA) {
      new Validator("apiGetListShare-otaKey", otaKey).required().string();
      return this.http.get(`pdex/v3/poolshare?otakey=${otaKey}`);
    }
  ...
  async apiGetContributeHistories(params) {
    const { nftId, otaKey, limit = 1e9, offset = 0 } = params;
    let prefix = '';
    if (Boolean(nftId)) {
      prefix = `nftid=${nftId}`;
    } else {
      prefix = `otakey=${otaKey}`;
    }
    return this.http.get(
      `pdex/v3/contributehistory?${prefix}&limit=${limit}&offset=${offset}`
    );
  }
  ...
  async apiGetWithdrawContributeHistories(params) {
    ...
    return this.http.get(
      `pdex/v3/withdrawhistory?${prefix}&limit=${limit}&offset=${offset}`
    );
  }
  ...
  async apiGetWithdrawFeeContributeHistories(params) {
    ...
    return this.http.get(
      `pdex/v3/withdrawfeehistory?${prefix}&limit=${limit}&offset=${offset}`
    );
  }
  ...
  async apiGetOpenOrder(params) {
    const { ID, otaKey } = params;
    if (otaKey) {
      return this.http.post(`pdex/v3/pendinglimit`, { Otakey: otaKey });
    }
