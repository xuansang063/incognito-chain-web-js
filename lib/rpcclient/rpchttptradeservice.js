...
class RpcHTTPTradeServiceClient {
  ...
  apiEstimateTrade(params) {
    const { selltoken, buytoken, buyamount, feetoken, sellamount } = params;
    ...
    new Validator("getEstimateTrade-buyamount", buyamount).number();
    new Validator("getEstimateTrade-sellamount", sellamount).number();
    new Validator("apiEstimateTrade-feetoken", feetoken).required().string();
    let url = `/pdex/v3/estimatetrade?selltoken=${selltoken}&buytoken=${buytoken}&feeinprv=${
      feetoken === PRVIDSTR
    }`;
    if (sellamount) {
