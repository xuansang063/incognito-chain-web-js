...
class RpcHTTPTradeServiceClient {
  ...
  apiEstimateTrade(params) {
    ...
    new Validator("apiEstimateTrade-amount", amount).required().number();
    new Validator("apiEstimateTrade-feetoken", feetoken).required().string();
    const url = `/pdex/v3/estimatetrade?selltoken=${selltoken}&buytoken=${buytoken}&amount=${amount}&feetoken=${feetoken}`;
  ...
  apiGetPriceHistory(params) {
    ...
    new Validator("apiGetPriceHistory-datapoint", datapoint)
      .required()
      .number();
    new Validator("apiGetPriceHistory-fromtime", fromtime).required().string();
    const url = `pdex/v3/pricehistory?poolid=${poolid}&period=${period}&datapoint=${datapoint}&fromtime=${fromtime}`;
    console.log("url", url);
    return this.http.get(url);
  }
  ...
  async apiGetWithdrawContributeHistories(params) {
    const { nftId, limit = 1e9, offset = 0, poolId } = params;
    new Validator("apiGetWithdrawContributeHistories-nftID", nftId)
      .required()
      .string();
    new Validator("apiGetWithdrawContributeHistories-poolId", poolId)
      .required()
      .string();
    return this.http.get(
      `pdex/v3/withdrawhistory?poolid=${poolId}&nftid=${nftId}&limit=${limit}&offset=${offset}`
    );
  }
