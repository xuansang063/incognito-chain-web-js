  ...
  try {
    if (numOfOtherPks > 0) {
      cc = await account.rpcCoinService.apiGetRandomCommitments(
        tokenID,
        shardID,
        inputCoinsToSpent.length * numOfOtherPks,
        tokenID
      );
      cc.Commitments = cc.Commitments.map((item) => {
        let base58 = checkDecode(item).bytesDecoded;
        const result = base64Encode(base58);
        return result;
      });
      cc.PublicKeys = cc.PublicKeys.map((item) => {
        let base58 = checkDecode(item).bytesDecoded;
        const result = base64Encode(base58);
        return result;
      });
      // const limit = inputCoinsToSpent.length * numOfOtherPks;
...
const prepareInputForConvertTxV2 = async ({
  ...
  if (isPToken) {
    ...
    const unspentPRV_v2 = (await account.getUnspentCoinsV2(PRVIDSTR)) || [];
    ...
    const shardID = getShardIDFromLastByte(
      account.key.KeySet.PaymentAddress.Pk[account.key.KeySet.PaymentAddress.Pk.length - 1]
    );
