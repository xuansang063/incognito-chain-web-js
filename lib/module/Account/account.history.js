...
async function getTxsHistory({ tokenID, isPToken = false } = {}) {
  ...
  try {
    ...
    if (isPToken) {
      task.push(this.getPTokenHistoryByTokenID({ tokenID }));
    }
    ...
    if (txsPToken.length > 0) {
      ...
      console.log(
        "txsPToken after filter by txsTransactor",
        txsReceiver.length
      );
    }
    ...
    await this.clearTxsHistory({ tokenID });
    throw error;
  }
  return result;
}
