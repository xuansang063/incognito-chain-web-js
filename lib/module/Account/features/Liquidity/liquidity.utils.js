...
import { uniqBy, map, orderBy, isString } from "lodash";
...
const mergeTokens = ({
  ...
  let pairs = Object.keys(pdePoolPairs).map((key) => {
    ...
    const token1Value = toRealTokenValue(
      tokens,
      pair.Token1IDStr,
      pair.Token1PoolValue
    );
    const token2Value = toRealTokenValue(
      tokens,
      pair.Token2IDStr,
  ...
  let pairTokens = tokens.filter((token) =>
    pairs.find((pair) => pair.keys.includes(token.id))
  );

  const { userPairs, feePairs } = pairs.reduce(
    (prev, pairInfo) => {
      ...
      let feeShareKey = Object.keys(sharesFee).find((key) =>
        key.includes(mergeKey)
      );
  ...
  tokens = tokens.map((token) => {
    ...
    userPairs.forEach((pair) => {
      ...
      if (
        ...
        token2 &&
        token.id !== PRVIDSTR &&
        (token.id === token1.id || token.id === token2.id)
      ) {
        share = pair[token.id];
      }
    });
