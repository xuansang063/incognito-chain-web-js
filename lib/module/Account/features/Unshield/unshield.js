...
async function createAndSendBurningRequestTx({
  ...
  try {
    ...
    const burningReqMetadata = newBurningMetadata(
      {
        metadataType: burningType,
...
function newBurningMetadata(
  { metadataType, burnerAddress, tokenID, burningAmount, remoteAddress }, 
  burningMetadata = {},
) {
  if (metadataType === BurningSOLRequestMeta || metadataType === BurningSOLForDepositToSCRequestMeta) {
    return {
      BurningAmount: burnAmount,
      TokenID: tokenID,
      RemoteAddress: remoteAddress,
