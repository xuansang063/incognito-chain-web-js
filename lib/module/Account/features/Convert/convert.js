...
async function createAndSendConvertPToken({ tokenID , balance, fee = MAX_FEE_PER_TX }) {
  new Validator("createAndSendConvertPToken-tokenID", tokenID).required().string();
  new Validator("createAndSendConvertPToken-balance", balance).required().amount();
    ...
    try {
      ...
      if (!nextStep) {
        ...
        throw new CustomError(
          ErrorObject.PrepareInputForFeeErr,
          ErrorObject.PrepareInputForFeeErr.description,
        );
      }
...
async function createAndSendConvertNativeToken({ tokenID = PRVIDSTR, balance, fee = MAX_FEE_PER_TX }) {
  new Validator("createAndSendConvertNativeToken-tokenID", tokenID).string();
  new Validator("createAndSendConvertNativeToken-balance", balance).required().amount();
