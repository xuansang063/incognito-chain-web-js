...
const TXS_PORTAL_KEY = "TXS_PORTAL_KEY";
const { 
	TX_TYPE, 
	TX_TYPE_STR, 
...
async function updateStatusPortalTxs( newPortalShieldTxs, newPortalUnShieldTxs ){
	...
	const unshieldTasks = newPortalUnShieldTxs.map(async (unshieldTx) => {
		...
		const { ExternalTxID, Status, ExternalFee, RemoteAddress } = unshieldStatus;
...
async function getTxsPortal(params, txsReceiver, txsTransactor) {
  try {
    ...
    const key = this.getKeyTxsPortalStorage(params); 
		const oldDetailPortalTxs = await this.getSetKeysStorage({ key }); 
    ...
    if (txsPortalShield.length === 0 && txsPortalUnShield.length === 0) {
      return oldDetailPortalTxs;
		...
		const finishedUnshieldTxs = oldDetailPortalTxs.filter(
			...
			(tx.status === STATUS_CODE_UNSHIELD_PORTAL.COMPLETE || STATUS_CODE_UNSHIELD_PORTAL.REFUND)
		);
		...
		newPortalShieldTxs = await this.updateStatusPortalTxs(newPortalShieldTxs, newPortalUnShieldTxs);

		// merge new portal txs and old portal txs from storage
		const allPortalTxs = [ ...newPortalShieldTxs, ...newPortalUnShieldTxs, ...oldDetailPortalTxs ];
		...
		await this.setSetKeysStorage({ tokenID, setKeys: portalTxs, key });
		return {
			txsPortal: portalTxs,
			txsReceiver: newTxsReceiver,
			txsTransactor: newTxsTransactor,
				...
				switch (type) {
					...
					case PortalV4UnshieldingResponseMeta:  	// filter unshield txs (refunded)
					{
						...
						if (foundIndex > -1)  {
							...
							txsPortalUnShield.push(_txsTransactor[foundIndex]);
							_txsTransactor.splice(foundIndex, 1); 
							return false;
						}
					}
