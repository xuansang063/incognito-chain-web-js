...
async function saveTxHistory(params) {
  try {
    const { tx, version, tokenID: _tokenID } = params;
    let tokenID = tx?.tokenID || _tokenID;
    new Validator("saveTxHistory-tx", tx).required().object();
    ...
    if (tx) {
      try {
        ...
        if (Number.isFinite(status) && status >= 0) {
          await this.saveTxHistory({ tx: { ...tx, status }, ...params });
        }
      } catch {
...
async function getTransactorHistoriesByTokenID(params) {
  ...
  try {
    ...
    new Validator("getTransactorHistoriesByTokenID-tokenID", tokenID)
      .required()
      .string();
    new Validator("getTransactorHistoriesByTokenID-number", number)
      .required()
      .number();
    const key = this.getKeyTxHistoryByTokenId(params);
...
async function getTxsHistory(params) {
  ...
  try {
    ...
    new Validator("getTxsHistory-isPToken", isPToken).boolean();
    new Validator("getTxsHistory-version", version).required().number();
    let task = [
      this.getSetKeyImages(params), //
      this.getSetPublicKeys(params), //
    ];
    if (isPToken) {
      task.push(this.getPTokenHistoryByTokenID(params));
    }
    let [setKeyImages, setPublicKeys, txsPToken = []] = await Promise.all(task);
    ...
    if (txsPToken.length > 0) {
      ...
      console.log("txsPToken after mapping by txsTransactor", txsPToken.length);
    }
    ...
    if (txsReceiver.length > 0) {
      ...
      console.log("txsReceiver after filter by txsPToken", txsReceiver.length);
    }
