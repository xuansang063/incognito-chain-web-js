...
function handleFilterTxsTransactorByTxsPToken({ txsPToken, txsTransactor }) {
  ...
  try {
    ...
    _txsTransactor = _txsTransactor.filter((txt) => {
      ...
      console.log("\ntxt", txt?.txId, "foundIndex", foundIndex);
      if (foundIndex > -1) {
        return false;
      }
      return true;
...
async function removeTxHistoryByTxIDs({ txIDs, tokenIDs, version }) {
  try {
    ...
    new Validator("removeTxHistoryByTxIDs-tokenIDs", tokenIDs).required().array();
    new Validator("removeTxHistoryByTxIDs-version", version).required().number();
    ...
    const tasks = uniq(tokenIDs).map(async (tokenID) => {
      const oldHistories = (await this.getTransactorHistoriesByTokenID({
        tokenID,
        version,
      })) || [];
      const newHistories = oldHistories.filter(history => !txIDs.includes(history?.txId));
