...
import { TX_STATUS, TX_STATUS_STR, TX_TYPE, TX_TYPE_STR, } from '@lib/module/Account/account.constants';
...
async function handleFilterTxsTransactorByTxsReceiver({ txsTransactor, txsReceiver }) {
  let _txsTransactor = []
  try {
    new Validator("handleFilterTxsTransactorByTxsReceiver-txsReceiver", txsReceiver).required().array();
    new Validator("handleFilterTxsTransactorByTxsReceiver-txsTransactor", txsTransactor).required().array();
    _txsTransactor = (txsTransactor || []).filter(({ txId }) => {
      return !(txsReceiver || []).some(txsReceiver => txsReceiver.txId === txId);
    })
  } catch (e) {
    throw e;
  }
