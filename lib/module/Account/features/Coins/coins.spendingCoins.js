...
async function getSpendingCoinsStorageByTokenId(params) {
  try {
    ...
    const spendingCoinsFilter = spendingCoins.filter(item => (Date.now() - item.createdAt) > lockTime)
    ...
    const spendingCoinsFilterByTime = spendingCoinsMapStatus.filter(item => {
      ...
      const isTxEnd = (
        ...
        ||
        ((timeExist > timeExpired) && ((status === TX_STATUS.TXSTATUS_CANCELED) || (status === TX_STATUS.TXSTATUS_FAILED)))
      )
      return !isTxEnd;
    })
...
async function getCoinsInMempoolCached() {
  const coins = await cachePromise(CACHE_KEYS.COIN_MEMPOOL_CACHE, this.rpcCoinService.apiGetSpendingCoinInMemPool);
  return coins || [];
}
...
async function getUnspentCoinsExcludeSpendingCoins(params) {
  try {
    const { tokenID, version } = params;
    new Validator(`getUnspentCoinsExcludeSpendingCoins-tokenID`, tokenID).required().string();
    new Validator(`getUnspentCoinsExcludeSpendingCoins-version`, version).required().number();
