...
async function requestAirdropNFTNoCached() {
  ...
  return this.rpcRequestService.apiRequestAirdropNFT({ paymentAddress });
}
...
async function requestAirdropNFT() {
  ...
  try {
    if (!submited) {
      const OTAKEY = this.getOTAKey();
      const key = CACHE_KEYS.REQUEST_AIRDROP_NFT + OTAKEY;
      status = await cachePromise(key, this.requestAirdropNFTNoCached.bind(this), 10000);
    }
  } catch (e) {
    throw new CustomError(
  ...
  try {
    ...
    console.log('setFlagAirdropNFT error: ', e)
  }
}
