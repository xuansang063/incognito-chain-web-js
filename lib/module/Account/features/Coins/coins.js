...
async function decryptCoins({ coins, version }) {
  try {
    ...
    const { SNDerivators, PublicKeys } = coins.reduce((prev, coin) => {
    ...
    if (coins.length > LIMIT_PAGES) {
      ...
      for (let index = 0; index < times; index++) {
        const sliceData = coins.slice(index * LIMIT_PAGES, (index * LIMIT_PAGES) + LIMIT_PAGES);
      ...
      if (remainder > 0) {
        let decryptCoins = coins.slice(times * LIMIT_PAGES, (times * LIMIT_PAGES) + remainder);
    ...
    decryptedCoins = [...decryptedCoins.map((coin) => {
    ...
    const uniqValue = version === PrivacyVersion.ver2 ? 'PublicKey' : 'SNDerivator';
    decryptedCoins = uniqBy(decryptedCoins, item => item[uniqValue]);
    if (
      (version === PrivacyVersion.ver2 && PublicKeys.length !== decryptedCoins.length) ||
      (version === PrivacyVersion.ver1 && SNDerivators.length !== decryptedCoins.length)) {
