...
// Lua coin nho hon threshold (nano token amount) -> rac -> gop
// 100 to, 70 amount < threshold (200) -> lua group 30, 30, 10
...
async function prepareInputConsolidate({
  ...
  try {
    ...
    new Validator("prepareInputConsolidate-version", version)
      .required()
      .number();
    new Validator("prepareInputConsolidate-threshold", threshold)
      .required()
      .number();
    const params = {
      tokenID,
    ...
    const spendableCoins = await this.getSpendingCoins(params);
    ...
    const paramsCC = {
      ...
      shardID,
      version,
      limit: numOfOtherPks * groupedCoins.length,
    };
    let coinsForRing = await this.rpcCoinService.apiGetRandomCommitments(
      paramsCC
    );
...
async function consolidate({
  ...
  extra: {
    inputsPerTx = MaxInputNumberForDefragment,
    threshold = 200,
    version,
  } = {},
}) {
  ...
  new Validator("consolidate-threshold", threshold).required().number();
  ...
  if (isToken) {
    ...
    for (const inputs of groupedInputs) {
      const { inputCoinStrs: prvInputs, coinsForRing: prvCoinsForRing } =
        await prepareInputForTxV2({
          amountTransfer: new bn(0).toString(),
          fee,
          tokenID: PRVIDSTR,
          account: this,
          version,
    ...
    const task = groupedInputs.map(async (inputs) => {
