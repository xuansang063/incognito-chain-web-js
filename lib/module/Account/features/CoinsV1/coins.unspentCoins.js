...
import { LIMIT, MAX_FEE_PER_TX } from '@lib/module/Account/account.constants';
import flatten from 'lodash/flatten';
import isEmpty from 'lodash/isEmpty';
import { DEFAULT_INPUT_PER_TX, MIN_AMOUNT_COIN_CONVERT } from '@lib/tx/constants';
  ...
  try {
    ...
    let { balance, unspentCoinsFiltered } = unspentCoinExceptSpendingCoin?.reduce(
      (prev, coin) => {
        ...
        if (tokenID === PRVIDSTR && amount.toNumber() <= MIN_AMOUNT_COIN_CONVERT ) {
          return prev;
        }
        return {
    ...
    if (balance.toNumber() <= MAX_FEE_PER_TX && tokenID === PRVIDSTR) {
      balance = new bn(0);
