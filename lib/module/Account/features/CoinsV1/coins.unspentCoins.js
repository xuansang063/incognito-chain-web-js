...
async function getUnspentCoinsByTokenIdV1({ tokenID, total, fromApi = true, version } = {}) {
  ...
  const LIMIT_COINS = LIMIT * 2;
  if (listOutputsCoins.length > LIMIT_COINS) {
    const { times, remainder } = pagination(listOutputsCoins.length);
    for (let index = 0; index < times; index++) {
      const { unspentCoins } = await this.checkKeyImages({
        listOutputsCoins: listOutputsCoins.slice(index, LIMIT_COINS),
        shardId,
        version,
        tokenID,
      });
      listUnspentCoinsFiltered = listUnspentCoinsFiltered.concat(unspentCoins);
    }
    if (remainder > 0) {
      const { unspentCoins } = await this.checkKeyImages({
        listOutputsCoins: listOutputsCoins.slice(times * LIMIT_COINS, LIMIT_COINS),
        shardId,
        version,
        tokenID,
      })
    ...
    listUnspentCoinsFiltered = (await this.checkKeyImages({
      listOutputsCoins,
      shardId,
      version,
      tokenID,
