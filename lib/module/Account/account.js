...
class Account {
  ...
  async getUnspentCoinsV1ByTokenId({ tokenId, total }) {
    ...
    const balance = (
      listUnspentCoinsFiltered?.reduce(
    ...
    console.debug("\n---------------------------------------");
    console.debug(`Unspent Coins V1 TokenId: ${tokenId}, balance: ${balance}`);
  ...
  async getAllUnspentCoinsV1() {
    if (!this.coinsV1Storage) this.coinsV1Storage = {};
  ...
  async getSpendingCoins(tokenId = PRVIDSTR) {
    ...
    try {
      ...
      try {
        ...
        const spendingCoins = await this.rpcCoinService.apiGetSpendingCoinInMemPool();
...
Object.assign(Account.prototype, transactor);
Object.assign(Account.prototype, history);
Object.assign(Account.prototype, progress);
