...
class Account {
  ...
  setRPCCoinServices(url) {
    new Validator("rpc coin serivce url", url).required().string();
  ...
  async setSpendingCoinsStorage({ coins, tokenId = PRVIDSTR } = {}) {
    try {
      ...
      console.log("GET SPENDING COINS", coins.length, tokenId);
      if (!coins) {
        return;
      }
      const task = coins
        ...
        .map(async (item) => {
          ...
          console.log("spendingCoins", spendingCoins.length);
          await this.setAccountStorage(key, spendingCoins);
        });
  ...
  async decryptCoins({ coins }) {
    try {
      ...
      result = result.map((coin) => {
        const keyImageBase64 = coin.KeyImage;
        return {
          ...coin,
  ...
  async getUnspentCoinsByTokenIdV1({ tokenId, total }) {
    if (Array.isArray(this.coinsV1Storage?.unspentCoinV1)) {
      return this.coinsV1Storage?.unspentCoinV1.find(coin => coin.tokenId === tokenId);
    }
    ...
    let { balance, unspentCoinsFiltered  } = listUnspentCoinsFiltered?.reduce((prev, coin) => {
  ...
  async getKeyListUnspentCoins() {
    return (
      this.getPrefixKeyStorage() +
      `-${TOTAL_UNSPENT_COINS}-${this.getReadonlyKey()}`
    );
  }
  ...
  async getListUnspentCoinsV1() {
    try {
      if (this.storage) {
        this.setPrivacyVersion(PrivacyVersion.ver1);
        const key = this.getKeyListUnspentCoins();
        await this.getAccountStorage(key);
      }
    } catch (error) {
      console.debug("ERROR GET ACCOUNT STORAGE", error?.message);
    }
  ...
  async getAllUnspentCoinsV1() {
    ...
    if (!isEmpty(this.coinsV1Storage.unspentCoinV1)) {
      return this.coinsV1Storage.unspentCoinV1;
    }
  ...
  async getSpendingCoins(tokenId = PRVIDSTR) {
    ...
    try {
      ...
      try {
        ...
        console.log("spendingCoinsStorage", spendingCoinsStorage.length);
        coins = coins.filter(
          (item) =>
        ...
        console.log("coins after filter by storage", coins.length);
        const spendingCoins =
          await this.rpcCoinService.apiGetSpendingCoinInMemPool();
        console.log("spendingCoins in mempool", spendingCoins.length);
        if (!!spendingCoins) {
          coins = coins.filter(
            (coin) => !spendingCoins.includes(coin.KeyImage)
          );
        }
        console.log("coins after filter by mempool", coins.length);
      } catch (error) {
        throw error;
      }
      return coins || [];
...
Object.assign(Account.prototype, transactor, history, progress, transactorConvert);
