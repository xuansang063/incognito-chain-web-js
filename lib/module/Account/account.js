...
class Account {
  ...
  async getFollowTokensBalance({ defaultTokens = [] }) {
    try {
      ...
      if (!isFollowTokens) {
        await this.followingDefaultTokens({ tokenIDs: defaultTokens }),
        followTokens = uniq(defaultTokens);
      } else {
        followTokens =
      ...
      balance = orderBy(balance, [(c) => c.tokenID === PRVIDSTR, (c) => Number(c.balance || '0')], ['desc', 'desc'])
      return {
        followTokens,
        balance
      }
