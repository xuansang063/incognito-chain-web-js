...
class Account {
  ...
  getKeySpendingCoinsV1ByTokenId({ tokenId }) {
    ...
    const key =
      this.getPrefixKeyStorage() + `-${tokenId}-${SPENDING_COINS_STORAGE}`;
    return key;
  }
  ...
  async getKeyListUnspentCoins() {
    const key =
      this.getPrefixKeyStorage() + `-${TOTAL_UNSPENT_COINS}-${viewkey}`;
    return key;
  }
  ...
  async getUnspentCoinsV2(tokenId = PRVIDSTR) {
    ...
    const listUnspentCoinsMerged = (
      await this.getListUnspentCoinsStorage(tokenId)
