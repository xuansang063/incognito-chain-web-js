...
class Account {
  ...
  async setAccountStorage(key, value) {
    try {
      ...
      console.log("typeof", typeof this.storage.setItem, this.storage.getItem);
      if (this.storage) {
  ...
  getKeySpendingCoinsV1ByTokenId({ tokenId }) {
    ...
    const key = this.getPrefixKeyStorage() + `-${tokenId}-${SPENDING_COINS_STORAGE}`;
    return key;
  }
  ...
  async storeListSpentCoins({ tokenId, spentCoins }) {
    new Validator("tokenId", tokenId).required().string();
    new Validator("spentCoins", spentCoins).array().required();
  ...
  async getKeyListUnspentCoins() {
    const key = this.getPrefixKeyStorage() + `-${TOTAL_UNSPENT_COINS}-${viewkey}`;
    return key;
  }
  ...
  async getUnspentCoinsV2(tokenId = PRVIDSTR) {
    ...
    const listUnspentCoinsMerged = (await this.getListUnspentCoinsStorage(
      tokenId
    )).filter(coin => coin.Version !== PrivacyVersion.ver1.toString());
    if (!this.coinUTXOs) {
      this.coinUTXOs = {};
    }
