import { MULTI_CALL_ABI } from "./pancake.constants";

export async function getBestRateFromPancake({
  sourceToken,
  destToken,
  amount,
  chainID,
  ...
  const PANCKAE_ROUTER_INST = new web3.eth.Contract(
    PANCAKE_ABI,
    PANCAKE_ROUTER_V2
  );
  let pairList = [];
  ...
  let outputs;
  if (!isSwapExactOut) {
    outputs = await PANCKAE_ROUTER_INST.methods
