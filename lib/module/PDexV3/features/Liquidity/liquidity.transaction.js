  ...
  try {
    ...
    if (isToken) {
      result = await this.account?.transact({
        transfer: { fee, info, tokenID, tokenPayments: burningPayments }, extra: { metadata, version, txType: TX_TYPE.CONTRIBUTE }
      });
    } else {
      result = await this.account?.transact({
        transfer: { fee, info, tokenID, prvPayments: burningPayments }, extra: { metadata, version, txType: TX_TYPE.CONTRIBUTE }
      });
    }
