...
async function createAndSendWithdrawContributeRequestTx(params) {
  try {
    ...
    const { nftToken: nftID } = await this.getNFTTokenData({
      version: PrivacyVersion.ver2,
    });
    console.log('SANG TEST: ', params)
...
async function createAndSendWithdrawLPFeeRequestTx (params) {
  try {
    ...
    await this.setStorageWithdrawLPWithPool({ poolId: poolPairID, nftId: nftID, txId: result.txId })
    await this.account?.updateProgressTx(100, "Completed");
    return result;
  } catch (e) {
    throw e;
