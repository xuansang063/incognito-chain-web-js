import { PaymentAddressType, PrivacyVersion, Validator } from '@lib/wallet';
...
async function createAndSendWithdrawLPFeeRequestTx(params) {
  try {
    ...
    new Validator("createAndSendWithdrawLPFeeRequestTx-amount1", amount1).required().string();
    new Validator("createAndSendWithdrawLPFeeRequestTx-amount2", amount2).required().number();
    await this.account?.updateProgressTx(10, "Generating Metadata");
    ...
    if (result && withdrawTokenIDs.length === 2) {
      ...
      console.log('setStorageWithdrawLPWithPool: params', params)
      await this.setStorageWithdrawLPWithPool(params)
    }
    await this.account?.updateProgressTx(100, "Completed");
    return result;
  } catch (e) {
