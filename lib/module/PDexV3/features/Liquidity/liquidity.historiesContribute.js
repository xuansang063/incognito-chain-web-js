...
async function getStorageContributeTxs({
  accountInst,
  tokenIDs
}) {
  new Validator("getStorageContributeTxs-accountInst", accountInst).required().object();
  new Validator("getStorageContributeTxs-tokenIDs", tokenIDs).required().array();
  const tasks = tokenIDs.map(async (tokenID) => {
    let histories = (await accountInst.getTransactorHistoriesByTokenID({
      tokenID,
    ...
    const tasks = histories.map(async (history) => {
      ...
      if (history.createdAt && (Date.now() - createdAt) < (2 * 60 * 1000)) {
        ...
        status = await accountInst.rpcTxService.apiGetTxStatus({ txId });
      }
...
async function getContributeHistories({
  ...
  new Validator("getContributeHistories-accountInst", accountInst).required().object();
  const tasks = [
  ...
  const storageContributeTxs = await this.getStorageContributeTxs({ accountInst, tokenIDs });
