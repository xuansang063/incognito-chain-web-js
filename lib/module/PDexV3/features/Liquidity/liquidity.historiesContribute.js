...
function mapHistories(histories) {
  return histories.map(history => {
    const { isStorage, storageContribute, requestTxs } = history;
    ...
    if (isStorage || requestTxs.length === 2 || isWaiting) return histories;
    let retry = {};
    let refund = {};
    refund = {
    ...
    console.log('mapHistories: ', { retry, refund })
    return {
      ...histories,
      retry,
      refund,
...
async function getContributeHistories({ offset = 0, limit = LIMIT_DEFAULT } = {}) {
  ...
  csvHistories = csvHistories.map(history => {
    ...
    console.log('getContributeHistories: storageContribute', storageContribute, pairHash)
    return {
      ...history,
      storageContribute,
      statusStr: CONTRIBUTE_HISTORIES_TX_STR[status],
  ...
  console.log('getContributeHistories: csvHistories', csvHistories);
  console.log('getContributeHistories: diffHistories', diffHistories);
