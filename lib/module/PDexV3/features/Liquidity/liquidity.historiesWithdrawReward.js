...
async function getWithdrawRewardHistories({ offset = 0, limit = LIMIT_DEFAULT, accountInst } = {}) {
  ...
  new Validator("getWithdrawRewardHistories-accountInst", accountInst).required().object();

  const tasks = [
    await this.getLiquidityWithdrawFeeHistoriesApi({ offset, limit }),
    await accountInst.getTransactorHistoriesByTokenID({ tokenID: PRVIDSTR, version: PrivacyVersion.ver2 })
  ];
  ...
  const tasksStorage = spendingStorage.map(async (history) => {
    ...
    const status = await accountInst.rpcTxService.apiGetTxStatus({ txId });
