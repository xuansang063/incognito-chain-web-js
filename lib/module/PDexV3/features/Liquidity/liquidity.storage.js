 ...
 */
async function setStorageHistoriesRemoveLP({
  amount1,
  amount2,
  requestTx,
  status,
  ...
  const key = this.getStorageHistoriesRemoveLP();
  const histories = (await this.getStorageHistoriesRemoveLP()) || [];
  const isExist = histories.some(history => requestTx === history?.requestTx)
  if (!isExist) {
    const params = { amount1, amount2, requestTx, status, tokenId1, tokenId2, requestTime: lockTime };
...
function getKeyStorageWithdrawLP() {
  ...
  return `${STORAGE_KEYS.WITHDRAW_LP}-${address}`;
}
