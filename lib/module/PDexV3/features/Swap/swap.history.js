import { TX_STATUS_STR, TX_TYPE, TX_STATUS } from "@lib/module/Account";
...
async function setStorageSwapTokenIDs({ tokenIDs = [], version }) {
  try {
    ...
    const key = this.getKeySwapTokenIDs({ version });
    await this.setStorage(key, newTokenIDs);
  } catch (error) {
    ...
    history = history?.map((h) => {
      try {
        ...
        const result = {
          ...
          respondTxs: [],
          status: TX_STATUS_STR[TX_STATUS.TXSTATUS_PENDING],
          statusCode: -1,
          fee: TradingFee,
          feeToken: FeeToken,
          fromStorage: true,
...
async function getSwapHistoryFromApi({ version, limit, offset }) {
  ...
  try {
    ...
    history = history
      ...
      ?.map((h) => ({ ...h, requestime: h?.requestime * 1000 }));
  } catch (error) {
