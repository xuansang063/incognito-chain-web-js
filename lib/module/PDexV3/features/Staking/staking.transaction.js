import { PaymentAddressType, PRVIDSTR, Validator } from '@lib/wallet';
...
async function createAndSendStakeRequestTx(params) {
  try {
    const {
      transfer: { fee, info = "", tokenID },
      extra: { tokenAmount, nftID, version }
    } = params
...
async function createAndSendStakeWithdrawRequestTx(params) {
  try {
    const {
      transfer: { fee, info = "" },
      extra: { unstakingAmount, nftID, stakingTokenID, stakingPoolID, version }
    } = params
...
async function createAndSendStakeWithdrawRewardRequestTx(params) {
  try {
    const { transfer: { fee, info = "" }, extra: { stakingTokenID, nftID, version }} = params;
...
export default ({
  createAndSendStakeRequestTx,
  createAndSendStakeWithdrawRequestTx,
  createAndSendStakeWithdrawRewardRequestTx,
})
