...
async function getListShare() {
  ...
  try {
    ...
    console.log("getListShare tsx", tsx);
    const tasks = nftIds.map(async (nftId) => {
      ...
      return res.map((item) => {
        ...
        const { poolId, token1Reward, token2Reward } = item;
        let withdrawing = false;
        const withdrawable = !!token1Reward || !!token2Reward;
        const key = this.getKeyWithdrawLPWithPool({ nftId, poolId });
        console.log("getListShare key", key);
        if (tsx[key]) {
          withdrawing = true;
        }
        return {
