...
async function getShareWithOTAKey() {
  ...
  try {
    ...
    let burningTxs = this.getStorageBurningTx();

    // Get List Share from api
    // Get Burning Status
    [share, burningTxs] = await Promise.all([
      (await cachePromise(`LIST_SHARE-${otaKey}`, () =>
          this.rpcTradeService.apiGetListShare({ otaKey })
      )) || [],
      burningTxs.map(async (item) => {
        const status = this.rpcTxService.apiGetTxStatus({ txId: item.txID })
        return {
          ...item,
          status,
        }
    ...
    share = share.map(async (item) => {
      ...
      const withdrawable = checkWithdrawableContribute(item);
      let waitBurningTx = false;
      const foundBurnTx = (burningTxs || []).find(burn => burn.accessID === item.nftId);
      if (foundBurnTx) {
        waitBurningTx =
            !(
