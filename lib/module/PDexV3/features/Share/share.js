  ...
  try {
    ...
    const tasks = nftIds.map(async (nftId) => {
      ...
      return res.map((item) => {
        ...
        const { poolId, token1Reward, token2Reward, prvReward, dexReward } = item;
        let withdrawing = false;
        const withdrawable = !!token1Reward || !!token2Reward || !!prvReward || !!dexReward;
...
async function getPriceHistory(params) {
  ...
  try {
    ...
    new Validator("getPriceHistory-fromtime", fromtime).required().string();
