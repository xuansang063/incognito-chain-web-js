...
async function getOrderLimitDetail({ requestTx }) {
  let result = {};
  try {
...
async function getOrderLimitHistoryFromStorage({
  ...
  try {
    ...
    let history = await Promise.all([
      this.account?.getTransactorHistoriesByTokenID({
        tokenID: token1ID,
        version,
      }),
      this.account?.getTransactorHistoriesByTokenID({
    ...
    history = history.map((h) => {
      try {
        const { metadata, txId, tx, tokenID } = h;
        const {
          SellAmount,
          MinAcceptableAmount,
          PoolPairID,
        ...
        const result = {
          ...
          nftid: NftID,
          requestTx: txId,
          requestime: tokenID === PRVIDSTR ? tx?.LockTime : tx?.Tx?.LockTime,
