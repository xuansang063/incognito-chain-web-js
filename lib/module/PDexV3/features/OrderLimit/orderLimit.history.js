...
async function getOrderLimitHistoryFromStorage({ tokenIds, version }) {
  ...
  try {
    ...
    history = flatten(history);
    historyStorage = history.filter((h) => !h.isSubmitted & !!h.requestTx && !h?.isCompleted);
  } catch (error) {
    console.log("error", error);
  }
...
async function getOpenOrderLimitHistoryFromApi({ version }) {
  let history = [];
  try {
    new Validator("apiGetHistory-version", version).required().number();
    const nftData = await this.getNFTTokenData({ version });
    const { listNFTToken } = nftData;
    new Validator("apiGetHistory-listNFTToken", listNFTToken)
      .required()
      .array();
    if (!listNFTToken) {
    ...
    history =
      (await this.rpcTradeService.apiGetOpenOrder({
        ID: listNFTToken,
      })) || [];
    history = history
      .filter((h) => !!h?.RequestTx)
