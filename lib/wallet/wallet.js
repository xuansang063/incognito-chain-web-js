...
class AccountWallet {
  constructor() {
    ...
    this.derivePointCache = {}
    this.deriveStringCache = {}
    this.sCCStorage = {}
    this.iCOStorage = {}
    this.iCJStorage = {}
  };
  ...
  mergeDeriveCache() {
    for (let k in this.derivePointCache) {
      if (k != undefined && this.deriveStringCache[k] == undefined) {
        this.deriveStringCache[k] = base58.checkEncode(this.derivePointCache[k].compress(), 0x00);
      }
    }
    console.log(`${this.name} mergeDeriveCache`, this.deriveStringCache);
  }

  async loadDeriveCache() {
    for (let k in this.deriveStringCache) {
      if (k != undefined && this.derivePointCache[k] == undefined) {
        this.derivePointCache[k] = P256.decompress(base58.checkDecode(this.deriveStringCache[k]).bytesDecoded);
      }
    }
    console.log(`${this.name} loadDeriveCache`, this.deriveStringCache);
  }

  mergeICOStorage(allOutputCoinStrs, inputCoins) {
    this.iCOStorage = this.iCOStorage == undefined ? {} : this.iCOStorage
    this.iCJStorage = this.iCJStorage == undefined ? {} : this.iCJStorage
    for (let i = 0; i < allOutputCoinStrs.length; i++) {
      const sndStr = allOutputCoinStrs[i].SNDerivator;
      const oObject = inputCoins[i];
      if (this.iCOStorage[sndStr] == undefined) {
        this.iCOStorage[sndStr] = oObject;
        const jsonObject = {};
        ...
        this.iCJStorage[sndStr] = jsonObject
      }
    }
    console.log(`${this.name} mergeICOStorage`, this.iCOStorage);
  }

  async loadICOStorage() {
    this.iCOStorage = this.iCOStorage == undefined ? {} : this.iCOStorage
    this.iCJStorage = this.iCJStorage == undefined ? {} : this.iCJStorage
    for (let sndStr in this.iCJStorage) {
      if (sndStr != undefined && this.iCOStorage[sndStr] == undefined) {
        const jsonObject = this.iCJStorage[sndStr];
    ...
    console.log(`${this.name} loadICOStorage`, this.iCOStorage);
  }

  analyzeFromICOStorage(allOutputCoinStrs) {
    this.iCOStorage = this.iCOStorage == undefined ? {} : this.iCOStorage
    console.log(`${this.name} analyzeFromICOStorage ICOStorage`, this.iCOStorage);
    let leftOutputCoinStrs = [];
    let cachedOutputCoinStrs = [];
    let cachedInputCoins = [];
    for (let i = 0; i < allOutputCoinStrs.length; i++) {
      const SNDerivator = allOutputCoinStrs[i].SNDerivator;
      if (this.iCOStorage[SNDerivator] != undefined) {
        cachedOutputCoinStrs.push(allOutputCoinStrs[i]);
        cachedInputCoins.push(this.iCOStorage[SNDerivator]);
      } else {
        leftOutputCoinStrs.push(allOutputCoinStrs[i]);
      }
    }
    console.log(`${this.name} analyzeFromICOStorage leftOutputCoinStrs`, leftOutputCoinStrs);
    console.log(`${this.name} analyzeFromICOStorage cachedOutputCoinStrs`, cachedOutputCoinStrs);
    console.log(`${this.name} analyzeFromICOStorage cachedInputCoins`, cachedInputCoins);
    return {
  ...
  mergeSCCStorage(unspentCoinStrs, inputCoins) {
    this.sCCStorage = this.sCCStorage == undefined ? {} : this.sCCStorage
    console.log(`${this.name} mergeSCCStorage unspentCoinStrs`, unspentCoinStrs);
    console.log(`${this.name} mergeSCCStorage inputCoins`, inputCoins);
    let chkAll = {};
    for (let i = 0; i < inputCoins.length; i++) {
      chkAll[inputCoins[i].coinDetails.SNDerivator] = true
    }
    for (let i = 0; i < unspentCoinStrs.length; i++) {
      chkAll[unspentCoinStrs[i].SNDerivator] = false
    }
    for(let sndStr in chkAll) {
      if (sndStr != undefined && chkAll[sndStr] == true) {
        this.sCCStorage[sndStr] = true
      }
    }
    console.log(`${this.name} mergeSCCStorage sCCStorage`, this.sCCStorage);
  }

  analyzeFromSCCStorage(inputCoins, allOutputCoinStrs) {
    console.log(`${this.name} analyzeFromSCCStorage inputCoins`, inputCoins);
    console.log(`${this.name} analyzeFromSCCStorage allOutputCoinStrs`, allOutputCoinStrs);
    this.sCCStorage = this.sCCStorage == undefined ? {} : this.sCCStorage
    let inputCoinsRet = []
    let allOutputCoinStrsRet = []
    for (let i = 0; i < inputCoins.length; i++) {
      const sndStr = inputCoins[i].coinDetails.snderivator;
      if (this.sCCStorage[sndStr] == undefined) {
        inputCoinsRet.push(inputCoins[i]);
        allOutputCoinStrsRet.push(allOutputCoinStrs[i]);
      }
    }
    console.log(`${this.name} analyzeFromSCCStorage inputCoinsRet`, inputCoinsRet);
    console.log(`${this.name} analyzeFromSCCStorage allOutputCoinStrsRet`, allOutputCoinStrsRet);
    return {
  ...
  async getBalance() {
    ...
    const { leftOutputCoinStrs, cachedOutputCoinStrs, cachedInputCoins } = this.analyzeFromICOStorage(allOutputCoinStrs);
    let inputCoins = cachedInputCoins
    if (leftOutputCoinStrs.length > 0) {
      let leftInputCoins = Wallet.RpcClient.parseInputCoinFromEncodedObject(leftOutputCoinStrs, this.key, this.derivePointCache);
      this.mergeICOStorage(leftOutputCoinStrs, leftInputCoins);
      inputCoins = inputCoins.concat(leftInputCoins);
      allOutputCoinStrs = cachedOutputCoinStrs.concat(leftOutputCoinStrs);
    }
    // analyze from cache
    let { inputCoinsRet, allOutputCoinStrsRet } = this.analyzeFromSCCStorage(inputCoins, allOutputCoinStrs);
    let unspentCoinList = await Wallet.RpcClient.getUnspentCoin(inputCoinsRet, paymentAddrSerialize, allOutputCoinStrsRet);
    // update cache
    this.mergeSCCStorage(unspentCoinList.unspentCoinStrs, inputCoins);
  ...
  async createAndSendConstant(param, fee) {
    ...
    try {
      ...
      let inputForTx;
      try {
        inputForTx = await Wallet.RpcClient.prepareInputForTx(senderSkStr, paymentInfos, new bn(fee));
      ...
      try {
        ...
        await tx.init(this.key.KeySet.PrivateKey, paymentAddressStr, paymentInfos,
          inputForTx.inputCoins, inputForTx.inputCoinStrs, new bn(fee), true, null, null);
...
class Wallet {
  ...
  save(password) {
    ...
    this.MasterAccount.mergeDeriveCache();
    let masterDerivePointCache = this.MasterAccount.derivePointCache;
    this.MasterAccount.derivePointCache = {};
    let masterICOStorage = this.MasterAccount.iCOStorage;
    this.MasterAccount.iCOStorage = {}
    let derivePointCaches = new Array(this.MasterAccount.child.length);
    let iCOStorages = new Array(this.MasterAccount.child.length);
    for (let i = 0; i < this.MasterAccount.child.length; i++) {
      this.MasterAccount.child[i].mergeDeriveCache()
      derivePointCaches[i] = this.MasterAccount.child[i].derivePointCache;
      iCOStorages[i] = this.MasterAccount.child[i].iCOStorage;
      this.MasterAccount.child[i].derivePointCache = {}
      this.MasterAccount.child[i].iCOStorage = {}
    }

    let data = JSON.stringify(this);

    this.MasterAccount.derivePointCache = masterDerivePointCache;
    this.MasterAccount.iCOStorage = masterICOStorage;
    for (let i = 0; i < derivePointCaches.length; i++) {
      this.MasterAccount.child[i].derivePointCache = derivePointCaches[i];
      this.MasterAccount.child[i].iCOStorage = iCOStorages[i];
    }
  ...
  async loadWallet(password) {
    if (this.Storage != null) {
      ...
      try {
        ...
        obj.MasterAccount.loadDeriveCache();
        obj.MasterAccount.loadICOStorage();
        ...
        for (let i = 0; i < obj.MasterAccount.child.length; i++) {
          ...
          obj.MasterAccount.child[i].loadDeriveCache();
          obj.MasterAccount.child[i].loadICOStorage();
        }
        delete obj.Storage
    ...
    for (let i = 0; i < this.MasterAccount.child.length; i++) {
      for (let j = 0; j < this.MasterAccount.child[i].trxHistory.NormalTrx.length; j++) {
        if (this.MasterAccount.child[i].trxHistory.NormalTrx[j].status == SuccessTx) {
          // console.log("TxID: ", this.MasterAccount.child[i].trxHistory.NormalTrx[j].txID)
      ...
      for (let j = 0; j < this.MasterAccount.child[i].trxHistory.CustomTokenTrx.length; j++) {
        if (this.MasterAccount.child[i].trxHistory.CustomTokenTrx[j].status == SuccessTx) {
          // console.log("TxID: ", this.MasterAccount.child[i].trxHistory.CustomTokenTrx[j].txID)
      ...
      for (let j = 0; j < this.MasterAccount.child[i].trxHistory.PrivacyCustomTokenTrx.length; j++) {
        if (this.MasterAccount.child[i].trxHistory.PrivacyCustomTokenTrx[j].status == SuccessTx) {
          // console.log("TxID: ", this.MasterAccount.child[i].trxHistory.PrivacyCustomTokenTrx[j].txID)
