...
class AccountWallet {
  constructor() {
    ...
    this.trxHistory = { NormalTrx: [], CustomTokenTrx: [], PrivacyCustomTokenTrx: [], StakingTrx : [] };
  ...
  // staking tx always send constant to address "1NHp16Y29xjc1PoXb1qwr65BfVVoHZuCbtTkVyucRzbeydgQHs2wPu5PC1hD"
  ...
  async createAndSendStakingTx(param, fee) {
    ...
    if (param.type == constantsWallet.ShardStakingType){
      amount = constantsWallet.AmountStakingShard;
      meta = {Type: constantsWallet.MetaStakingShard};
    } else{
      amount = constantsWallet.AmountStakingBeacon;
    ...
    receiverPaymentAddrStr[0] = constantsWallet.BurnAddress;

    let keyWallet = KeyWallet.base58CheckDeserialize(
      constantsWallet.BurnAddress
    );
