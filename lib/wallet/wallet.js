...
class AccountWallet {
  ...
  savePrivacyCustomTokenTx(txID, type, amount, fee, receiver, tokenName, tokenID, tokenSymbol, mileseconds, status){
    let saveTrxObj = new TrxHistoryInfo();
    saveTrxObj.addHistoryInfo(txID, type, amount, fee, receiver, tokenName, tokenID, tokenSymbol, mileseconds);
    saveTrxObj.updateStatus(status);
  ...
  async createAndSendPrivacyCustomToken(paymentInfos = null, tokenParams, receiverPaymentAddrStr) {
    ...
    try {
      ...
      this.savePrivacyCustomTokenTx(tx.txId, tx.type, tx.txTokenPrivacyData.amount, tx.fee, receiverPaymentAddrStr, tx.txTokenPrivacyData.propertyName, tx.txTokenPrivacyData.propertyID, tx.txTokenPrivacyData.propertySymbol, tx.lockTime, status);
      console.log("history: ", this.trxHistory);
