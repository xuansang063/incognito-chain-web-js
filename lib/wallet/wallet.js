...
class AccountWallet {
  ...
  saveCustomTokenTx(tx, receivers, status, isIn) {

    let saveTrxObj = new TrxHistoryInfo();
    saveTrxObj.addHistoryInfo(tx.txId, tx.type, tx.txTokenData.amount, tx.fee, receivers, tx.txTokenData.propertyName, tx.txTokenData.propertyID, tx.txTokenData.propertySymbol, tx.lockTime, isIn);
    saveTrxObj.updateStatus(status);
    this.trxHistory.CustomTokenTrx.push(saveTrxObj);
  };

  savePrivacyCustomTokenTx(tx, receivers, status, isIn) {
    let saveTrxObj = new TrxHistoryInfo();
    // , receiverPaymentAddrStr,
    saveTrxObj.addHistoryInfo(tx.txId, tx.type, tx.txTokenPrivacyData.amount, tx.fee, receivers, tx.txTokenPrivacyData.propertyName, tx.txTokenPrivacyData.propertyID, tx.txTokenPrivacyData.propertySymbol, tx.lockTime, isIn);
    saveTrxObj.updateStatus(status);
  ...
  async createAndSendConstant(paymentInfos, receiverPaymentAddrStr) {
    ...
    try {
      ...
      this.saveNormalTx(tx, amount, receiverPaymentAddrStr, null, null, null, status, false);
  ...
  async createAndSendCustomToken(paymentInfos = null, tokenParams, receiverPaymentAddrStr) {
    ...
    try {
      ...
      this.saveCustomTokenTx(tx, receiverPaymentAddrStr, status, isIn);
      console.log("history: ", this.trxHistory);
  ...
  async createAndSendPrivacyCustomToken(paymentInfos = null, tokenParams, receiverPaymentAddrStr) {
    ...
    try {
      ...
      this.savePrivacyCustomTokenTx(tx, receiverPaymentAddrStr, status, isIn);
      console.log("history: ", this.trxHistory);
