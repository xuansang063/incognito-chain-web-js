...
class AccountWallet {
  ...
  saveCustomTokenTx(tx, receivers, status, isIn, amount) {
    ...
    let saveTrxObj = new TrxHistoryInfo();
    saveTrxObj.addHistoryInfo(tx.txId, tx.type, amount, tx.fee.toNumber() / 100, receivers, tx.txTokenData.propertyName, tx.txTokenData.propertyID, tx.txTokenData.propertySymbol, tx.lockTime, isIn);
    saveTrxObj.updateStatus(status);
  ...
  savePrivacyCustomTokenTx(tx, receivers, status, isIn, amount) {
    ...
    saveTrxObj.addHistoryInfo(tx.txId, tx.type, amount, tx.fee.toNumber() / 100, receivers, tx.txTokenPrivacyData.propertyName, tx.txTokenPrivacyData.propertyID, tx.txTokenPrivacyData.propertySymbol, tx.lockTime, isIn);
    saveTrxObj.updateStatus(status);
  ...
  async createAndSendConstant(paymentInfos, receiverPaymentAddrStr, fee) {
    ...
    try {
      ...
      let inputForTx;
      try {
        inputForTx = await Wallet.RpcClient.prepareInputForTx(senderSkStr, paymentInfos, fee);
      ...
      try {
        ...
        await tx.init(this.key.KeySet.PrivateKey, paymentAddressStr, paymentInfos,
          inputForTx.inputCoins, inputForTx.inputCoinStrs, fee, true, null, null);
  ...
  async createAndSendCustomToken(paymentInfos = null, tokenParams, receiverPaymentAddrStr, fee) {
    ...
    try {
      ...
      try {
        ...
        inputForTx = await Wallet.RpcClient.prepareInputForTx(senderSkStr, paymentInfos, new bn(0));
      ...
      try {
        ...
        inputForCustomTokenTx = await Wallet.RpcClient.prepareInputForCustomTokenTx(senderSkStr, tokenParams, fee);
      } catch (e) {
        throw e;
      }
      ...
      try {
        await tx.init(this.key.KeySet.PrivateKey, paymentAddressStr, paymentInfos, inputForTx.inputCoins, inputForTx.inputCoinStrs, fee, tokenParams, inputForCustomTokenTx.listCustomToken, null, false);
      } catch (e) {
  ...
  async createAndSendPrivacyCustomToken(paymentInfos = null, tokenParams, receiverPaymentAddrStr, fee) {
    let senderSkStr = this.key.base58CheckSerialize(constantsWallet.PriKeyType);
    let paymentAddressStr = this.key.base58CheckSerialize(constantsWallet.PaymentAddressType);
    ...
    try {
      ...
      try {
        ...
        inputForPrivacyCustomTokenTx = await Wallet.RpcClient.prepareInputForTxCustomTokenPrivacy(senderSkStr, tokenParams, fee);
      } catch (e) {
        throw e;
      }
      ...
      try {
        ...
        await tx.init(this.key.KeySet.PrivateKey, paymentAddressStr, paymentInfos, inputForTx.inputCoins, inputForTx.inputCoinStrs, fee, tokenParams, inputForPrivacyCustomTokenTx.listCustomToken, null, false);
      } catch (e) {
        throw e;
      }
