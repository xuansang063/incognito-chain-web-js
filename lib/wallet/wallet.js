...
import * as txConstant from '../tx/constants';
...
class AccountWallet {
  ...
  saveNormalTrx = (tx) => {
    let fee = tx.fee;
    let receiver = new Array(tx.proof.outputCoins.length);
    for (let i=0;i<tx.proof.outputCoins.length;i++){
      receiver.push(tx.proof.outputCoins[i].coinDetails.publicKey)
    }
    let saveTrxObj = {
      Amount: 0,
      Fee: tx.fee,
...
class Wallet {
  ...
  setHistory(){
    for (let i=0;i<this.MasterAccount.child.length;i++){
      this.walletTrx[this.MasterAccount.child[i].Name] = []
    }
    for (let i=0;i<this.MasterAccount.child.length;i++){
      let child = this.MasterAccount.child[i];
      this.walletTrx[child.Name].push(child.trxHistory)
    }
    this.Storage.setItem("Wallet Trx History", this.walletTrx)
  }
  getHistoryByAccount(accName){
    let accList = this.Storage.getItem("Wallet Trx History");
    return accList[accName]
  }
  loadWallet(password) {
    if (this.Storage != null) {
