...
class AccountWallet {
  ...
  mergeDeriveCache() {
    console.log('this.derivePointCache', this.derivePointCache)
    for (let k in this.deriveStringCache) {
      if (this.derivePointCache[k] == undefined) {
    ...
    for (let k in this.derivePointCache) {
      if (this.deriveStringCache[k] == undefined) {
  ...
  async getBalance() {
    ...
    let inputCoins = Wallet.RpcClient.parseInputCoinFromEncodedObject(allOutputCoinStrs, this.key, this.derivePointCache);
    // remove spent coin
    // let length = inputCoins.length;
    // for (let i = 0; i < length; i++) {
    //   if (this.spentCoinCache[k] == true) {
    //     delete (inputCoins[i]);
    //     delete (allOutputCoinStrs[i]);
    //     i--;
    //     length--;
    //   }
    // }
    ...
    //   spentCoinAll[e.coinDetails.snderivator] = true
    // });
    let unspentCoinList = await Wallet.RpcClient.getUnspentCoin(inputCoins, paymentAddrSerialize, allOutputCoinStrs);
    // update spent coin
    // let unspentCoin = unspentCoinList.unspentCoin
    // unspentCoin.forEach(e => {
    //   spentCoinAll[e.coinDetails.snderivator] = false
    // });
    // for (k in spentCoinAll) {
    //   if (spentCoinAll[k] == true) {
    //     this.spentCoinCache[k] = true
    //   }
    // }
  ...
  async loadWallet(password) {
    if (this.Storage != null) {
      ...
      try {
        ...
        obj.MasterAccount.mergeDeriveCache();
        ...
        for (let i = 0; i < obj.MasterAccount.child.length; i++) {
          ...
          obj.MasterAccount.child[i].mergeDeriveCache();
        }
        delete obj.Storage
