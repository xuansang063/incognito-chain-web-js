...
class AccountWallet {
  ...
  saveNormalTx(tx, amount, receivers, tokenName, tokenID, tokenSymbol, status, isIn, isPrivacy) {
    ...
    saveTrxObj.addHistoryInfo(tx.txId, tx.type, amount, tx.fee / 100, receivers, tokenName, tokenID, tokenSymbol, tx.lockTime, isIn, isPrivacy);
  ...
  async createAndSendConstant(param, fee, isPrivacy) {
    ...
    try {
      ...
      this.saveNormalTx(tx, amount.toNumber() / 100, receiverPaymentAddrStr, null, null, null, status, false, isPrivacy);
