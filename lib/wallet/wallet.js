...
class AccountWallet {
  ...
  createAndSendCustomToken = async (paymentInfos = null, tokenParams) => {
    ...
    try {
      ...
      console.log("Token ID bytes:  ", tx.txTokenData.propertyID.join(", "));
  ...
  createAndSendPrivacyCustomToken = async (paymentInfos = null, tokenParams) =>{
    ...
    try {
      ...
      await tx.init(this.key.KeySet.PrivateKey, paymentAddressStr,
          paymentInfos, inputForTx.inputCoins, inputForTx.inputCoinStrs,
          new bn(0), tokenParams, inputForPrivacyCustomTokenTx.listCustomToken, null, false);

      // console.log("Token ID:  ", convertHashToStr(tx.txTokenPrivacyData.propertyID));
      // console.log("Token ID bytes :  ", tx.txTokenPrivacyData.propertyID.join(', '));
...
class Wallet {
  ...
  createNewAccount(accountName) {
    ...
    // TODO: call api to get active shard
    // random shard id
    // loop to create child key with last byte of
