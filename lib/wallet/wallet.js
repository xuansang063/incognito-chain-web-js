...
import { fixInvalidMnemonic, KeyWallet, mapInvalidMnemonic, NewMasterKey } from "./hdwallet";
...
class Wallet {
  ...
  async init(passPhrase, storage, walletName, accountName = 'Anon') {
    ...
    this.Mnemonic = fixInvalidMnemonic(this.Mnemonic);

    // seed
    this.Seed = mnemonicGen.newSeed(mapInvalidMnemonic(this.Mnemonic), passPhrase);
  ...
  async import(mnemonicWords, passPhrase, numOfAccount, name, storage, shardID = null) {
    ...
    // mnemonic
    this.Mnemonic = fixInvalidMnemonic(mnemonicWords);
