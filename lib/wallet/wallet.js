...
class AccountWallet {
  ...
  saveCustomTokenTx(tx, receivers, status, isIn, amount) {
    ...
    let saveTrxObj = new TrxHistoryInfo();
    saveTrxObj.addHistoryInfo(tx.txId, tx.type, amount, tx.fee / 100, receivers, tx.txTokenData.propertyName, tx.txTokenData.propertyID, tx.txTokenData.propertySymbol, tx.lockTime, isIn, false);
    saveTrxObj.updateStatus(status);
  ...
  savePrivacyCustomTokenTx(tx, receivers, status, isIn, amount) {
    ...
    saveTrxObj.addHistoryInfo(tx.txId, tx.type, amount, tx.fee / 100, receivers, tx.txTokenPrivacyData.propertyName, tx.txTokenPrivacyData.propertyID, tx.txTokenPrivacyData.propertySymbol, tx.lockTime, isIn, true);
    saveTrxObj.updateStatus(status);
