...
class AccountWallet {
  ...
  // spendingCoinObj: Object{txID ="", spendingSNs = []}
  addSpendingCoins(spendingCoinObj) {
    console.log("spendingCoinObj: ", spendingCoinObj);
    console.log("this.spendingCoins: ", this.spendingCoins.length);
    // this.spendingCoins.push(...serialNumberCoins);

    if (!this.spendingCoins) {
      this.spendingCoins = [];
    }
    this.spendingCoins.push(spendingCoinObj);
    console.log("this.spendingCoins[0].txID after : ", this.spendingCoins[0].txID);
  }

  removeObjectFromSpendingCoins(txId) {
    for (let i = 0; i < this.spendingCoins.length; i++) {
      console.log("TXID is confirmed: ", txId);
      console.log("this.spendingCoins[i].txID ", this.spendingCoins[i].txID);
  ...
  saveNormalTx(tx, amount, receivers, tokenName, tokenID, tokenSymbol, status, isIn, isPrivacy) {
    let saveTrxObj = new TrxHistoryInfo();
    console.log("TX FEE when saving: ", tx.fee);
    saveTrxObj.addHistoryInfo(tx.txId, tx.type, amount, tx.fee / 100, receivers, tokenName, tokenID, tokenSymbol, tx.lockTime * 1000, isIn, isPrivacy);
  ...
  saveCustomTokenTx(tx, receivers, status, isIn, amount) {
    ...
    let saveTrxObj = new TrxHistoryInfo();
    saveTrxObj.addHistoryInfo(tx.txId, tx.type, amount, tx.fee / 100, receivers, tx.txTokenData.propertyName, tx.txTokenData.propertyID, tx.txTokenData.propertySymbol, tx.lockTime * 1000, isIn, false);
    saveTrxObj.updateStatus(status);
  ...
  savePrivacyCustomTokenTx(tx, receivers, status, isIn, amount) {
    ...
    saveTrxObj.addHistoryInfo(tx.txId, tx.type, amount, tx.fee / 100, receivers, tx.txTokenPrivacyData.propertyName, tx.txTokenPrivacyData.propertyID, tx.txTokenPrivacyData.propertySymbol, tx.lockTime * 1000, isIn, true);
    saveTrxObj.updateStatus(status);
  ...
  async mergeInputCoinJsonCached(allOutputCoinStrs, inputCoins, tokenID = 'constant') {
    ...
    for (let i = 0; i < allOutputCoinStrs.length; i++) {
      ...
      if (this.inputCoinCached[sndStr] == undefined) {
        ...
        const encodedCoin = {};
        encodedCoin.publicKey = base58.checkEncode(inputCoinTmp.coinDetails.publicKey.compress(), 0x00);
        encodedCoin.coinCommitment = base58.checkEncode(inputCoinTmp.coinDetails.coinCommitment.compress(), 0x00);
        ...
        encodedCoin.info = base58.checkEncode(inputCoinTmp.coinDetails.info, 0x00);
        encodedCoin.serialNumber = base58.checkEncode(inputCoinTmp.coinDetails.serialNumber.compress(), 0x00);
  ...
  async getBalance() {
    ...
    if (uncachedOutputCoinStrs.length > 0) {
      let uncachedInputCoins = Wallet.RpcClient.parseInputCoinFromEncodedObject(uncachedOutputCoinStrs, this.key, this.derivatorPointCached);
    ...
    let { unspentCoins, unspentCoinStrs } = await Wallet.RpcClient.getUnspentCoin(unspentInputCoinsFromCached, paymentAddrSerialize, unspentInputCoinsFromCachedStrs);
  ...
  async getPrivacyCustomTokenBalance(privacyCustomTokenID) {
    ...
    let inputCoins = cachedInputCoins;
    if (uncachedOutputCoinStrs.length > 0) {
      let uncachedInputCoins = Wallet.RpcClient.parseInputCoinFromEncodedObject(uncachedOutputCoinStrs, this.key, this.derivatorPointCached, privacyCustomTokenID);
      this.mergeDerivatorCached();
    ...
    let unspentCoinList = await Wallet.RpcClient.getUnspentCoin(unspentInputCoinsFromCached, paymentAddrSerialize, unspentInputCoinsFromCachedStrs, privacyCustomTokenID);
  ...
  async createAndSendConstant(param, fee, isPrivacy) {
    ...
    try {
      ...
      if (response.txId) {
        ...
        console.log("account.spendingCoins: ", this.spendingCoins);
      }

      console.log("Amount number when saving: ", amount.toNumber());
      this.saveNormalTx(tx, amount.toNumber(), receiverPaymentAddrStr, null, null, null, status, false, isPrivacy);
  ...
  async createAndSendStakingTx(param, fee) {
    ...
    // console.log("Amount getStakingAmount: ", amount);
    ...
    try {
      ...
      console.log("amountToSpent: ", amountToSpent);
      ...
      this.saveNormalTx(tx, amountToSpent.toNumber() / 100, receiverPaymentAddrStr, null, null, null, status, false, false);
      console.log("history after saving: ", this.trxHistory);
      ...
      if (responseSendTX.txId) {
        ...
        if (tx.txTokenData.type == constantsTx.CustomTokenInit) {
          ...
          const followingToken = {
            ...
            Image: await this.getTokenImage(txTokenData.propertyID),
      ...
      if (responseSendTX.txId) {
        ...
        if (tx.txTokenPrivacyData.type == constantsTx.CustomTokenInit) {
          ...
          this.addFollowingToken({
            ...
            Image: await this.getTokenImage(txTokenPrivacyData.propertyID),
  ...
  async getTokenImage(tokenId) {
    let res;
    try {
      res = await Wallet.RpcClient.hashToIdenticon([tokenId]);
      if (res.err != null) {
        console.log("Error when get token image: ", res.err);
        throw res.err;
      }
    } catch (e) {
      console.log("Error when get token image: ", e);
      throw e;
    }
    return res.images[0];
  }
  ...
  updateSpendingList() {
    let interval = setInterval(async () => {
      ...
      for (let i = 0; i < this.MasterAccount.child.length; i++) {
        if (this.MasterAccount.child[i].spendingCoins) {
          for (let j = 0; j < this.MasterAccount.child[i].spendingCoins.length; j++) {
            let response = await Wallet.RpcClient.getTransactionByHash(this.MasterAccount.child[i].spendingCoins[j].txID);
            if ((response.err === null && response.isInBlock) || (response.err != null && response.isInBlock === false && response.isInMempool === false)) {
              console.log("CCCC spendingCoins before update history: ", this.MasterAccount.child[i].spendingCoins);
              this.MasterAccount.child[i].removeObjectFromSpendingCoins(this.MasterAccount.child[i].spendingCoins[j].txID);
              console.log("BBBBB this.spendingCoins after update history: ", this.MasterAccount.child[i].spendingCoins);
              update = true;
            }
          }
        }
