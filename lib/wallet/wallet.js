...
class AccountWallet {
  ...
  createAndSendCustomToken = async (paymentInfos = null, tokenParams) => {
    ...
    let response = await rpcClient.listCustomTokens();
    let listCustomToken = response.listCustomToken;
      ...
      switch (tokenParams.tokenTxType) {
        case constantsTx.CustomTokenInit:{
          ...
          await tx.init(this.key.KeySet.PrivateKey, paymentAddressStr, paymentInfos, inputForTx.inputCoins, inputForTx.inputCoinStrs,  new bn(0), tokenParams, listCustomToken, null,  false);
        }
