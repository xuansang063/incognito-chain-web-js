...
class KeyWallet {
  ...
  getIntermediary(childIdx) {
    ...
    let hmac = CryptoJS.HmacSHA512(chainCode, this.byteArrayToWordArray(childIndexBytes));
    let intermediary = this.wordArrayToByteArray(hmac)
    return intermediary;
  }
  ...
  serialize(keyType) {
    ...
    if (keyType === constants.PriKeyType) {
      ...
      console.log("this.ChildNumber", this.ChildNumber)
      keyBytes.set(this.ChildNumber.buffer, offset);
      offset += 4;
  ...
  static deserialize(bytes) {
    ...
    if (keyType === constants.PriKeyType) {
      ...
      key.KeySet.PaymentAddress.Rk = bytes.slice(PublicKeyLength + 3, PublicKeyLength + 3 + ReceivingKeyLength);
    }
