...
function fixInvalidMnemonic(mnemonic) {
  const newMnemonic =
    mnemonic
      .split(' ')
      .map(word => {
        if (INVALID_WORDS.includes(word)) {
          return VALID_WORDS[INVALID_WORDS.indexOf(word)];
        }
...
function mapInvalidMnemonic(mnemonic) {
  const newMnemonic =
    mnemonic
      .split(' ')
      .map(word => {
        if (VALID_WORDS.includes(word)) {
          return INVALID_WORDS[VALID_WORDS.indexOf(word)];
        }
