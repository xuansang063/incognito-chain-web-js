...
class AccountWallet {
  ...
  savePrivacyCustomTokenTx(tx, receivers, isIn, listUTXOForPRV, listUTXOForPToken, hashOriginalTx = "") {
    let txHistory = new TxHistoryInfo();
  ...
  async createAndSendStakingTx(param, fee, candidatePaymentAddress, candidateMiningSeedKey, rewardReceiverPaymentAddress, autoReStaking = true) {
    ...
    try {
      ...
      if (response.txId) {
        ...
        response.typeTx = tx.type;
        response.feeNativeToken = tx.fee;
      ...
      this.saveNormalTx(tx, receiverPaymentAddrStr, false, false, [], []);
  ...
  async createAndSendPrivacyToken(paymentInfos = [], submitParam, feeNativeToken, feePToken, hasPrivacyForNativeToken, hasPrivacyForPToken, info = "") {
    ...
    // try {
      ...
      if (typeof randomScalars == "function") {
        sndOutputStrsForNativeToken = randomScalars(nOutputForNativeToken.toString());
      ...
      if (typeof randomScalars == "function") {
        sndOutputStrsForPToken = randomScalars(nOutputForPToken.toString());
      ...
      let paramInitTx = newParamInitPrivacyTokenTx(
        senderSkStr, paymentInfos, inputForTx.inputCoinStrs,
      ...
      if (typeof initPrivacyTokenTx == "function") {
        let paramInitTxJson = JSON.stringify(paramInitTx);
        console.log("paramInitTxJson: ", paramInitTxJson);
        resInitTx = initPrivacyTokenTx(paramInitTxJson);
        if (resInitTx == null) {
          throw new CustomError(ErrorObject.InitNormalTxErr, "Can not init transaction tranfering PRV");
        } 
      }
      ...
      if (submitParam.TokenTxType == CustomTokenInit) {
        isIn = true;
        amount = submitParam.Amount;
      } else {
        isIn = false;
        amount = submitParam.TokenReceivers.Amount;
      }

      this.savePrivacyCustomTokenTx(response, receiverPaymentAddrStr, isIn, listUTXOForPRV, listUTXOForPToken, "");
      await Wallet.updateProgressTx(100);
      return response;
    // } catch (e) {
  ...
  async createAndSendBurningRequestTx(paymentInfos = [], submitParam, feePRV, feeToken, remoteAddress) {
    ...
    try {
      ...
      try {
        inputForPrivacyCustomTokenTx = await prepareInputForTxPrivacyToken(tokenParams, this, Wallet.RpcClient, new bn(feeToken));
      } catch (e) {
        throw e;
      }
      await Wallet.updateProgressTx(50);
