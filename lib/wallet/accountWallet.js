...
class AccountWallet {
  ...
  async setAccountStorage(key, value) {
    try {
      if (this.storage) {
        L.info('SET ACCOUNT STORAGE', key, value.length || value);
  ...
  async getListUnspentCoinsStorage() {
    try {
      if (this.storage) {
        const key = this.getKeyListUnspentCoinsByTokenId();
        const listUnspentCoins = await this.getAccountStorage(key);
        L.info('GET LIST UNSPENT COINS FROM STORAGE', listUnspentCoins?.length);
  ...
  async setListUnspentCoinsStorage({ value }) {
    try {
      if (this.storage) {
        const key = this.getKeyListUnspentCoinsByTokenId();
  ...
  async getTotalCoinsStorage() {
    try {
      if (this.storage) {
        const key = this.getKeyTotalCoinsStorageByTokenId();
        const total = await this.getAccountStorage(key);
        L.info('GET TOTAL COINS STORAGE', total);
        return total || 0;
      }
    } catch (error) {
      throw error;
  ...
  async setTotalCoinsStorage({ value }) {
    try {
      if (this.storage) {
        const key = this.getKeyTotalCoinsStorageByTokenId();
        L.info('SET NEW TOTAL COINS STORAGE', value);
  ...
  async getListSerialsNumber({ listOutputsCoins }) {
    try {
      ...
      L.info('SNDS LENGTH', snds.length);
      const param = {
        privateKey,
        snds,
      };
  ...
  async checkStatusListUnspentCoinsStorage() {
    try {
      L.info('CHECK STATUS LIST UNSPENT COINS FROM STORAGE');
      const total = await this.getTotalCoinsStorage();
      L.info('OLD TOTAL COINS', total);
      if (total === 0) {
        return;
      }
      const listUnspentCoins = await this.getListUnspentCoinsStorage();
      L.info('OLD LIST UNSPENT COINS', listUnspentCoins?.length);
      ...
      L.info('SIZE LIST SN DECODE', listSerialNumberDecode?.length);
      const listSerialNumberStatus = await apiCheckKeyImages({
        keyImages: listSerialNumberDecode,
        shardId,
      });
