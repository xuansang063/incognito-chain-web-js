...
class AccountWallet {
    ...
    saveAccountCached(password){
      ...
      if (this.Storage != null) {
        console.log("this.Storage: ", this.Storage);
        return this.Storage.setItem(`${this.name} cached`, cipherText.toString());
      }
    }

    async loadAccountCached(password){
      if (this.Storage != null) {
        let cipherText = await this.Storage.getItem(`${this.name} cached`);
        ...
        try {
          ...
          tasks.push(Promise.all([
            this.loadDerivatorCached(),
            this.loadInputCoinCached()
          ]).then(() => {
