...
class AccountWallet {
    ...
    async createAndSendPrivacyCustomToken (paymentInfos = [], submitParam, fee, hasPrivacyForToken) {
      await Wallet.updateProgressTx(10);
      ...
      try {
        ...
        try {
          console.time("Time for preparing input for custom token tx");
          inputForTx = await prepareInputForTx(senderSkStr, paymentInfoForPRV, new bn(fee), this, Wallet.RpcClient);
          console.timeEnd("Time for preparing input for custom token tx");
        } catch (e) {
          throw e;
        }
