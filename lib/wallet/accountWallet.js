...
class AccountWallet {
   ...
   */
  saveNormalTxHistory(tx, receivers, isIn, isPrivacyNativeToken, listUTXOForPRV, hashOriginalTx = "", metaData = null, info = "") {
   ...
   * @param {string} hashOriginalTx
   */
  savePrivacyTokenTxHistory(tx, receivers, isIn, isPrivacyNativeToken, isPrivacyForPToken, listUTXOForPRV, listUTXOForPToken, hashOriginalTx = "", metaData = null, info = "") {
    let txHistory = new TxHistoryInfo();
  ...
  async getAllPrivacyTokenBalance() {
    try {
      ...
      for (let i = 0; i < pTokenList.length; i++) {
        ...
        let balance = await this.getBalance(tokenID);
        console.log("Balance: ", balance);

        if (balance > 0) {
          let tokenBalanceItem = {
            TokenID: tokenID,
            Balance: balance
          };
          result.push(tokenBalanceItem);
        }
      }
  ...
  async createAndSendNativeToken(paramPaymentInfos, fee, isPrivacy, info = "", isEncryptMessageOutCoin = true) {
    ...
    try {
      ...
      // saving history tx
      this.saveNormalTxHistory(response, receiverPaymentAddrStr, false, isPrivacy, listUTXOForPRV, "", null, info);
  ...
  async createAndSendStakingTx(param, feeNativeToken, candidatePaymentAddress, candidateMiningSeedKey, rewardReceiverPaymentAddress, autoReStaking = true) {
    ...
    try {
      ...
      // saving history tx
      this.saveNormalTxHistory(response, BurnAddress, false, false, listUTXOForPRV, "", meta, info);
  ...
  async createAndSendPrivacyToken(paramPaymentInfosForNativeToken = [], submitParam, feeNativeToken, feePToken,
    ...
    this.savePrivacyTokenTxHistory(response, [submitParam.TokenReceivers.PaymentAddress], isIn, hasPrivacyForNativeToken, hasPrivacyForPToken, listUTXOForPRV, listUTXOForPToken, "", null, info);
    await Wallet.updateProgressTx(100);
    return response;
  };
  ...
  async replaceTx(txId, newFee, newFeePToken, newInfo = null) {
    ...
    if (isNormalTx) {
      try {
        response = await this.replaceTxNormal(txHistory, newFee, newInfo);
      } catch (e) {
        throw e;
      }
    } else {
   ...
   * @param {number} newFee 
   */
  async replaceTxNormal(txHistory, newFee, newInfo = null) {
    ...
    try {
      ...
      let paramInitTx = newParamInitTx(
        senderSkStr, paramPaymentInfos, inputForTx.inputCoinStrs,
        newFee, isPrivacy, null, null, info,
        inputForTx.commitmentIndices, inputForTx.myCommitmentIndices, inputForTx.commitmentStrs, sndOutputs);
      console.log("createAndSendNativeToken paramInitTx: ", paramInitTx);
  ...
  async replaceTxPToken(txHistory, newFee, newFeePToken, newInfo = null) {
    // check new fee
  ...
  async createAndSendBurningRequestTx(paramPaymentInfosForNativeToken = [], submitParam, feeNativeToken, feePToken, remoteAddress,
    ...
    let paramInitTx = newParamInitPrivacyTokenTx(
      senderSkStr, paramPaymentInfosForNativeToken, inputForTx.inputCoinStrs,
      feeNativeToken, true, false, tokenParamJson, burningReqMetadata, "",
      inputForTx.commitmentIndices, inputForTx.myCommitmentIndices, inputForTx.commitmentStrs, sndOutputsForNativeToken,
      inputForPrivacyTokenTx.commitmentIndices, inputForPrivacyTokenTx.myCommitmentIndices, inputForPrivacyTokenTx.commitmentStrs, sndOutputsForPToken
    );
    ...
    let isIn = false;
    this.savePrivacyTokenTxHistory(response, [BurnAddress], isIn, false, false, listUTXOForPRV, listUTXOForPToken, "", burningReqMetadata);
    await Wallet.updateProgressTx(100);
    return response;
  };
