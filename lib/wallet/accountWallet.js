...
class AccountWallet {
  ...
  async setListSpentCoinsStorage({ key, value, tokenId }) {
    try {
      if (this.storage) {
        const keyStorage = this.getKeyListSpentCoinsByTokenId(tokenId);
        let oldListUnspentCoins = await this.getListSpentCoinsStorage(tokenId);
        if (!oldListUnspentCoins[key]) {
          oldListUnspentCoins = { ...oldListUnspentCoins, [key]: value };
        }
        await this.setAccountStorage(keyStorage, oldListUnspentCoins);
      }
    } catch (error) {
      throw error;
    }
  ...
  async storeListSpentCoins({ tokenId, spentCoins }) {
    ...
    const task = spentCoins.map((item) => {
  ...
  async checkStatusListUnspentCoinsStorage(tokenId) {
    try {
      ...
      if (listSerialsNumberEncodeBase64.length !== 0) {
        ...
        await this.storeListSpentCoins({ spentCoins, tokenId });
      }
