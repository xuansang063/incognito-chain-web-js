...
import {
  ...
  CustomTokenTransfer,
  MaxInputNumberForDefragment, MAX_DEFRAGMENT_TXS
} from '../tx/constants';
...
import {
  ...
  prepareInputForReplaceTxPrivacyToken, prepareInputForDefragments
} from "../tx/utils";
...
class AccountWallet {
  ...
  async defragmentNativeCoin(fee, isPrivacy = true, noOfInputPerTx = MaxInputNumberForDefragment, maxTxs = MAX_DEFRAGMENT_TXS) {
    await Wallet.updateProgressTx(10);
    ...
    try {
      ...
      for (const inputInfo of inputForTxs) {
        ...
        const paymentInfos = [{
          ...
          amount: inputInfo.totalValueInput.sub(feeBN).toNumber(),
        }];
      ...
      for (const rawTx of rawTxs) {
        ...
        if (response.txId) {
          ...
          status = SuccessTx;
          response.typeTx = TxNormalType;
          response.feeNativeToken = fee;
          response.lockTime = lockTime;
          response.amountNativeToken = totalAmount.toNumber();
          response.txStatus = status;
  ...
  async createAndSendBurningRequestTx(paramPaymentInfosForNativeToken = [], submitParam,
    ...
    submitParam.TokenReceivers.forEach(receiver => paymentInfoForPToken.push({
