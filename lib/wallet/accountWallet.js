...
class AccountWallet {
    ...
    saveNormalTx(tx, amount, receivers, tokenName, tokenID, tokenSymbol, status, isIn, isPrivacy) {
      let saveTrxObj = new TxHistoryInfo();
    ...
    async mergeDerivatorCached() {
      ...
      for (let k in this.derivatorPointCached) {
        if (k != undefined && this.derivatorJsonCached[k] == undefined) {
          this.derivatorJsonCached[k] = checkEncode(this.derivatorPointCached[k].compress(), 0x00);
        }
      }
    ...
    async mergeInputCoinJsonCached(allOutputCoinStrs, inputCoins, tokenID = 'constant') {
      ...
      for (let i = 0; i < allOutputCoinStrs.length; i++) {
        ...
        if (this.inputCoinCached[sndStr] == undefined) {
          ...
          const encodedCoin = {};
          encodedCoin.publicKey = checkEncode(inputCoinTmp.coinDetails.publicKey.compress(), 0x00);
          encodedCoin.coinCommitment = checkEncode(inputCoinTmp.coinDetails.coinCommitment.compress(), 0x00);
          ...
          encodedCoin.info = checkEncode(inputCoinTmp.coinDetails.info, 0x00);
          encodedCoin.serialNumber = checkEncode(inputCoinTmp.coinDetails.serialNumber.compress(), 0x00);
    ...
    async getCustomTokenBalance(customTokenIDStr) {
      ...
      let res0 = await Wallet.RpcClient.getUnspentCustomToken(this.key.base58CheckSerialize(PaymentAddressType),
        customTokenIDStr);
