...
import { 
 ...
 import { ShardStakingType } from './constants';
...
class AccountWallet {
    ...
    saveNormalTx(tx, amount, receivers, status, isIn, isPrivacy, listUTXOForPRV, hashOriginalTx = ""){
      ...
      txHistory.setHistoryInfo(tx.txId, tx.type, amount, toConstant(tx.fee), receivers, tx.lockTime * 1000, isIn, isPrivacy, '', '', '', listUTXOForPRV, [], hashOriginalTx);
    ...
    async createAndSendConstant(param, fee, isPrivacy, info){
      ...
      try {
        ...
        this.saveNormalTx(tx, amount.toNumber(), receiverPaymentAddrStr, status, false, isPrivacy, listUTXOForPRV, "");
