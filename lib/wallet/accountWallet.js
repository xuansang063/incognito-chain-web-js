...
import { getListOutputCoins } from '../http/fullnodeServices';
...
class AccountWallet {
  ...
  setHighestHeight({ tokenId, value }) {
    this.highestHeight = { ...this.highestHeight, [tokenId]: value };
  }

  getHighestHeight({ tokenId }) {
    return this.highestHeight[tokenId]?.value || 0;
  }
  ...
  async getAllOutputCoinsV2({ tokenId, highestHeight }) {
    try {
      const fromHeight = this.getHighestHeight({ tokenId });
      const readOnlyKeySerialize = this.key.base58CheckSerialize(
        ReadonlyKeyType
      );
      const response = await getListOutputCoins({
        viewKey: readOnlyKeySerialize,
        fromHeight,
      });
      const currentHeight = highestHeight || this.getHighestHeight({ tokenId });
      this.setHighestHeight({ tokenId, value: currentHeight });
  ...
  async getBalanceV2(tokenId) {
    ...
    console.log('START GET BALANCE TOKEN', tokenID);
    try {
      const outputs = await this.getAllOutputCoinsV2({ tokenId: tokenID });
      console.log('outputs', outputs);
      console.log(
        'highest height',
        this.getHighestHeight({ tokenId: tokenID })
      );
    } catch (error) {}
    return accountBalance;
  }
