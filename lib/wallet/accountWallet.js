...
class AccountWallet {
  ...
  async createAndSendNativeToken(paramPaymentInfos, fee, isPrivacy, info = "") {
    ...
    try {
      ...
      let inputForTx;
      try {
        inputForTx = await prepareInputForTx(senderSkStr, paymentAddressStr, viewingKeyStr, totalAmountTransfer, feeBN, isPrivacy, null,  this, Wallet.RpcClient);
      ...
      if (typeof randomScalars == "function"){
        ...
        let tmp = base64Decode(sndOutputStrs);
        console.log("tmp AAA:", tmp);

        for (let i =0; i<nOutput; i++){
          let tmp2 = tmp.slice(i*ED25519_KEY_SIZE, (i+1)*ED25519_KEY_SIZE);
          console.log("tmp2 AAA:", tmp2);
          sndOutputs[i] = checkEncode(tmp2, ENCODE_VERSION);
        }
      }
    ...
    let paramInitTx = newParamInitTx(
      ...
      let serializedTxJson;
      if (typeof initTx == "function"){
        let paramJson = JSON.stringify(paramInitTx);
        console.log("paramJson: ", paramJson);
        let res  = initTx(paramJson);
        if (res == null){
          throw new CustomError(ErrorObject.InitNormalTxErr, "Can not init transaction tranfering PRV");
        } else{
          serializedTxJson = res;
        }
      }

      console.log("serializedTxJson: ", serializedTxJson);

      //base64 decode txjson
      let txJson = base64Decode(serializedTxJson);
      


      let b58CheckEncodeTx = checkEncode(txJson.slice(0, txJson.length - 8),  ENCODE_VERSION);

      // get lock time tx
      let lockTimeBytes = txJson.slice(txJson.length - 8);
      let lockTime = new bn(lockTimeBytes).toNumber();

      // console.log("Tx json: ", txJson);

      // let tx = JSON.parse(txJson);
      // if (err != null){
      //   console.log("Error when unmarshal tx: ", err);
      // }
      // console.log("tx: ", tx);
