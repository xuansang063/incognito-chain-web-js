...
import _, { chunk, flatten, isEmpty, uniqBy } from 'lodash';
import {
  CustomTokenInit,
  TxNormalType,
...
class AccountWallet {
  ...
  async getListUnspentCoinsStorage(tokenId) {
    try {
      if (this.storage) {
        ...
        console.debug('LIST UNSPENT COINS', listUnspentCoins?.length);
  ...
  async setListUnspentCoinsStorage({ value, tokenId }) {
    try {
      if (this.storage) {
        ...
        console.debug(
          'SET LIST UNSPENT COINS NIST OLD LUT ',
          oldListUnspentCoins
        );
        ...
        console.debug('SET LIST UNSPENT COINS NEW ', listUnspentCoins);
  ...
  async getUnspentToken(tokenID) {
    try {
      console.debug('GET LIST UNSPENT TOKENS FROM STORAGE');
      const unspentCoins = await this.getListUnspentCoinsStorage(
        tokenID || PRVIDSTR
      );
      if (!unspentCoins) {
        throw new Error('Can not get list unspent coins');
      }
      let publicKey = this.key.getPublicKeyCheckEncode();
      return unspentCoins.map((item) => {
        let sn64Decode = base64Decode(item?.SerialNumber);
        let SerialNumber = checkEncode(sn64Decode, ENCODE_VERSION);
        return {
          ...item,
  ...
  getListSerialsNumberDecode({ listSerialsNumberEncode, size }) {
    console.debug('listSerialsNumberEncode', listSerialsNumberEncode);
  ...
  async updateListUnspentCoinsStorage({ listUnspentCoins, tokenId }) {
    try {
      const key = this.getKeyListUnspentCoinsByTokenId(tokenId);
      console.debug('listUnspentCoins', listUnspentCoins);
      await this.setAccountStorage(key, listUnspentCoins);
    } catch (error) {
      throw error;
    }
  ...
  async checkStatusListUnspentCoinsStorage(tokenId) {
    try {
      ...
      console.debug('CHECK STATUS SN FROM LOCAL STORAGE', total);
      if (total === 0) {
        return;
      }
      ...
      console.debug('listSerialNumberDecode', listSerialNumberDecode);
      if (listSerialNumberDecode.length !== 0) {
        const listSerialNumberStatus = await apiCheckKeyImages({
          keyImages: listSerialNumberDecode,
          shardId,
        });
        console.debug('listSerialNumberStatus', listSerialNumberStatus);
        const listUnspentCoinsFiltered = listOutputsCoins?.filter(
          (coin, index) => !listSerialNumberStatus[index]
        );
        console.debug('listUnspentCoinsFiltered', listUnspentCoinsFiltered);
  ...
  async getListUnspentCoins(tokenId) {
    try {
      ...
      if (calcTotal > 0) {
        ...
        console.debug(
          'GET LIST OUTPUTS COINS SUCCESS',
        ...
        console.debug(
          'SET LIST UNSPENT COINS TO STORAGE SUCCESS',
        ...
        console.debug('SET TOTAL COINS TO STORAGE', total);
