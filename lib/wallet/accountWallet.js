...
class AccountWallet {
  ...
  async defragmentNativeCoin(fee, isPrivacy = true, noOfInputPerTx = MaxInputNumberForDefragment) {
    ...
    try {
      const inputForTxs = await prepareInputForDefragments(PRVIDSTR, this, Wallet.RpcClient, noOfInputPerTx);
      ...
      for (const inputInfo of inputForTxs) {
        ...
        const paymentInfos = [{
          ...
          amount: inputInfo.totalValueInput.toNumber() - fee,
        }];
        const nOutput = 1;
        ...
        await Wallet.updateProgressTx(startProcess + Math.floor((index / totalTxs * initTxProcess) / 5) * 5);
      }
