...
class AccountWallet {
  ...
  getShardId() {
    ...
    console.debug('SHARD ID', shardId);
    return shardId;
  }
  ...
  async setAccountStorage(key, value) {
    try {
      if (this.storage) {
        console.debug('SET ACCOUNT STORAGE', key, value.length || value);
  ...
  async getListUnspentCoinsStorage() {
    try {
      if (this.storage) {
        ...
        console.debug(
          'GET LIST UNSPENT COINS FROM STORAGE',
  ...
  async getTotalCoinsStorage() {
    try {
      if (this.storage) {
        ...
        console.debug('GET TOTAL COINS STORAGE', total);
        return total || 0;
      }
    } catch (error) {
      throw error;
  ...
  async setTotalCoinsStorage({ value }) {
    try {
      if (this.storage) {
        const key = this.getKeyTotalCoinsStorageByTokenId();
        console.debug('SET NEW TOTAL COINS STORAGE', value);
  ...
  async getListSerialsNumber({ listOutputsCoins }) {
    try {
      ...
      console.debug('SNDS LENGTH', snds.length);
      const param = {
        privateKey,
        snds,
      };
      const result = await wasmFuncs.deriveSerialNumber(JSON.stringify(param));
      console.debug('TIME GET LIST SN DERIVATOR', performance.now() - t0);
      console.debug('GET LIST SN DERIVATOR LENGTH', result.length);
      return result || [];
    } catch (error) {
      throw error;
    }
  ...
  async checkKeyImages({ listOutputsCoins, shardId }) {
    console.debug('GET LIST SN DERIVATOR BY RPC');
  ...
  async checkStatusListUnspentCoinsStorage() {
    try {
      console.debug('CHECK STATUS LIST UNSPENT COINS FROM STORAGE');
      const total = await this.getTotalCoinsStorage();
      console.debug('OLD TOTAL COINS', total);
      if (total === 0) {
        return;
      }
      const listUnspentCoins = await this.getListUnspentCoinsStorage();
      console.debug('OLD LIST UNSPENT COINS', listUnspentCoins?.length);
      ...
      console.debug('SIZE LIST SN DECODE', listSerialNumberDecode?.length);
      const listSerialNumberStatus = await apiCheckKeyImages({
        keyImages: listSerialNumberDecode,
        shardId,
      });
  ...
  async getListUnspentCoins() {
    try {
      ...
      console.debug('KEY INFO', keyInfo);
      let listOutputsCoins = [];
      const { total } = keyInfo;
      console.debug('NEW TOTAL COINS', total);
      const oldTotal = await this.getTotalCoinsStorage();
      console.debug('OLD TOTAL COINS', oldTotal);
