...
class AccountWallet {
   ...
   * @param {array of string} proofStrs
   * @param {string} incTokenID
   * @param {number} feeNativeToken
   */
  async createAndSendIssuingETHRequestTx(blockHash, txIndex, proofStrs, incTokenID, feeNativeToken) {
    await Wallet.updateProgressTx(10);
    // check network fee
    if (feeNativeToken < 0) {
      feeNativeToken = 0
    ...
    try {
      ...
      let resInitTx;
      let paramInitTxJson = JSON.stringify(paramInitTx);
      resInitTx = await wasmFuncs.initIssuingETHReqTx(paramInitTxJson);
      if (resInitTx === null || resInitTx === "") {
        throw new CustomError(ErrorObject.InitNormalTxErr, "Can not init transaction tranfering PRV");
      }
      ...
      if (response.txId) {
        ...
        response.feeNativeToken = feeBN.toString();
        response.lockTime = lockTime;
        response.amountNativeToken = amountBN.toNumber();
        response.txStatus = status;
      ...
      // saving history tx
      this.saveNormalTxHistory(response, [], false, false, listUTXOForPRV, "", meta, "", messageForNativeToken);
