...
class AccountWallet {
  ...
  storeListSerialsNumberDecodeBase58({ tokenId, spentCoins }) {
    spentCoins.forEach((item, i) => {
      const sndStr = `${tokenId}_${item.SNDerivator}`;

      if (
        this.derivatorToSerialNumberCache &&
        this.derivatorToSerialNumberCache[sndStr] &&
        this.derivatorToSerialNumberCache[sndStr].length > 0
      ) {
        return;
      }

      const bytes = base64Decode(item.SerialNumber);
      this.derivatorToSerialNumberCache[sndStr] = checkEncode(
        bytes,
        ENCODE_VERSION
      );
    });
  ...
  async getKeyInfo(tokenId) {
    try {
      ...
      if (keyInfo && keyInfo.v1startindex && keyInfo.v1startindex[tokenId]) {
        total = keyInfo.v1startindex[tokenId];
      }
  ...
  async checkKeyImages({ listOutputsCoins, shardId, tokenId }) {
    ...
    if (listSerialNumberDecode.length !== 0) {
      ...
      const spentCoins = listOutputsCoinsWithSNDecode?.filter(
        (coin, index) => listSerialNumberStatus[index]
      );
      this.storeListSerialsNumberDecodeBase58({
        tokenId,
        spentCoins,
      });
    }
