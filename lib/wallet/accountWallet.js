...
class AccountWallet {
  ...
  async deriveSerialNumbers(spendingKeyStr, inCoinStrs, tokenID = null) {
    ...
    const keys = Object.keys(this.derivatorToSerialNumberCache) || [];

    let param = {
      "privateKey": spendingKeyStr,
      "snds": snds.filter(item => !keys.includes(`${tokenID}_${item}`)),
    };
    ...
    if (typeof deriveSerialNumber === "function") {
      ...
      if (param.snds.length) {
        res = await deriveSerialNumber(paramJson);

        if (res === null || res === "") {
          throw new Error("Can not derive serial number");
        }

        tmpBytes = base64Decode(res);
      }


      for (let i = 0; i < snds.length; i++) {
        const sndStr = `${tokenID}_${snds[i]}`;
        if (!this.derivatorToSerialNumberCache[sndStr]) {
          ...
          inCoinStrs[i].SerialNumber = this.derivatorToSerialNumberCache[sndStr];
        }
      }
    }
