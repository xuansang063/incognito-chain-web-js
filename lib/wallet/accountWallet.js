...
class AccountWallet {
   ...
   */
  saveNormalTxHistory(tx, receivers, isIn, isPrivacyNativeToken, listUTXOForPRV, hashOriginalTx = "", metaData = null) {
   ...
   * @param {string} hashOriginalTx
   */
  savePrivacyTokenTxHistory(tx, receivers, isIn, isPrivacyNativeToken, isPrivacyForPToken, listUTXOForPRV, listUTXOForPToken, hashOriginalTx = "", metaData = null) {
    let txHistory = new TxHistoryInfo();
  ...
  async createAndSendStakingTx(param, feeNativeToken, candidatePaymentAddress, candidateMiningSeedKey, rewardReceiverPaymentAddress, autoReStaking = true) {
    ...
    try {
      ...
      // saving history tx
      this.saveNormalTxHistory(response, BurnAddress, false, false, listUTXOForPRV, "", meta);
  ...
  async createAndSendTxWithNativeTokenContribution(fee, pdeContributionPairID, contributedAmount, info = "") {
    ...
    try {
      ...
      // saving history tx
      this.saveNormalTxHistory(response, [BurnAddress], false, isPrivacy, listUTXOForPRV, "", metadata);
      console.log("createAndSendWithdrawRewardTx History account after saving: ", this.txHistory.NormalTx);
  ...
  async getReceivedTransaction(rpcClient) {
    // call api to get info from node
    let txs = rpcClient.getTransactionByReceiver(this.key.base58CheckSerialize(PaymentAddressType), this.key.base58CheckSerialize(ReadonlyKeyType));
    for (let i = 0; i < txs.length; i++) {
      let tx = txs[i];
      let historyObj = {
        txID: tx.Hash,
        amountNativeToken: tx.ReceivedAmounts[PRVIDSTR],   // in nano PRV
