...
class AccountWallet {
  constructor() {
    ...
    this.followingTokens = [
      { ID: 'ffd8d42dc40a8d166ea4848baf8b5f6e912ad79875f4373070b59392b1756c8f', Image: '', Name: 'pETH', Symbol: 'pETH', Amount: 0, IsPrivacy: true, isInit: false, metaData: { decimals: 18, type: 0 } },
      { ID: 'b832e5d3b1f01a4f0623f7fe91d6673461e1f5d37d91fe78c5c2e6183ff39696', Image: '', Name: 'pBTC', Symbol: 'pBTC', Amount: 0, IsPrivacy: true, isInit: false, metaData: { decimals: 9, type: 0 } }
    ];
  ...
  async createAndSendBurningRequestTx(paymentInfos = [], submitParam, feePRV, feeToken, remoteAddress) {
    ...
    try {
      ...
      let burningReq = {
        ...
        TokenID: newHashFromStr(tokenParams.propertyID),
  ...
  async createAndSendWithdrawRewardTx(tokenID = "") {
    // let privateKeyStr = this.key.base58CheckSerialize(PriKeyType);
    let paymentAddrStr = this.key.base58CheckSerialize(PaymentAddressType);
    ...
    try {
      ...
      let status = FailedTx;
      if (response.txId) {
        console.log("HHHH Create and send tx successfully!!!!");
        tx.txId = response.txId
        status = SuccessTx;
