...
class AccountWallet {
  ...
  async replaceTx(txId, newFee, newFeePToken,
    newInfo = null, newMessageForNativeToken = null, newMessageForPToken = null,
    ...
    if (txType == TxNormalType) {
      ...
      try {
        response = await this.replaceTxPToken(txHistory, newFee, newFeePToken, newInfo, 
          newMessageForNativeToken, newMessageForPToken, isEncryptMessageOutCoinNativeToken, isEncryptMessageOutCoinPToken);
      } catch (e) {
        throw e;
      }
    } else{
  ...
  async replaceTxPToken(txHistory, newFee, newFeePToken,
    ...
    let tokenParamJson = {
      ...
      tokenTxType: txHistory.TokenTxType || CustomTokenTransfer,
      fee: feePToken,
      paymentInfoForPToken: [{
