...
class AccountWallet {
  ...
  analyzeOutputCoinFromCached(allOutputCoinStrs, tokenID = 'constant') {
    ...
    for (let i = 0; i < allOutputCoinStrs.length; i++) {
      ...
      if (this.inputCoinJsonCached[sndStr] !== undefined) {
  ...
  async mergeSpentCoinCached(unspentCoinStrs, unspentCoinStrsFromCache, tokenID = 'constant') {
  ...
  analyzeSpentCoinFromCached(inCoinStrs, tokenID = 'constant') {
    this.spentCoinCached = this.spentCoinCached == undefined ? {} : this.spentCoinCached;
  ...
  deriveSerialNumbers = async (spendingKeyStr, inCoinStrs, tokenID) => {
   ...
   */
  saveNormalTx(tx, receivers, isIn, isPrivacy, listUTXOForPRV, hashOriginalTx = "") {
    let txHistory = new TxHistoryInfo();

    let historyObj = {
      amount: tx.amount,   // in nano PRV
      fee: tx.fee,      // in nano PRV
  ...
  async getUnspentToken(tokenID = null, rpcClient){
    ...
    const { uncachedOutputCoinStrs, cachedOutputCoinStrs } = this.analyzeOutputCoinFromCached(allOutputCoinStrs);
    ...
    if (uncachedOutputCoinStrs.length > 0) {
      uncachedOutputCoinStrs = this.deriveSerialNumbers(spendingKeyStr, uncachedOutputCoinStrs, tokenID);
      console.log("AA cachedOutputCoinStrs after: ", cachedOutputCoinStrs);
  ...
  async createAndSendNativeToken(paramPaymentInfos, fee, isPrivacy, info = "") {
    ...
    console.log("paymentAddressStr: ", paymentAddressStr);
    console.log("viewingKeyStr: ", viewingKeyStr);

    // let senderSkStr = "112t8rnX7qWSJFCnGBq4YPHYN2D29NmGowC5RSbuDUC8Kg8ywg6GsPda5xRJMAmzmVKwLevdJNi5XfrqHRWDzSGEg37kbsrcWrAEQatR1UQQ";
    // let paymentAddressStr = "12S4NL3DZ1KoprFRy1k5DdYSXUq81NtxFKdvUTP3PLqQypWzceL5fBBwXooAsX5s23j7cpb1Za37ddmfSaMpEJDPsnJGZuyWTXJSZZ5";
    // let viewingKeyStr = "13hfxVR8JoQCqZuub5vS4KXNTCsKDPtUMXqjjS3dUHxeuCTPyJBhiYSZaoqD52vYrpmNXesveU6csJsnfEbpC99HM84FkGmDN5vjYs5";
    ...
    try {
      ...
      let inputForTx;
      try {
        inputForTx = await prepareInputForTx(paymentAddressStr, totalAmountTransfer, feeBN, isPrivacy, null, this, Wallet.RpcClient);
      ...
      if (typeof randomScalars == "function") {
        sndOutputStrs = randomScalars(nOutput.toString());
      ...
      if (typeof initTx == "function") {
        ...
        resInitTx = initTx(paramInitTxJson);
        if (resInitTx == null) {
      ...
      if (response.txId) {
        ...
        response.type = TxNormalType;
        response.fee = feeBN.toNumber();
        response.lockTime = lockTime;
        response.amount = totalAmountTransfer.toNumber();
  ...
  // fee in Number
  async createAndSendStakingTx(param, fee, candidatePaymentAddress, candidateMiningSeedKey, rewardReceiverPaymentAddress, autoReStaking = true) {
    await Wallet.updateProgressTx(10);
