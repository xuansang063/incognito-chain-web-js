...
import _, { chunk, flatten, uniqBy } from 'lodash';
...
class AccountWallet {
  ...
  async getAccountStorage(key) {
    ...
    try {
      ...
      L.info('ERROR GET ACCOUNT STORAGE', error?.message);
    }
    return result;
  }
  ...
  async setAccountStorage(key, value) {
    try {
      ...
      L.error('ERROR SET ACCOUNT STORAGE', error?.message);
    }
  }
  ...
  async initAccountStorageByTokenId(tokenId) {
    try {
      if (this.storage) {
        const tokenId = this.tokenId;
        const key = this.getKeyStorageByTokenId(tokenId);
        const data = await this.getAccountStorage(key);
        if (!data) {
          await this.setAccountStorage(key, {
            total: 0,
            listUnspentCoins: [],
          });
        }
  ...
  async getListUnspentCoinsStorage() {
    try {
      if (this.storage) {
        const tokenId = this.tokenId;
        const key = this.getKeyStorageByTokenId(tokenId);
        const data = await this.getAccountStorage(key);
        const { listUnspentCoins } = data;
  ...
  async setListUnspentCoinsStorage({ tokenId, value }) {
    try {
      if (this.storage) {
        const key = this.getKeyStorageByTokenId(tokenId);
        const oldData = await this.getAccountStorage(key);
        L.info('SET LIST UNSPENT COINS STORAGE');
        const { listUnspentCoins: oldListUnspentCoins } = oldData;
        L.info('OLD LIST', oldListUnspentCoins.length);
        const listUnspentCoins =
          !oldListUnspentCoins || oldListUnspentCoins.length === 0
            ? value
            : uniqBy([...oldListUnspentCoins, ...value], 'SNDerivator');
        L.info('NEW LIST', listUnspentCoins.length);
        await this.setAccountStorage(key, {
          ...oldData,
          listUnspentCoins: [...listUnspentCoins],
        });
      }
    } catch (error) {
      throw error;
  ...
  async setTotalCoinsStorage({ value }) {
    try {
      if (this.storage) {
        const tokenId = this.tokenId;
        const key = this.getKeyStorageByTokenId(tokenId);
      ...
      if (this.txHistory.NormalTx) {
        for (let j = 0; j < this.txHistory.NormalTx.length; j++) {
          ...
          if (this.txHistory.NormalTx[j].status === SuccessTx) {
            const txID = this.txHistory.NormalTx[j].txID;
  ...
  async getListSerialsNumber({ listOutputsCoins }) {
    try {
      ...
      const privateKey = this.key.base58CheckSerialize(PriKeyType);
      const chunkListOutputCoins = chunk(listOutputsCoins, LIMIT_COINS);
      L.info('listOutputsCoins length', listOutputsCoins.length);
      L.info('chunkListOutputCoins length', chunkListOutputCoins.length);
      const task = [];
      chunkListOutputCoins.forEach((list, index) => {
        const snds = list.map((item) => item?.SNDerivator);
        L.info('SNDS LENGTH', snds.length, index);
        const param = {
          privateKey,
          snds,
        };
        ...
        L.info('PUSH LIST');
        task.push(wasmFuncs.deriveSerialNumber(paramJson));
        L.info('END PUSH LIST', performance.now() - t1);
      });
      const res = await Promise.all(task);
      L.info('TIME GET LIST SN DERIVATOR', performance.now() - t0);
      const result = flatten(res);
      L.info('GET LIST SN DERIVATOR LENGTH', result.length);
      return result || [];
    } catch (error) {
      throw error;
    }
  ...
  async getKeyInfo() {
    try {
      ...
      const result = await apiGetKeyInfo({ viewKey: readOnlyKeySerialize });
      const total = result?.v1startindex[tokenId]?.Total || 0;
      return {
        total,
      };
    } catch (error) {
  ...
  async checkKeyImages({ listOutputsCoins, shardId }) {
    L.info('GET LIST SN DERIVATOR BY RPC');
    ...
    L.info('CHECK KEY IMAGES');
    const t0 = performance.now();
    const listSerialNumberStatus = await apiCheckKeyImages({
      keyImages: listSerialNumberDecode,
      shardId,
    });
    L.info('CHECKED KEY IMAGES', performance.now() - t0);
    L.info(
      'LIST SN STATUS LENGTH BEFORE FILTER',
      listSerialNumberStatus.length
    );
    const listUnspentCoinsFiltered = listOutputsCoinsWithSNDecode?.filter(
      (coin, index) => !listSerialNumberStatus[index]
    );
    L.info('\nLIST UNSPENT COINS FILTERED', listUnspentCoinsFiltered.length);
  ...
  async updateListUnspentCoinsStorage({ listUnspentCoins }) {
    try {
      const tokenId = this.tokenId;
      const key = this.getKeyStorageByTokenId(tokenId);
      const oldData = await this.getAccountStorage(key);
  ...
  async checkStatusListUnspentCoinsStorage() {
    try {
      const tokenId = this.tokenId;
      const total = await this.getTotalCoinsStorage();
      if (total === 0) {
        return;
      }
      const listUnspentCoins = await this.getListUnspentCoinsStorage({
        tokenId,
      });
  ...
  async getListUnspentCoins() {
    try {
      const tokenId = this.tokenId;
      L.info(`\nINIT ACCOUNT STORAGE BY TOKEN ID ${tokenId}`);
      await this.initAccountStorageByTokenId();
      L.info('\nGET KEY INFO');
      const keyInfo = await this.getKeyInfo();
      L.info('KEY INFO', keyInfo);
      let listOutputsCoins = [];
      const { total } = keyInfo;
      L.info('NEW TOTAL COINS', total);
      const oldTotal = await this.getTotalCoinsStorage();
      L.info('OLD TOTAL COINS', oldTotal);
      await this.checkStatusListUnspentCoinsStorage();
      if (total !== oldTotal && total !== 0) {
        await this.setTotalCoinsStorage({ value: total });
        const t0 = performance.now();
        listOutputsCoins = await this.getListOutputsCoins({ total });
        L.info(
          `TIME START GET LIST OUTPUTS COINS WITH LIMITS = ${LIMIT_COINS}`,
          performance.now() - t0
        );
        const shardId = this.getShardId();
        const listUnspentCoinsFiltered = await this.checkKeyImages({
      ...
      L.info('listUnspentCoinsMerged', listUnspentCoinsMerged.length);
  ...
  async getBalanceV2(tokenID) {
    let accountBalance = '0';
    try {
      const tokenId = tokenID || PRVIDSTR;
      L.info('GET BALANCE V2', tokenId);
      this.setTokenId(tokenId);
      const t0 = performance.now();
      const listUnspentCoins = await this.getListUnspentCoins();
      L.info(
        `TIME INVOKE GET BALANCE WITH LIMIT = ${LIMIT_COINS}`,
        performance.now() - t0
      );
      accountBalance =
