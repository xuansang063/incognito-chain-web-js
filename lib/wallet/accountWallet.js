...
class AccountWallet {
    ...
    mergeInputCoinJsonCached = async (allOutputCoinStrs, inputCoins, tokenID = 'constant') => {
    ...
    mergeSpentCoinCached = async (unspentCoinStrs, inputCoins, tokenID = 'constant') => {
    ...
    getBalance = async () => {
      ...
      let { unspentCoins, unspentCoinStrs } = await getUnspentCoin(unspentInputCoinsFromCached, paymentAddrSerialize, unspentInputCoinsFromCachedStrs, Wallet.RpcClient);
    ...
    createAndSendConstant = async (param, fee, isPrivacy) => {
    ...
    // fee in Number
    createAndSendStakingTx = async (param, fee) => {
      await Wallet.updateProgressTx(10)
      let amount;
      try {
    ...
    createAndSendCustomToken = async (paymentInfos = null, tokenParams, receiverPaymentAddrStr, fee) => {
      await Wallet.updateProgressTx(10);
      let senderSkStr = this.key.base58CheckSerialize(PriKeyType);
      let paymentAddressStr = this.key.base58CheckSerialize(PaymentAddressType);
    ...
    createAndSendPrivacyCustomToken = async (paymentInfos = null, submitParam, fee) => {
      await Wallet.updateProgressTx(10);
    ...
    defragment = async (amount, fee, isPrivacy) => {
      await Wallet.updateProgressTx(10);
