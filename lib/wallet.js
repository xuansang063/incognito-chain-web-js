...
let rpc = import("./rpcclient/rpcclient");
import {
    setRandBytesFunc,
...
class Wallet {
    ...
    static NewTransactor() {
    	// let m = await txModule;
    ...
    async init(passPhrase, storage, walletName, accountName = 'Anon') {
        ...
        this.MasterAccount = Wallet.NewTransactor();
        ...
        const account = Wallet.NewTransactor();
    ...
    async import (mnemonicWords, passPhrase, numOfAccount, name, storage, shardID = null) {
        ...
        if (shardID != null) {
            ...
            for (let i = 0; i < numOfAccount; i++) {
                ...
                let account = Wallet.NewTransactor();
    ...
    async createAccount(accountName, shardID) {
        ...
        while (true) {
            ...
            const accountWallet = Wallet.NewTransactor();
    ...
    async importAccount(privakeyStr, accountName, passPhrase) {
        ...
        // console.log("WALLET LOADED", JSON.stringify(keyWallet.KeySet));
    ...
    async loadWallet(password) {
        if (this.Storage != null) {
            ...
            try {
                ...
                Object.setPrototypeOf(obj.MasterAccount, m.AccountWallet.prototype);
                Object.setPrototypeOf(obj.MasterAccount.key, KeyWallet.prototype);

                Object.assign(this, obj);

                for (let i = 0; i < obj.MasterAccount.child.length; i++) {
                    Object.setPrototypeOf(obj.MasterAccount.child[i], m.AccountWallet.prototype);
    ...
    static RpcClient;
    static setProvider = async (url, user, password) => {
    	let m = await rpc;
    	Wallet.RpcClient = new m.RpcClient(url, user, password);
    }
