...
class Wallet {
    ...
    try {
      ...
      if (this.Storage) {
        ...
        try {
          ...
          const walletName = toLower(this?.Name);
          this.IsMasterless =
            includes(walletName, "masterless") ||
            includes(walletName, "unlinked");
          this.configWallet({
            passPhrase: aesKey,
            name: this.Name,
          ...
          console.log("set storage backup master key");
  ...
  getKeyStorageBackupMasterKey() {
    return `${MATER_KEYS_BACKUP_BY_PREFIX_SERVER_ID}-${this.Network}`;
  }
