...
import { setRandBytesFunc, hashSha3BytesToBytes, stringToBytes, bytesToString } from "./privacy/utils";
...
class Wallet {
  ...
  async save(password = "") {
    ...
    // encrypt
    let cipherText = await wasm.aesEncrypt(password + data);
    // storage
  ...
  async loadWallet(password) {
    if (this.Storage != null) {
      ...
      let data = await wasm.aesDecrypt(password + cipherText);
      let jsonStr = bytesToString(hexStringToByte(data));
      try {
