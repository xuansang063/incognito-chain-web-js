import _, { cloneDeep, isEqual, lowerCase, set, toString } from "lodash";
...
class Wallet {
  ...
  async getCreatedAccounts({ deserialize } = {}) {
    new Validator("deserialize", deserialize).required().boolean();
    let createdAccounts = [];
    for (const account of this.MasterAccount.child) {
      ...
      const newPrivateKey = newAccount.getPrivateKey();
      const oldPrivateKey = account.getPrivateKey();
  ...
  async measureCreateAccount(accountName, shardID, excludedIds) {
    try {
      ...
      const createdAccounts = await this.measureAsyncFn(
        () => this.getCreatedAccounts({ deserialize: true }),
  ...
  validatePrivateKey(privateKey) {
    ...
    for (let i = 0; i < this.MasterAccount.child.length; i++) {
      let account = this.MasterAccount.child[i];
      if (account.getPrivateKey() === privateKey) {
        existed = true;
        throw new CustomError(
          ErrorObject.ExistedAccountErr,
  ...
  async measureImportAccountWithId(accountId, accountName, index) {
    try {
      ...
      this.validatePrivateKey(account.getPrivateKey());
      this.MasterAccount.child.push(account);
      return account;
    } catch (error) {
      throw error;
    ...
    try {
      ...
      if (legacyEncryption) {
        ...
        cipherText = await this.measureAsyncFn(
          () => sjcl.encrypt(sjcl.codec.hex.toBits(password), data),
      ...
      set(this.measureStorage, "saveWallet.walletJSON", data);
      // storage
  ...
  async reImportPrototype(account) {
    try {
      ...
      await newAccount.key.KeySet.importFromPrivateKey(
        new Uint8Array(account.key.KeySet.PrivateKey)
      );
      return newAccount;
    } catch (error) {
    ...
    try {
      if (!this.measureStorage.walletName) {
        ...
        set(
          this.measureStorage,
          "walletMnemonic",
          this.ellipsisCenter({ str: this.Mnemonic })
        );
      }
