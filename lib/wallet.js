...
import { cachePromise, clearCache } from "@lib/utils/cache";
...
class Wallet {
  ...
  async save(password = "", legacyEncryption = false) {
    try {
      ...
      this.setStorageBackupMasterKey(this, { aesKey: password });
    }
  }
  ...
  async measureLoadWallet(passphrase) {
    try {
      ...
      if (this.Storage) {
        console.time("GET_CIPHER_TEXT");
        let cipherText = await this.Storage.getItem(this.Name);
        console.timeEnd("GET_CIPHER_TEXT");
        if (!cipherText) return false;
        let jsonStr;
        try {
          console.time("DECRYPT_BY_NEW_METHOD");
          jsonStr = sjcl.decrypt(sjcl.codec.hex.toBits(aesKey), cipherText);
          console.timeEnd("DECRYPT_BY_NEW_METHOD");
          if (!!jsonStr) {
        ...
        try {
          ...
          console.time("RE-IMPORT-PROTOYPE-MASTER-ACCOUNT");
          const masterAccount = await this.reImportPrototype(obj.MasterAccount);
          console.timeEnd("RE-IMPORT-PROTOYPE-MASTER-ACCOUNT");
          obj.MasterAccount = cloneDeep(masterAccount);
          console.time("RE-IMPORT-PROTOYPE-ACCOUNTS");
          let task = [
          ...
          console.timeEnd("RE-IMPORT-PROTOYPE-ACCOUNTS");
          ...
          this.setStorageBackupMasterKey(this, passphrase);
        }
      }
    } catch (error) {
      throw error;
  ...
  getKeyStorageBackupMasterKey() {
    return `${MATER_KEYS_BACKUP_BY_PREFIX_SERVER_ID}-${this.Network}-v4.2.17.65`;
  }
  ...
  async setStorageBackupMasterKey(wallet, passphrase) {
    try {
      ...
      const oldList =
        (await cachePromise(key, () =>
          this.getListStorageBackup(passphrase)
        )) || [];
