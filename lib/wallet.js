...
import { PaymentAddress, ViewingKey, OTAKey, PaymentInfo } from "./common/key";
...
import { Transactor as AccountWallet } from "./transactor";
import {
  getEstimateFee,
...
class Wallet {
  ...
  async reImportPrototype(account) {
    Object.setPrototypeOf(account, AccountWallet.prototype);
  ...
  setRpcHTTPCoinServiceClient(url) {
    Wallet.RpcCoinService = new RpcHTTPCoinServiceClient(url);
  }

  static PrivacyVersion;

  setPrivacyVersion(privacyVersion = 2) {
    Wallet.PrivacyVersion = privacyVersion;
  }
}
