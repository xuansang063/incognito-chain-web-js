...
const snippets = [
    ...
    'await senders[0].waitTx(tx0.Response.txId)',
    'let tx1 = await senders[0].prv({ transfer: { prvPayments: [new Inc.types.PaymentInfo(addresses[1], 8000)] , fee: 10 }})',
    'let tx2 = await senders[0].newToken({ transfer: { tokenPayments: [new Inc.types.PaymentInfo(addresses[1], 77000)] , fee: 50 }, extra: { tokenName: "My New Token", tokenSymbol: "MT1" }})',
...
let m = () => {
    ...
    .then(_ => {
        ...
        // .then(_senders => {
        //     return Promise.all(_senders.map(item => item.submitKeyAndSync()))
        //     .then(_ => _senders)
        // })
        // aggregate relevant variables and unfold them onto global
        .then(_senders => {
            _senders.map(s => s.isSubmitOtaKey = true);
            const res = {
                privateKeys,
