...
import * as aggRange from 'privacy-js-lib/lib/zkps/aggregatedrange';
import * as snNoPrivacy from 'privacy-js-lib/lib/zkps/snnoprivacy';
import * as snPrivacy from 'privacy-js-lib/lib/zkps/snprivacy';
import * as oneOfMany from 'privacy-js-lib/lib/zkps/oneoutofmany';
import * as pc from 'privacy-js-lib/lib/pedersen';
...
class PaymentWitness {
  ...
  init(hasPrivacy, spendingKey, inputCoins, outputCoins, pkLastByteSender, commitments, commitmentIndices, myCommitmentIndices, fee) {
    ...
    if (!hasPrivacy) {
      for (let i = 0; i < outputCoins.length; i++) {
        outputCoins[i].coinDetails.randomness = utils.randScalar();
  ...
  prove(hasPrivacy) {
    ...
    console.log("this.comInputSK: ", this.comInputSK);
...
class PaymentProof {
  ...
  toBytes() {
    ...
    partialBytes[2] = new Uint8Array(1 + snNoPrivacyArrLen + snNoPrivacyArrLen * constants.SN_NO_PRIVACY_PROOF_SIZE);
    ...
    for (let i = 0; i < constants.NUM_PROOF_PROPERTIES; i++) {
      // console.log("i: ", i, partialBytes[i])
